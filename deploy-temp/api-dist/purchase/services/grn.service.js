...
Object.defineProperty(exports, "GrnService", {
    enumerable: true,
    get: function() {
        return GrnService;
    }
...
let GrnService = class GrnService {
    ...
    async uploadInvoice(tenantId, userId, file) {
        if (!file) {
        ...
        if (!file.mimetype || !allowedTypes.has(file.mimetype)) {
            ...
            throw new _common.BadRequestException(`Unsupported file type: ${file.mimetype || 'unknown'}`);
        }
        ...
        if (typeof file.size === 'number' && file.size > maxSizeBytes) {
            ...
            throw new _common.BadRequestException('File too large (max 50MB)');
        }
        ...
        if (filePath && filePath.length > 0) {
            ...
            const relativePath = filePath.startsWith(uploadsRoot) ? filePath.slice(uploadsRoot.length) : filePath;
            const urlPath = relativePath.replace(/\\/g, '/');
            return {
                url: `/uploads${urlPath.startsWith('/') ? '' : '/'}${urlPath}`,
                name: file.originalname || file.filename || 'invoice',
                type: file.mimetype,
                size: file.size
            };
        }
        ...
        const safeExtension = extensionFromName && extensionFromName.length <= 10 ? extensionFromName : file.mimetype === 'application/pdf' ? '.pdf' : '';
        ...
        const targetPath = (0, _path.join)(targetDir, fileName);
    ...
    async uploadQcAttachment(tenantId, userId, file) {
        if (!file) {
            throw new _common.BadRequestException('No file uploaded');
        }
        const allowedTypes = new Set([
        ...
        if (filePath && filePath.length > 0) {
            ...
            return {
                url: `/uploads${urlPath.startsWith('/') ? '' : '/'}${urlPath}`,
                name: file.originalname || file.filename || 'qc',
                type: file.mimetype,
                size: file.size
            };
        }
    ...
    async create(tenantId, userId, data) {
        ...
        const { data: existingGRN } = await this.supabase.from('grns').select('id, grn_number').eq('tenant_id', tenantId).eq('po_id', data.poId).maybeSingle();
        if (existingGRN) {
            throw new _common.BadRequestException(`GRN already exists for this Purchase Order (${existingGRN.grn_number}). Cannot create multiple receipts for the same PO.`);
        }
        // Generate GRN number
        const grnNumber = await this.generateGRNNumber(tenantId);
        const { data: grn, error } = await this.supabase.from('grns').insert({
            tenant_id: tenantId,
        ...
        const { data: vendor } = await this.supabase.from('vendors').select('code, name').eq('id', data.vendorId).single();
        // Get warehouse details
        const { data: warehouse } = await this.supabase.from('warehouses').select('code, name').eq('id', data.warehouseId).single();
        // Insert GRN items
        if (data.items && data.items.length > 0) {
            ...
            const { error: itemsError } = await this.supabase.from('grn_items').insert(items);
    ...
    async findAll(tenantId, filters) {
        ...
        let query = this.supabase.from('grns').select(`
        *,
    ...
    async findOne(tenantId, id) {
        const { data, error } = await this.supabase.from('grns').select(`
        *,
        purchase_order:purchase_orders(id, po_number, po_date),
        vendor:vendors(id, code, name, contact_person),
        warehouse:warehouses(id, code, name),
      ...
      `).eq('tenant_id', tenantId).eq('id', id).single();
        if (error) throw new _common.NotFoundException('GRN not found');
        return data;
    }
    async update(tenantId, id, data) {
        const { error } = await this.supabase.from('grns').update({
        ...
        if (data.items) {
            await this.supabase.from('grn_items').delete().eq('grn_id', id);
            if (data.items.length > 0) {
                ...
                await this.supabase.from('grn_items').insert(items);
            }
        }
    ...
    async updateStatus(tenantId, id, status, userId) {
        ...
        if (!validStatuses.includes(status)) {
            throw new _common.BadRequestException(`Invalid status: ${status}. Must be one of: ${validStatuses.join(', ')}`);
        }
        // Get current GRN
        const grn = await this.findOne(tenantId, id);
        ...
        if (status === 'APPROVED') {
            ...
            if (!grn.qc_completed) {
                throw new _common.BadRequestException('Cannot approve GRN: QC inspection must be completed first. Please complete QC via the QC Accept action.');
            }
    ...
    async generateUIDsForItem(tenantId, userId, grn, grnItem) {
        try {
            ...
            const { data: item } = await this.supabase.from('items').select('id, code, name, category, uid_tracking, uid_strategy, batch_quantity, batch_uom').eq('code', grnItem.item_code).single();
            ...
            const { data: existingUIDs, count } = await this.supabase.from('uid_registry').select('uid', {
                count: 'exact'
            }).eq('grn_id', grn.id).eq('entity_id', item.id).eq('tenant_id', tenantId);
            if (count && count > 0) {
            ...
            if (uidsCreated.length > 0) {
                await this.supabase.from('grn_items').update({
    ...
    async delete(tenantId, id) {
        const { error } = await this.supabase.from('grns').delete().eq('tenant_id', tenantId).eq('id', id);
        if (error) throw new _common.BadRequestException(error.message);
        return {
            message: 'GRN deleted successfully'
        };
    }
    async qcAccept(tenantId, grnId, userId, body) {
        ...
        try {
            ...
            for (const item of body.items){
                ...
                const qcStatus = item.rejectedQty > 0 && item.acceptedQty > 0 ? 'PARTIAL' : item.rejectedQty > 0 ? 'REJECTED' : 'ACCEPTED';
                const updatePayload = {
                ...
                let { error } = await this.supabase.from('grn_items').update(updatePayload).eq('id', item.itemId);
                ...
                if (error && /qc_file_(url|name|type|size)/i.test(error.message || '')) {
                    ...
                    const retry = await this.supabase.from('grn_items').update(fallbackPayload).eq('id', item.itemId);
                    error = retry.error;
                }
                ...
                if (item.acceptedQty > 0) {
                    ...
                    const { data: grnItem, error: grnItemError } = await this.supabase.from('grn_items').select('item_id, grn_id, unit_price, batch_number, item_code').eq('id', item.itemId).single();
                    if (grnItemError || !grnItem) {
                        console.error(`Failed to retrieve GRN item details for id: ${item.itemId}`, grnItemError);
                        continue; // Skip to next item
                    }
                    const { data: grn } = await this.supabase.from('grns').select('warehouse_id, grn_number').eq('id', grnItem.grn_id).single();
                    if (!grn) {
                ...
                if (item.rejectedQty > 0) {
                    ...
                    const { data: grnItemData } = await this.supabase.from('grn_items').select('rate').eq('id', item.itemId).single();
                    if (grnItemData?.rate) {
            ...
            const { data: allItems } = await this.supabase.from('grn_items').select('qc_status').eq('grn_id', grnId);
            const allCompleted = allItems?.every((item)=>item.qc_status === 'ACCEPTED' || item.qc_status === 'REJECTED' || item.qc_status === 'PARTIAL');
            if (allCompleted) {
                console.log('All items QC completed, updating GRN qc_completed flag...');
                const { error: grnUpdateError } = await this.supabase.from('grns').update({
                    qc_completed: true,
                    updated_at: now
                }).eq('id', grnId).eq('tenant_id', tenantId);
            ...
            throw new _common.BadRequestException(`QC acceptance failed: ${error.message}`);
        }
    }
    // Auto-create debit note for rejected materials
    async createDebitNoteForRejections(tenantId, grnId, userId) {
        try {
            ...
            const { data: grn, error: grnError } = await this.supabase.from('grns').select(`
          id,
          vendor_id,
          grn_items (
            id,
        ...
        `).eq('id', grnId).eq('tenant_id', tenantId).single();
            ...
            for (const item of grn.grn_items || []){
                ...
                if (!item.rate && item.po_item_id) {
                    if (!poItemCache.has(item.po_item_id)) {
                        const { data: poItem } = await this.supabase.from('po_items').select('id, item_id, rate, unit_price').eq('id', item.po_item_id).maybeSingle();
            ...
            const totalAmount = rejectedItems.reduce((sum, item)=>sum + parseFloat(item.computed_amount), 0);
            // Generate debit note number
            const { data: dnNumber } = await this.supabase.rpc('generate_debit_note_number', {
                p_tenant_id: tenantId
            });
            // Create debit note
            const { data: debitNote, error: dnError } = await this.supabase.from('debit_notes').insert({
                tenant_id: tenantId,
            ...
            const dnItems = rejectedItems.map((item)=>({
            ...
            const { error: itemsError } = await this.supabase.from('debit_note_items').insert(dnItems);
            if (itemsError) {
            ...
            // Update GRN amounts
            const { data: grnAmounts } = await this.supabase.from('grn_items').select('unit_price, received_qty').eq('grn_id', grnId);
            const grossAmount = grnAmounts?.reduce((sum, item)=>sum + parseFloat(item.unit_price) * parseFloat(item.received_qty), 0) || 0;
            await this.supabase.from('grns').update({
    ...
    async createStockEntry(stockData) {
        try {
            ...
            const availableQuantity = stockData.available_quantity === undefined || stockData.available_quantity === null ? quantityChange : Number(stockData.available_quantity) || 0;
            const allocatedQuantity = Number(stockData.allocated_quantity ?? 0) || 0;
            const unitPrice = stockData.unit_price === undefined || stockData.unit_price === null ? null : Number(stockData.unit_price) || 0;
            const metadataFromCaller = stockData.metadata && typeof stockData.metadata === 'object' ? stockData.metadata : {};
            const metadata = {
                ...metadataFromCaller,
            ...
            const { data: item } = await this.supabase.from('items').select('category').eq('id', stockData.item_id).single();
    ...
    async generateUIDs(tenantId, grnItemId, data) {
        // Get GRN item details
        const { data: grnItem, error } = await this.supabase.from('grn_items').select('*').eq('id', grnItemId).single();
        if (error || !grnItem) {
            throw new _common.NotFoundException('GRN item not found');
        }
        // Call the stored function to generate UIDs
        const { data: result, error: generateError } = await this.supabase.rpc('generate_uids_for_grn_item', {
        ...
        if (generateError) {
            throw new _common.BadRequestException(generateError.message);
        }
        // Get generated UIDs
        const { data: uids } = await this.supabase.from('uids').select('*').eq('grn_item_id', grnItemId);
        return {
            grnItemId,
    ...
    async getUIDsByGRN(tenantId, grnId) {
        console.log('=== GET UIDs BY GRN ===');
        console.log('TenantId:', tenantId);
        console.log('GRN ID:', grnId);
        // First, get UIDs from uid_registry
        const { data: uidData, error: uidError } = await this.supabase.from('uid_registry').select('*').eq('tenant_id', tenantId).eq('grn_id', grnId).order('created_at', {
            ascending: false
        });
        if (uidError) {
            console.error('Error fetching UIDs:', uidError);
            throw new _common.BadRequestException(uidError.message);
        }
        ...
        const itemEntityIds = [
            ...new Set(uidData.filter((uid)=>uid.entity_type && uid.entity_id).map((uid)=>uid.entity_id))
        ].filter(Boolean);
        // Fetch items data if we have entity_ids
        let itemsMap = new Map();
        if (itemEntityIds.length > 0) {
            const { data: itemsData } = await this.supabase.from('items').select('id, code, name').in('id', itemEntityIds);
            itemsMap = new Map(itemsData?.map((item)=>[
                    item.id,
                    item
                ]) || []);
        }
    ...
    async updateGRNTotals(grnId) {
        // Get sum of quantities from items
        const { data: items } = await this.supabase.from('grn_items').select('received_qty, accepted_qty, rejected_qty').eq('grn_id', grnId);
    ...
    async generateGRNNumber(tenantId) {
        ...
        const { data } = await this.supabase.from('grns').select('grn_number').eq('tenant_id', tenantId).like('grn_number', `${prefix}%`).order('grn_number', {
            ascending: false
        }).limit(1).single();
        if (!data) {
            return `${prefix}-001`;
