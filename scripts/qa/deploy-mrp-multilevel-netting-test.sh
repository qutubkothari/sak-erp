...
stage=/tmp/mizantra-mrp-multilevel-netting-20260830
backup=/root/sak-deploy-backups/mizantra-mrp-multilevel-netting-$(date +%Y%m%d-%H%M%S)
...
if [ "$verified" != true ]; then
  ...
  pm2 restart sak-api-test sak-web-test
  echo 'MRP multi-level netting deployment failed health checks; previous test runtime restored.' >&2
  exit 1
fi

echo "Mizantra TEST multi-level MRP deployed and verified. Backup: $backup"
