...
export class AuthService {
  ...
  private readonly validationCache = new Map<string, { expiresAt: number; user: any }>();
  private readonly validationInFlight = new Map<string, Promise<any>>();
  private readonly validationCacheTtlMs = 3_000;
  ...
  async validateUser(userId: string, tenantId: string) {
    ...
    const inFlight = this.validationInFlight.get(cacheKey);
    if (inFlight) return inFlight;

    const request = this.loadValidatedUser(userId, tenantId)
      .then((user) => {
      ...
      .finally(() => this.validationInFlight.delete(cacheKey));

    this.validationInFlight.set(cacheKey, request);
    return request;
  }

  private async loadValidatedUser(userId: string, tenantId: string) {
    const { data: user, error: userError } = await this.supabase
      .from('users')
      .select(`
        id,
