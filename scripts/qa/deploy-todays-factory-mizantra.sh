...
stage=/tmp/mizantra-todays-factory-20260906-v1
release=todays-factory-20260906
backup="$app/backups/$release-$(date +%Y%m%d-%H%M%S)"
page=apps/web/src/app/dashboard/production/job-orders/page.tsx
...
for _ in $(seq 1 40); do
  status="$(curl -sS -o /dev/null -w '%{http_code}' http://127.0.0.1:3001/dashboard/production/job-orders || true)"
  [[ "$status" == 200 ]] && break
  sleep 2
done
[[ "$status" == 200 ]]
...
trap - ERR
rm -rf -- "$stage"
printf '{"deployed":true,"target":"mizantra-only","todaysFactory":%s,"backup":"%s"}\n' "$status" "$backup"
