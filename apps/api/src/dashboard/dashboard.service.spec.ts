...
describe('DashboardService reminder queue', () => {
  it('uses the same complete PO and QC definitions for dashboard consumers', async () => {
    ...
    const qcQuery = new QueryStub({
      data: [
        { id: 'grn-1', status: 'DRAFT', grn_items: [{ id: 'line-1' }] },
        { id: 'grn-cancelled', status: 'CANCELLED', grn_items: [] },
        { id: 'grn-empty', status: 'DRAFT', grn_items: [] },
      ],
      error: null,
    });
    ...
    await expect(service.getReminderQueue('tenant-1')).resolves.toEqual({
      pendingPOs: [{ id: 'po-1' }],
      pendingQC: [{ id: 'grn-1', status: 'DRAFT', grn_items: [{ id: 'line-1' }] }],
    });
