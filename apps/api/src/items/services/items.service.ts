...
export class ItemsService {
  ...
  constructor(
    ...
    private readonly drawingStorage: EngineeringDrawingStorageService,
  ) {}
  ...
  private async getCurrentStockTotals(tenantId: string, itemIds: string[]) {
    ...
    for (let index = 0; index < ids.length; index += 250) {
      ...
      if (error) {
        console.error(
          "[ItemsService.getCurrentStockTotals] inventory_stock lookup error:",
          error.message,
        );
      ...
      for (const row of data || []) {
        ...
        const quantity = Number(
          (row as any).available_quantity ?? (row as any).quantity ?? 0,
        ) || 0;
  ...
  async findAll(
    ...
    // Apply the R&D-only scope in PostgREST before its row limit is applied.
    // Filtering the first page in memory made older temporary items disappear
    // once a tenant had a large material catalogue.
    if (options.onlyRnd) {
    ...
    if (data && data.length > 0) {
      ...
      // The Stock Master only needs the live balance.  Replaying every stock
      // entry and movement for every listed item made the page slow as the
      // catalogue and transaction history grew.  Inventory_stock is the
      // authoritative operational balance used by adjustments and planning;
      // the detailed ledger calculation remains available in Stock Trail.
      const stockTotals = await this.getCurrentStockTotals(tenantId, itemIds);

      // Add live stock and resolved user names to each item.
      ...
      // them from the same live balance source used by inventory operations.
  ...
  async createRndTemporary(
    ...
    const { data: item, error: itemError } = await this.supabase
      ...
      .insert({
        tenant_id: tenantId,
        code,
        // Item name is a concise display/search label (database limit: 200).
        // Preserve the complete technical specification in description/metadata.
        name: description ? description.slice(0, 200) : identifier.slice(0, 200),
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      try {
        ...
        const validatedHsn = this.normalizeAndValidateHsn(rawHsn, {
          // Imported masters remain unverified until a checker approves them,
          // so HSN/SAC may be completed during that controlled review.
          required: false,
        });
        ...
        const resolvedCode = String(rawCode || "").trim()
          ? toUpperCode(rawCode)
          : await this.generateNextItemCode(tenantId, mappedCategory);
        ...
        const itemData = {
          ...
          selling_price: this.normalizeNumber(
            item.selling_price ||
              item.SellingPrice ||
              item["Selling Price"] ||
              item["Selling price"],
        ...
        if (error) {
          ...
          results.rows.push({
            ...
            message: existingRow?.id
              ? "Existing item matched by code and updated."
              : "Item created and sent for verification.",
          });
        }
      } catch (err: any) {
        results.failed++;
        ...
        results.rows.push({
          row: i + 2,
          code: String(item?.code || item?.Code || item?.["Item Code"] || "").trim() || null,
          name: String(item?.name || item?.Name || item?.["Item Name"] || ""),
          category: String(item?.category || item?.Category || item?.["Item Group"] || "").trim() || null,
          status: "FAILED",
          message: String(err?.message || "Import failed"),
        });
      }
    }
  ...
  async prepareDrawingUpload(
    ...
    const contentHash = String(
      drawingData?.contentHash || drawingData?.content_hash || "",
    )
      .trim()
      .toLowerCase();
    if (/^[a-f0-9]{64}$/.test(contentHash)) {
      const { data: existing, error } = await this.supabase
        .from("item_drawings")
        .select("id,file_name,file_type,file_size,version,is_active")
        .eq("tenant_id", tenantId)
        ...
        .limit(1)
        .maybeSingle();
      if (error) throw new Error(`Unable to check drawing duplicate: ${error.message}`);
      if (existing) {
        return {
          deduplicated: true,
          existingDrawingId: existing.id,
          existingDrawing: existing,
        };
      }
    }
    return this.drawingStorage.prepare(
      tenantId,
      itemId,
      drawingData?.fileName,
      drawingData?.fileSize,
    );
  }

  async getDrawingFileUrl(
    tenantId: string,
    itemId: string,
    drawingId: string,
    download = false,
    ...
    const { data: drawing, error } = await this.supabase
      ...
      .eq("id", drawingId)
      .single();
    if (error || !drawing) throw new NotFoundException("Drawing not found");
    await this.assertDrawingLinkedToItem(tenantId, drawingId, itemId);

    const storageReference = String(drawing.file_url || "");
    const storageParts = storageReference.startsWith("storage://")
      ? storageReference.slice("storage://".length).split("/")
      : [];
    const storageBucket = storageParts.shift() || "";
    const storagePath = storageParts.join("/").trim();
    if (!storagePath) {
      return { url: drawing.file_url, expiresIn: null };
    }

    return this.drawingStorage.createAccessUrl(
      storageBucket || this.drawingStorage.bucket,
      storagePath,
      drawing.file_name,
      download,
    );
  ...
  async getDrawings(tenantId: string, itemId: string) {
    await this.assertItemBelongsToTenant(tenantId, itemId);
    const [{ data: owned, error: ownedError }, { data: links, error: linkError }] =
      await Promise.all([
        this.supabase
          .from("item_drawings")
          .select("*")
    ...
    if (ownedError) throw new Error(`Failed to fetch drawings: ${ownedError.message}`);
    if (linkError) throw new Error(`Failed to fetch drawing links: ${linkError.message}`);

    const linkedIds = (links || []).map((row: any) => row.drawing_id).filter(Boolean);
    let linked: any[] = [];
    if (linkedIds.length > 0) {
      const { data, error } = await this.supabase
        ...
        .in("id", linkedIds)
        .neq("lifecycle_status", "DELETED");
      if (error) throw new Error(`Failed to fetch linked drawings: ${error.message}`);
      linked = data || [];
    }

    const byId = new Map<string, any>();
    for (const drawing of [...(owned || []), ...linked]) byId.set(drawing.id, drawing);
    const drawings = Array.from(byId.values()).sort(
      (a: any, b: any) => Number(b.version || 0) - Number(a.version || 0),
    );
    if (drawings.length === 0) return [];

    const drawingIds = drawings.map((drawing: any) => drawing.id);
    const { data: allLinks, error: allLinkError } = await this.supabase
      .from("engineering_drawing_item_links")
      .select("drawing_id,item_id")
      .eq("tenant_id", tenantId)
      .in("drawing_id", drawingIds);
    if (allLinkError) throw new Error(`Failed to fetch document coverage: ${allLinkError.message}`);
    const itemIds = Array.from(new Set((allLinks || []).map((row: any) => row.item_id)));
    ...
    if (itemError) throw new Error(`Failed to fetch linked items: ${itemError.message}`);
    const itemById = new Map((items || []).map((item: any) => [item.id, item]));
    return drawings.map((drawing: any) => ({
      ...drawing,
      linked_items: (allLinks || [])
        .filter((row: any) => row.drawing_id === drawing.id)
        .map((row: any) => itemById.get(row.item_id))
        .filter(Boolean),
      linked_item_codes: (allLinks || [])
        .filter((row: any) => row.drawing_id === drawing.id)
        .map((row: any) => itemById.get(row.item_id)?.code)
        .filter(Boolean),
    }));
  }

  async linkDrawingToItems(
    tenantId: string,
    userId: string,
    itemId: string,
    drawingId: string,
    ...
    await this.assertDrawingLinkedToItem(tenantId, drawingId, itemId);
    const { data: drawingOwner, error: drawingOwnerError } = await this.supabase
      .from("item_drawings")
      .select("item_id")
      .eq("tenant_id", tenantId)
      .eq("id", drawingId)
    ...
    const itemCodes = Array.from(
      new Set(
        (Array.isArray(body?.itemCodes) ? body.itemCodes : body?.linkedItemCodes || [])
          .map((value: any) => String(value || "").trim().toUpperCase())
          .filter(Boolean),
      ),
    );
    const explicitIds = (Array.isArray(body?.itemIds) ? body.itemIds : [])
    ...
    if (itemCodes.length > 0) {
      ...
      matchedItems = data || [];
      const matchedCodes = new Set(matchedItems.map((item: any) => String(item.code).toUpperCase()));
      const missing = itemCodes.filter((code: string) => !matchedCodes.has(code));
      if (missing.length > 0)
        throw new BadRequestException(`Item code(s) not found: ${missing.join(", ")}`);
    }
    if (explicitIds.length > 0) {
      const { data, error } = await this.supabase
        ...
        .in("id", explicitIds);
      if (error) throw new Error(error.message);
      if ((data || []).length !== new Set(explicitIds).size)
        throw new BadRequestException("One or more selected items were not found");
      matchedItems.push(...(data || []));
    }
    const ids = Array.from(new Set([itemId, ...matchedItems.map((item: any) => item.id)]));
    const rows = ids.map((linkedItemId) => ({
      tenant_id: tenantId,
      drawing_id: drawingId,
      item_id: linkedItemId,
      relation_type:
        linkedItemId === drawingOwner.item_id ? "OWNER" : "APPLICABLE",
      created_by: userId || null,
    }));
    const { error } = await this.supabase
      .from("engineering_drawing_item_links")
      .upsert(rows, { onConflict: "tenant_id,drawing_id,item_id", ignoreDuplicates: true });
    if (error) throw new Error(`Unable to link drawing to items: ${error.message}`);
    return this.getDrawings(tenantId, itemId);
  }

  private async assertDrawingLinkedToItem(
    tenantId: string,
    drawingId: string,
    itemId: string,
  ) {
    const { data: drawing, error } = await this.supabase
      ...
      .maybeSingle();
    if (error || !drawing) throw new NotFoundException("Drawing not found");
    if (drawing.item_id === itemId) return;
    const { data: link, error: linkError } = await this.supabase
      ...
      .eq("item_id", itemId)
      .maybeSingle();
    if (linkError || !link) throw new NotFoundException("Drawing is not linked to this item");
  }
  ...
  async uploadDrawing(
    ...
    const existingDrawingId = String(
      drawingData?.existingDrawingId || drawingData?.existing_drawing_id || "",
    ).trim();
    if (existingDrawingId) {
      await this.assertDrawingLinkedToItem(tenantId, existingDrawingId, itemId).catch(
        async () => {
          const { data, error } = await this.supabase
            .from("item_drawings")
            .select("id")
            .eq("tenant_id", tenantId)
            .eq("id", existingDrawingId)
            .maybeSingle();
          if (error || !data) throw new NotFoundException("Existing shared drawing not found");
          const { error: ownerLinkError } = await this.supabase
            .from("engineering_drawing_item_links")
            .upsert(
              {
                tenant_id: tenantId,
                drawing_id: existingDrawingId,
                item_id: itemId,
                relation_type: "APPLICABLE",
                created_by: userId || null,
              },
              { onConflict: "tenant_id,drawing_id,item_id", ignoreDuplicates: true },
            );
          if (ownerLinkError) throw new Error(ownerLinkError.message);
        },
      );
      await this.linkDrawingToItems(tenantId, userId, itemId, existingDrawingId, drawingData);
    ...
    const storagePath = String(
      drawingData?.storagePath || drawingData?.storage_path || "",
    ).trim();
    const storageBucket = String(
      drawingData?.storageBucket ||
        drawingData?.storage_bucket ||
        this.drawingStorage.bucket,
    ).trim();

    if (storagePath) {
      if (storageBucket !== this.drawingStorage.bucket) {
        throw new BadRequestException("Invalid drawing storage bucket");
      }
      const validated = this.drawingStorage.validateFile(
        resolvedFileName,
        resolvedFileSize,
      );
      await this.drawingStorage.verify(
        tenantId,
        itemId,
        storagePath,
        validated.size,
      );
      resolvedFileUrl = `storage://${storageBucket}/${storagePath}`;
    }
    ...
    const revisionCode = String(
      drawingData?.revisionCode || drawingData?.revision_code || `R${nextVersion}`,
    ).trim().toUpperCase();
    const extension = String(resolvedFileName || "").toLowerCase().split(".").pop() || "";
    const inferredRole = ["step", "stp", "dwg", "dxf", "iges", "igs", "ifc", "stl", "rvt", "sldprt", "sldasm", "ipt", "iam"].includes(extension)
      ? "NATIVE_CAD"
      : ["pdf", "png", "jpg", "jpeg", "tif", "tiff", "webp"].includes(extension)
        ? "CONTROLLED_2D"
        : "SUPPORTING";
    ...
    const baseInsert: any = {
      ...
      is_active: false,
      uploaded_by: userId,
      content_hash: /^[a-f0-9]{64}$/i.test(
        String(drawingData?.contentHash || drawingData?.content_hash || ""),
      )
        ? String(drawingData?.contentHash || drawingData?.content_hash).toLowerCase()
        : null,
    };
    ...
    const { error: packageLinkError } = await this.supabase
      .from("item_drawings")
      .update({ revision_package_id: revisionPackage.id })
      .eq("tenant_id", tenantId)
      .eq("id", data.id);
    ...
    await this.linkDrawingToItems(tenantId, userId, itemId, data.id, drawingData);
  ...
  async transitionDrawing(
    ...
    const { data, error } = await this.supabase
      ...
      .single();
    if (error) throw new Error(error.message);
    if (drawing.revision_package_id) {
      const { error: packageFilesError } = await this.supabase
        .from("item_drawings")
        .update(payload)
        .eq("tenant_id", tenantId)
        .eq("revision_package_id", drawing.revision_package_id);
      if (packageFilesError) throw new Error(packageFilesError.message);
      const packagePayload: any = {
        lifecycle_status: payload.lifecycle_status,
        updated_at: now,
      };
      if (normalizedAction === "APPROVE") {
        packagePayload.approved_by = userId;
        packagePayload.approved_at = now;
      }
      const { error: packageStatusError } = await this.supabase
        .from("engineering_drawing_revision_packages")
        .update(packagePayload)
        .eq("tenant_id", tenantId)
        .eq("id", drawing.revision_package_id);
  ...
  async deleteDrawing(
    ...
    const { data: drawing, error: findError } = await this.supabase
      .from("item_drawings")
      .select("id,item_id,file_name,drawing_number,revision_code,lifecycle_status,is_active,metadata")
      .eq("tenant_id", tenantId)
      .eq("id", drawingId)
      .maybeSingle();
    if (findError) {
      throw new Error(`Unable to resolve drawing for deletion: ${findError.message}`);
    }
    if (!drawing || drawing.lifecycle_status === "DELETED") {
      throw new NotFoundException("Drawing not found");
    }
    ...
    const { error } = await this.supabase
      ...
      .update({
        ...
        metadata: {
          ...(drawing.metadata && typeof drawing.metadata === "object"
            ? drawing.metadata
            : {}),
          deleted_at: deletedAt,
          deleted_by: userId || null,
          deleted_from_item_id: itemId,
        },
      })
      .eq("tenant_id", tenantId)
      .eq("id", drawingId)
    ...
    // Preserve the physical file and all frozen PO/job references. The audit
    // row is best-effort because older deployments may not have every audit
    // column, while the deletion identity also remains in drawing metadata.
    await this.supabase.from("activity_logs").insert({
      tenant_id: tenantId,
      user_id: userId || null,
      action: "DELETE",
      resource_type: "ENGINEERING_DRAWING",
      resource_id: drawingId,
      resource_code: drawing.drawing_number || null,
      resource_name: drawing.file_name || null,
      old_value: {
        lifecycle_status: drawing.lifecycle_status,
        is_active: drawing.is_active,
        revision_code: drawing.revision_code,
      },
      new_value: { lifecycle_status: "DELETED", is_active: false },
      metadata: { item_id: itemId, preserved_for_audit: true },
    });
