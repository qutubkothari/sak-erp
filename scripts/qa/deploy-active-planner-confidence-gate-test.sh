...
stage=/tmp/mizantra-active-planner-confidence-gate
backup=/root/sak-deploy-backups/mizantra-active-planner-confidence-gate-$(date +%Y%m%d-%H%M%S)
name=semantic-erp-query.service
...
rollback() {
  echo 'Semantic-confidence deployment failed; restoring Mizantra API.' >&2
