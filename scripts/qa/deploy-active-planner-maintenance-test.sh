...
stage=/tmp/mizantra-active-planner-maintenance-20260830
backup=/root/sak-deploy-backups/mizantra-active-planner-maintenance-$(date +%Y%m%d-%H%M%S)
...
paths=(
  ...
  scripts/qa/deploy-active-planner-maintenance-test.sh
)
...
if [ "$verified" != true ]; then
  ...
  pm2 restart sak-api-test
  echo 'Maintenance prompt deployment failed; previous Mizantra test API restored.' >&2
  exit 1
fi

pm2 save
echo "Mizantra TEST maintenance prompt deployed and verified. Backup: $backup"
