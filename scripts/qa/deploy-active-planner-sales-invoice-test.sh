...
stage=/tmp/mizantra-active-planner-sales-invoice-20260830
backup=/root/sak-deploy-backups/mizantra-active-planner-sales-invoice-$(date +%Y%m%d-%H%M%S)
...
paths=(
  ...
  scripts/qa/deploy-active-planner-sales-invoice-test.sh
)
...
if [ "$verified" != true ]; then
  ...
  pm2 restart sak-api-test
  echo 'Governed sales-invoice deployment failed; previous Mizantra test API restored.' >&2
  exit 1
fi

pm2 save
echo "Mizantra TEST governed sales-invoice prompt deployed and verified. Backup: $backup"
