...
const _supabasestorageservice = require("./supabase-storage.service");
const _openaidocumentservice = require("./openai-document.service");
const _quotepdfservice = require("./quote-pdf.service");
...
let DocumentsService = class DocumentsService {
    async uploadBufferAsFile(params) {
        const fakeFile = {
            fieldname: 'file',
    ...
    async generateQuote(req, dto) {
        ...
        if (!quoteNumber) {
            ...
            const { data: lastDoc } = await this.supabase.from('documents').select('document_number').eq('tenant_id', tenantId).like('document_number', 'QTE-%').order('document_number', {
                ascending: false
            }).limit(1).single();
            const lastNumber = lastDoc?.document_number ? parseInt(String(lastDoc.document_number).split('-')[1]) : 0;
        ...
        try {
            ...
            if (extractedText) {
                ...
                if (this.openaiDocumentService.isEnabled()) {
                    const analysis = await this.openaiDocumentService.analyzeDocumentFromText(extractedText, fileName);
                ...
                await this.supabase.from('documents').update(updatePayload).eq('tenant_id', tenantId).eq('id', doc.id);
            }
        } catch (error) {
    ...
    async tryExtractTextFromFile(file) {
        const mime = (file?.mimetype || '').toLowerCase();
        if (mime === 'application/pdf') {
            try {
                const mod = await Promise.resolve().then(()=>/*#__PURE__*/ _interop_require_wildcard(require("pdf-parse")));
    ...
    guessMimeType(fileName, fileType) {
    ...
    async getDocumentForTenant(tenantId, id) {
        ...
        for(let attempt = 0; attempt < 3; attempt++){
            ...
            const { data, error } = await this.supabase.from('documents').select(columns.join(', ')).eq('tenant_id', tenantId).eq('id', id).single();
            if (!error) {
    ...
    async getSignedUrl(req, id, expiresIn) {
        const tenantId = req.user.tenantId;
        const doc = await this.getDocumentForTenant(tenantId, id);
        const requested = Number(expiresIn);
        const safeExpiresIn = Number.isFinite(requested) ? Math.min(60 * 60, Math.max(60, Math.floor(requested))) : 15 * 60;
        if (doc.file_path) {
    ...
    async reanalyze(req, id) {
        ...
        if (mime === 'application/pdf') {
            ...
            const urlToUse = filePath ? await this.storageService.getSignedUrl(filePath, 15 * 60) : fileUrl;
            if (!urlToUse) {
            ...
            const analysis = await this.openaiDocumentService.analyzeDocumentFromText(String(doc.ocr_text), fileName);
            updatePayload.ai_classification = analysis.classification;
            updatePayload.ai_extracted_data = analysis.extractedData;
            updatePayload.ai_suggested_tags = analysis.suggestedTags;
        }
        if (Object.keys(updatePayload).length === 0) {
            return {
                ok: true,
                message: 'Nothing to reanalyze (no supported file type or no extracted text available).'
            };
        }
        const { data, error } = await this.supabase.from('documents').update(updatePayload).eq('tenant_id', tenantId).eq('id', id).select().single();
        if (error) throw new Error(error.message);
        return data;
    }
    async qualityCheck(req, id) {
        try {
            const tenantId = req.user.tenantId;
            const doc = await this.getDocumentForTenant(tenantId, id);
            if (!this.openaiDocumentService.isEnabled()) {
                throw new _common.ServiceUnavailableException('OpenAI not configured; quality check is unavailable');
            }
            const mime = this.guessMimeType(doc.file_name, doc.file_type);
            const fileName = doc.file_name || 'document';
            ...
            if (!text) {
                if (mime === 'application/pdf' && doc.file_path) {
                    ...
                    const urlToUse = doc.file_path ? await this.storageService.getSignedUrl(doc.file_path, 15 * 60) : doc.file_url;
                    if (urlToUse) {
            ...
            const existingMetadata = doc.metadata && typeof doc.metadata === 'object' ? doc.metadata : {};
            const newMetadata = {
                ...existingMetadata,
                quality_check: result,
                quality_checked_at: qualityCheckedAt
            };
            let { error } = await this.supabase.from('documents').update({
                ...extractionUpdate,
            ...
            if (error && String(error.message || '').toLowerCase().includes('documents.metadata')) {
                ({ error } = await this.supabase.from('documents').update({
                    ...extractionUpdate,
                    ai_extracted_data: {
                        quality_check: result,
                        quality_checked_at: qualityCheckedAt
            ...
            if (error) {
                ...
                if (msg.includes("could not find the") || msg.includes('does not exist')) {
                    this.logger.warn(`Quality check result not persisted: ${error.message}`);
                } else {
                    throw new Error(error.message);
                }
            }
            ...
            if (error instanceof _common.ServiceUnavailableException) {
                throw error; // Let NestJS handle this properly
            }
            throw new Error(`Quality check failed: ${error.message || 'Unknown error'}`);
        }
    }
    async uploadAndCreate(req, file, body) {
        if (!file) {
            throw new Error('File is required');
        }
        const entityTypeRaw = body.related_entity_type || body.entityType || body.entity_type || 'general';
        const entityIdRaw = body.related_entity_id || body.entityId || body.entity_id || 'unlinked';
        ...
        const tags = Array.isArray(tagsValue) ? tagsValue : typeof tagsValue === 'string' && tagsValue.trim().length > 0 ? tagsValue.split(',').map((t)=>t.trim()).filter(Boolean) : [];
        ...
        try {
            ...
            if (Object.keys(updatePayload).length > 0) {
                const { error } = await this.supabase.from('documents').update(updatePayload).eq('tenant_id', req.user.tenantId).eq('id', doc.id);
                if (error) {
    ...
    async uploadAndAddRevision(req, documentId, file, body) {
        if (!file) {
            throw new Error('File is required');
        }
        const tenantId = req.user.tenantId;
        ...
        const revisionNumber = body.revision_number || body.revisionNumber || body.revision || null;
        if (!revisionNumber) {
        ...
        try {
            ...
            if (Object.keys(updatePayload).length > 0) {
                await this.supabase.from('documents').update(updatePayload).eq('tenant_id', tenantId).eq('id', documentId);
            }
        } catch (error) {
            this.logger.warn(`Enrichment skipped/failed (revision upload): ${error?.message || error}`);
        }
        return revision;
    }
    async create(req, createDto) {
        ...
        if (!createDto.document_number) {
            const { data: lastDoc } = await this.supabase.from('documents').select('document_number').eq('tenant_id', tenantId).like('document_number', 'DOC-%').order('document_number', {
                ascending: false
            }).limit(1).single();
            const lastNumber = lastDoc?.document_number ? parseInt(lastDoc.document_number.split('-')[1]) : 0;
            createDto.document_number = `DOC-${String(lastNumber + 1).padStart(6, '0')}`;
        }
        const buildInsertRow = (payload)=>({
                ...payload,
                tenant_id: tenantId,
                created_by: req.user.userId,
        ...
        const stripKeyIfPresent = (obj, key)=>{
        ...
        for(let attempt = 0; attempt < 4; attempt++){
            // eslint-disable-next-line no-await-in-loop
            const { data: inserted, error } = await this.supabase.from('documents').insert([
                buildInsertRow(insertPayload)
            ]).select().single();
            if (!error) {
                data = inserted;
                break;
    ...
    async findOne(req, id) {
      ...
      `;
        let { data, error } = await this.supabase.from('documents').select(selectWithFileType).eq('tenant_id', tenantId).eq('id', id).single();
        if (error && String(error.message || '').toLowerCase().includes('file_type')) {
            ({ data, error } = await this.supabase.from('documents').select(selectWithoutFileType).eq('tenant_id', tenantId).eq('id', id).single());
        }
        if (error) throw new Error(error.message);
        // Log access
        await this.logAccess(tenantId, id, req.user.userId, 'VIEW');
    ...
    async update(req, id, updateDto) {
        const tenantId = req.user.tenantId;
        const { data, error } = await this.supabase.from('documents').update(updateDto).eq('tenant_id', tenantId).eq('id', id).select().single();
        if (error) throw new Error(error.message);
        // Log access
    ...
    async delete(req, id) {
        ...
        const tryDeleteByDocumentId = async (table)=>{
            // Some deployments don't have `tenant_id` on child tables, and some don't have `document_access_logs` at all.
            let { error } = await this.supabase.from(table).delete().eq('tenant_id', tenantId).eq('document_id', id);
            const msg = String(error?.message || '').toLowerCase();
        ...
        const { error } = await this.supabase.from('documents').delete().eq('tenant_id', tenantId).eq('id', id);
        if (error) throw new Error(error.message);
        return {
            message: 'Document deleted successfully'
        };
    }
    async createRevision(tenantId, documentId, revisionDto) {
        const insertPayload = {
            ...revisionDto,
            tenant_id: tenantId,
        ...
        for(let attempt = 0; attempt < 5; attempt++){
            // eslint-disable-next-line no-await-in-loop
            const { data: inserted, error } = await this.supabase.from('document_revisions').insert([
                insertPayload
            ]).select().single();
            if (!error) {
                data = inserted;
        ...
        if (revisionDto.status === 'ACTIVE') {
            let { error } = await this.supabase.from('document_revisions').update({
                status: 'SUPERSEDED'
            }).eq('tenant_id', tenantId).eq('document_id', documentId).neq('id', data.id);
            if (error && String(error.message || '').toLowerCase().includes('tenant_id')) {
                ({ error } = await this.supabase.from('document_revisions').update({
                    status: 'SUPERSEDED'
                }).eq('document_id', documentId).neq('id', data.id));
            }
            if (error) {
    ...
    async addRevision(req, id, revisionDto) {
        const tenantId = req.user.tenantId;
        // Get current document
        const { data: doc } = await this.supabase.from('documents').select('current_revision').eq('tenant_id', tenantId).eq('id', id).single();
    ...
    async submitForApproval(req, id, approvalWorkflow) {
        const tenantId = req.user.tenantId;
        ...
        const approvalRecords = approvalWorkflow.approvers.map((approver, index)=>({
                tenant_id: tenantId,
                document_id: id,
        ...
        const { data, error } = await this.supabase.from('document_approvals').insert(approvalRecords).select();
        if (error) throw new Error(error.message);
        return data;
    }
    async approveDocument(req, id, approvalId, comments) {
        const tenantId = req.user.tenantId;
        // Update approval record
        await this.supabase.from('document_approvals').update({
            status: 'APPROVED',
            comments,
            responded_at: new Date().toISOString()
        }).eq('tenant_id', tenantId).eq('id', approvalId).eq('approver_user_id', req.user.userId);
        // Check if all approvals are complete
        const { data: allApprovals } = await this.supabase.from('document_approvals').select('status, is_mandatory').eq('tenant_id', tenantId).eq('document_id', id);
        const allApproved = allApprovals?.every((a)=>!a.is_mandatory || a.status === 'APPROVED' || a.status === 'SKIPPED');
        if (allApproved) {
    ...
    async rejectDocument(req, id, approvalId, reason) {
        const tenantId = req.user.tenantId;
        // Update approval record
        await this.supabase.from('document_approvals').update({
            status: 'REJECTED',
    ...
    async archiveDocument(req, id) {
        const tenantId = req.user.tenantId;
        const { data, error } = await this.supabase.from('documents').update({
            status: 'ARCHIVED',
    ...
    async logAccess(tenantId, documentId, userId, action) {
    ...
    async getAccessLogs(req, documentId) {
        const tenantId = req.user.tenantId;
        const { data, error } = await this.supabase.from('document_access_logs').select(`
        *,
        user:users(id, first_name, last_name, email)
      `).eq('tenant_id', tenantId).eq('document_id', documentId).order('created_at', {
            ascending: false
        });
        if (error) {
            const msg = String(error.message || '').toLowerCase();
    ...
    constructor(storageService, openaiDocumentService, quotePdfService){
        this.storageService = storageService;
        this.openaiDocumentService = openaiDocumentService;
        this.quotePdfService = quotePdfService;
        this.logger = new _common.Logger(DocumentsService.name);
...
DocumentsService = _ts_decorate([
    ...
    _ts_metadata("design:paramtypes", [
        typeof _supabasestorageservice.SupabaseStorageService === "undefined" ? Object : _supabasestorageservice.SupabaseStorageService,
        typeof _openaidocumentservice.OpenAIDocumentService === "undefined" ? Object : _openaidocumentservice.OpenAIDocumentService,
        typeof _quotepdfservice.QuotePdfService === "undefined" ? Object : _quotepdfservice.QuotePdfService
    ])
], DocumentsService);
