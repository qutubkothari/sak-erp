...
function buildPayload(form: VendorFormState) {
  ...
  return {
    ...
    taxId: form.isGstUnregistered ? "" : form.taxId.trim().toUpperCase(),
    gstVerification: form.isGstUnregistered ? null : form.gstVerification,
...
/** Catches multiple numbers typed into one phone field (e.g. "9876543210, 9123456789") before it ever reaches the server. */
function findInvalidContactPhone(
  contacts: VendorFormState["contacts"],
): string | null {
  for (const contact of contacts) {
    ...
    if (digits.length < 7 || digits.length > 15) {
      ...
      return `${label}: "${raw}" is not a valid phone number. Enter only one phone number (7–15 digits).`;
    }
  }
  return null;
}
...
export default function VendorsPage() {
  ...
  const openEdit = (vendor: Vendor) => {
    ...
    setForm({
      ...
      isGstUnregistered: vendor.is_gst_unregistered === true,
  ...
  const handleSave = async () => {
    ...
    const invalidPhone = findInvalidContactPhone(payload.contacts);
    if (invalidPhone) {
      setFormSection("contacts");
        ...
        <ListTable
          ...
          emptyState={
            <ErpWorkspaceState
              ...
              description="Adjust the supplier filters, or create a new vendor master record."
              action={
                canCreate ? (
                  <ErpButton variant="primary" onClick={openCreate}>
                    <Plus className="h-4 w-4" />
            ...
            <section>
              ...
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                <label className="md:col-span-2 xl:col-span-4 flex min-h-10 items-center gap-3 rounded-md border border-[#E8DCC4] bg-[#FFFCF7] px-3">
                  <input
                    type="checkbox"
                    checked={form.isGstUnregistered}
                    onChange={(event) =>
                      setForm({
                        ...form,
                        isGstUnregistered: event.target.checked,
                        ...(event.target.checked
                          ? { taxId: "", gstVerification: null }
                          : {}),
                      })
                    }
                  />
                ...
                <div className="md:col-span-2">
                  ...
                  <div className="flex gap-2">
                    <input
                      className={inputClass}
                      value={form.taxId}
                      disabled={form.isGstUnregistered}
                      onChange={(event) =>
                        setForm({
                          ...form,
