...
stage=/tmp/mizantra-mrp-forecast-consumption-20260830
backup=/root/sak-deploy-backups/mizantra-mrp-forecast-consumption-$(date +%Y%m%d-%H%M%S)
...
if [ "$verified" != true ]; then
  ...
  pm2 restart sak-api-test sak-web-test
  echo 'MRP forecast-consumption deployment failed health checks; previous test runtime restored.' >&2
  exit 1
fi

echo "Mizantra TEST MRP forecast consumption deployed and verified. Backup: $backup"
