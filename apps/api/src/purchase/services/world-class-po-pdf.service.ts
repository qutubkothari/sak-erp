...
import {
  ...
  PdfBrandingAssets,
} from "../../common/services/document-branding.service";
...
export function parseSupabaseStorageReference(fileUrl: string): {
  bucket: string;
  path: string;
} | null {
  ...
  const parts = value.slice("storage://".length).split("/");
...
export function resolveVendorContactSalutation(vendor: any): string {
  const metadata =
    vendor?.metadata &&
    typeof vendor.metadata === "object" &&
    !Array.isArray(vendor.metadata)
      ? vendor.metadata
      : {};
  const contacts = Array.isArray(metadata.contacts) ? metadata.contacts : [];
  const contactName = String(vendor?.contact_person || "")
    .trim()
    .toLocaleLowerCase();
  const matchingContact = contacts.find(
    (contact: any) =>
      ...
      String(contact?.name || "")
        .trim()
        .toLocaleLowerCase() === contactName,
  );
  const defaultContact = contacts.find(
    (contact: any) =>
      contact?.isDefault === true || contact?.is_default === true,
  );
  return String(
    matchingContact?.salutation ||
      defaultContact?.salutation ||
      metadata.salutation ||
      "",
  ).trim();
}

export function formatVendorContactName(
  salutation?: string,
  contactName?: string,
): string {
...
/**
 * Standard PDF fonts use WinAnsi encoding and throw when user-entered text
 * contains characters outside that character set. Keep common business
 * symbols readable and degrade unknown symbols safely instead of failing the
 * entire document download.
 */
export function makePdfTextSafe(
  value: unknown,
  font: Pick<PDFFont, "encodeText">,
): string {
  ...
  for (const character of source) {
    ...
    const replacement = PDF_TEXT_REPLACEMENTS[character] ?? character;
...
const ROW_ALT = rgb(0.96, 0.965, 0.975);
...
export class WorldClassPoPdfService {
  ...
  private getStorageClient(): SupabaseClient {
    ...
    this.storageClient = createClient(url, key);
  ...
  async generatePOPdf(tenantId: string, data: POPdfData): Promise<Buffer> {
    ...
    const orderTypeStr = data.isServiceOrder
      ? "Service Order"
      : "Purchase Order";
    ...
    const pages: PDFPage[] = [];
    const branding = await this.documentBrandingService.getBranding(
      tenantId || undefined,
      DEFAULT_PO_ISSUER,
    );
    const pdfSafeBranding = this.makePdfValueSafe(
      branding,
      font,
    ) as DocumentBranding;
    const assets = await this.documentBrandingService.preparePdfBrandingAssets(
      pdfDoc,
      branding,
    );
    const normalizedData = this.withRoundedGrandTotal({
      ...data,
      companyName: pdfSafeBranding.companyName,
      companyAddress: pdfSafeBranding.address || data.companyAddress,
      companyEmail: pdfSafeBranding.email || data.companyEmail,
      companyPhone: pdfSafeBranding.phone || data.companyPhone,
      companyWebsite: pdfSafeBranding.website || data.companyWebsite,
      companyGSTIN: pdfSafeBranding.taxId || data.companyGSTIN,
    });
    const pdfSafeData = this.makePdfValueSafe(
      normalizedData,
      font,
    ) as POPdfData;
    pdfDoc.setAuthor(pdfSafeBranding.companyName);
  ...
  async loadFileBuffer(fileUrl: string): Promise<Buffer> {
    ...
    const storageReference = parseSupabaseStorageReference(cleanUrl);
    if (storageReference) {
      const { data, error } = await this.getStorageClient()
        .storage.from(storageReference.bucket)
        .download(storageReference.path);
      if (error || !data) {
        throw new Error(
          `Unable to load Supabase Storage object: ${error?.message || "No file returned"}`,
        );
      }
    ...
    if (cleanUrl.startsWith("data:")) {
      ...
      return Buffer.from(
        cleanUrl.slice(commaIndex + 1),
        cleanUrl.slice(0, commaIndex).includes(";base64") ? "base64" : "utf8",
      );
    }
    ...
    const localPath = localCandidates.find((candidate) => {
      ...
      return (
        ...
        (resolvedCandidate.startsWith(resolvedUploadsRoot) ||
          isAbsolute(cleanUrl))
      );
    });
  ...
  private repairPdfCatalogAndXrefForPdfLib(pdfBuffer: Buffer): Buffer {
    ...
    const rootObjectPattern = new RegExp(
      `${rootObjNo}\\s+${rootGenNo}\\s+obj\\s*<<([\\s\\S]*?)>>`,
    );
    ...
    body = body.replace(
      ...
      (full, objNoText, genNoText, dict, terminator) => {
        ...
        const isPage =
          /\/MediaBox\s*\[/.test(nextDict) &&
          /\/Parent\s+\d+\s+\d+\s+R/.test(nextDict);
    ...
    const maxObjectNo = Math.max(
      0,
      ...Object.keys(offsets).map((value) => Number(value)),
    );
    let xref = `xref\n0 ${maxObjectNo + 1}\n`;
    for (let index = 0; index <= maxObjectNo; index += 1) {
      xref +=
        index === 0 || offsets[index] === undefined
          ? "0000000000 65535 f \n"
  ...
  private async loadPdfDocumentForAppend(
    pdfBuffer: Buffer,
  ): Promise<PDFDocument> {
    try {
    ...
    for (const drawing of drawings) {
      ...
      try {
        ...
        if (isPdf) {
          ...
          console.warn(
            `[PoPdf] Unsupported drawing type: ${drawing.file_type} (${drawing.file_name})`,
          );
        }
      } catch (err: any) {
        console.warn(
          `[PoPdf] Failed to embed drawing "${drawing.file_name}":`,
          err?.message || err,
        );
      }
    }
  ...
  private estimateSummaryAndTermsH(ctx: any): number {
    ...
    const leftW = Math.round(USABLE * 0.6);

    // --- Summary block (mirrors drawSummaryBlock logic) ---
    const rowCount =
      5 + // Subtotal, Discount, Taxable, CGST, SGST
      (data.igstTotal ? 1 : 0) +
      (data.roundOff ? 1 : 0) +
      1; // Grand Total
    ...
    const statLines = [`GST No : ${data.companyGSTIN || "-"}`];
    const termLines = [
  ...
  private async newPage(ctx: any): Promise<PageState> {
    const page = await this.documentBrandingService.createBrandedPage(
      ctx.pdfDoc,
      ctx.assets,
      [PAGE_W, PAGE_H],
    );
  ...
  private async drawReferenceStylePurchaseOrder(
    state: PageState,
    ctx: any,
  ): Promise<PageState> {
  ...
  private drawReferenceHeader(state: PageState, ctx: any): PageState {
    ...
    const addressLines = (
      branding.addressLines?.length
        ...
        : this.wrapText(data.companyAddress || "", USABLE, 8, font)
    ).slice(0, 3);
    for (const line of addressLines) {
      const safe = this.fitText(line, USABLE, font, 8);
      page.drawText(safe, {
        x: centerX - font.widthOfTextAtSize(safe, 8) / 2,
        y,
        size: 8,
        font,
        color: GRAY,
    ...
    if (contactParts.length) {
      for (const part of contactParts) {
        page.drawText(part, {
          x: centerX - font.widthOfTextAtSize(part, 7.5) / 2,
          y,
          size: 7.5,
          font,
          color: DARK,
  ...
  private drawReferenceMeta(state: PageState, ctx: any): PageState {
    ...
    page.drawLine({
      start: { x: MX + leftW, y: top },
      end: { x: MX + leftW, y: top - h },
      thickness: 0.4,
      color: BORDER,
    });
    ...
    rows.forEach((row, index) => {
      ...
      if (index > 0) {
        page.drawLine({
          start: { x: MX, y: top - rowH * index },
          end: { x: PAGE_W - MX, y: top - rowH * index },
          thickness: 0.35,
          color: BORDER,
        });
      }
      page.drawText(this.fitText(row[0], leftW - 12, font, 7.5), {
        x: MX + 6,
        y,
        size: 7.5,
        font: row[0].includes(data.poNumber || "###") ? fontBold : font,
        color: DARK,
      });
      page.drawText(this.fitText(row[1], USABLE - leftW - 12, font, 7.5), {
        x: MX + leftW + 6,
        y,
        size: 7.5,
        font,
  ...
  private drawReferencePartyBlocks(state: PageState, ctx: any): PageState {
    ...
    const leftLines = [
      ...
      [data.vendorCity, data.vendorState, data.vendorPincode]
        .filter(Boolean)
        .join(", "),
      ...
      data.vendorContactPerson
        ? `Name : ${formatVendorContactName(data.vendorSalutation, data.vendorContactPerson)}`
        : "",
      data.vendorPhone ? `Contact : ${data.vendorPhone}` : "",
    ].filter(Boolean);

    const deliveryAddress =
      data.deliveryAddress || branding.address || data.companyAddress || "";
    const deliveryLocation = data.deliveryAddress
      ? ""
      : [
          data.deliveryCity || data.companyCity,
          data.deliveryState || data.companyState,
          data.deliveryPincode || data.companyPincode,
        ]
          .filter(Boolean)
          .join(", ");
    const rightLines = [
      ...
      deliveryLocation,
      !branding.address && (data.deliveryPincode || data.companyPincode)
        ? `PIN : ${data.deliveryPincode || data.companyPincode}`
        : "",
      data.deliveryContactPerson ? `Name : ${data.deliveryContactPerson}` : "",
      data.deliveryPhone || branding.phone
        ? `Contact : ${data.deliveryPhone || branding.phone}`
        : "",
    ].filter(Boolean);
  ...
  private async drawReferenceItemsTable(
    state: PageState,
    ctx: any,
  ): Promise<PageState> {
    const { font, fontBold, data } = ctx;
    ...
    for (let i = 0; i < data.items.length; i++) {
      ...
      const descLines = this.wrapMultiline(
        this.composeReferenceDescription(item),
        cols[1].width - 8,
        7.1,
        font,
      );
      ...
      const rowH = Math.max(
        24,
        Math.max(descLines.length, taxLines.length) * 9 + 8,
      );
  ...
  private async drawReferenceAmountSummary(
    state: PageState,
    ctx: any,
  ): Promise<PageState> {
    const { page } = state;
    ...
    rows.forEach((row, index) => {
      ...
      this.drawWrappedLines(
        page,
        this.wrapText(row[0], leftW - 12, 7.2, f),
        MX + 6,
        y - 11,
        leftW - 12,
        7.2,
  ...
  private async drawReferenceTermsAndSignature(
    state: PageState,
    ctx: any,
  ): Promise<PageState> {
    const { page } = state;
  ...
  private async pinReferenceBottomBlocks(
    state: PageState,
    ctx: any,
  ): Promise<PageState> {
  ...
  private referenceBottomBlocksHeight(ctx: any): number {
    return (
      this.referenceAmountRows(ctx).length * 18 +
      10 +
      this.referenceTermsHeight(ctx) +
      8
    );
  }
  ...
  private referenceTermsHeight(ctx: any): number {
    const contentLines = Math.max(
      this.referenceTermLines(ctx).length,
      this.referenceInstructionLines(ctx).length,
    );
  ...
  private referenceAmountRows(ctx: any): string[][] {
    const { data } = ctx;
    const taxTotal =
      (Number(data.cgstTotal || 0) || 0) +
      (Number(data.sgstTotal || 0) || 0) +
      (Number(data.igstTotal || 0) || 0);
    const rows = [
      [
        `Value of Supply : ${this.amountInWords(data.taxableAmount || data.subtotal || 0)} Only`,
        `Value of Supply : ${this.fmtCur(data.taxableAmount || data.subtotal || 0)}`,
      ],
    ];
    ...
    if (freightAmount > 0)
      rows.push([
        `Freight Value : ${this.amountInWords(freightAmount)} Only`,
        `Freight Value : ${this.fmtCur(freightAmount)}`,
      ]);
    if (freightGstAmount > 0)
      rows.push([
        `Freight GST @ ${this.fmtNum(freightGstPercent)}% : ${this.amountInWords(freightGstAmount)} Only`,
        `Freight GST : ${this.fmtCur(freightGstAmount)}`,
      ]);
    if (customsDuty > 0)
      rows.push([
        `Customs Duty : ${this.amountInWords(customsDuty)} Only`,
        `Customs Duty : ${this.fmtCur(customsDuty)}`,
      ]);
    if (additionalExpenses > 0)
      rows.push([
        `Additional Expenses : ${this.amountInWords(additionalExpenses)} Only`,
        `Additional Expenses : ${this.fmtCur(additionalExpenses)}`,
      ]);
    rows.push([
      `Total Tax Amount : ${this.amountInWords(taxTotal)} Only`,
      `Tax Amount : ${this.fmtCur(taxTotal)}`,
    ]);
    const roundOff = this.safeNumber(data.roundOff);
    if (Math.abs(roundOff) >= 0.01)
      rows.push([
        `Rounding : ${this.fmtCur(roundOff)}`,
        `Rounding : ${this.fmtCur(roundOff)}`,
      ]);
    rows.push([
      `Total Amount : ${this.amountInWords(data.grandTotal || 0)} Only`,
      `Total Amount (INR) : ${this.fmtCur(data.grandTotal || 0)}`,
    ]);
    return rows;
  }
  ...
  private referenceTermLines(ctx: any): string[] {
    const { font, data } = ctx;
    const terms = [
      data.paymentTerms || data.terms?.payment_terms
        ? `Payment Terms : ${data.paymentTerms || data.terms?.payment_terms}`
        : "",
      data.terms?.delivery_terms
        ? `Delivery Terms : ${data.terms.delivery_terms}`
        : "",
      data.terms?.freight_terms
        ? `Freight Terms : ${data.terms.freight_terms}`
        : "",
      Number(data.freightAmount || 0) > 0
        ? `Freight Value : ${this.fmtCur(data.freightAmount)}`
        : "",
      data.freightGstApplicable
        ? `Freight GST : Applicable @ ${this.fmtNum(data.freightGstPercent || 0)}% (${this.fmtCur(data.freightGstAmount || 0)})`
        : "",
      Number(data.additionalExpenses || 0) > 0
        ? `Additional Expenses : ${this.fmtCur(data.additionalExpenses)}`
        : "",
    ].filter(Boolean);
    return this.wrapMultiline(
  ...
  private async referenceEnsureSpace(
    state: PageState,
    needed: number,
    ctx: any,
    cols?: any[],
    ...
    if (cols && tw)
      next = this.drawReferenceTableHeader(next, cols, tw, ctx.fontBold);
    return next;
  }

  private drawReferenceTableHeader(
    state: PageState,
    cols: any[],
    tw: number,
    fontBold: PDFFont,
  ...
  private drawReferenceLines(
    ...
    for (const line of lines) {
      ...
      for (const part of this.wrapText(
        line,
        width,
        7.3,
        isName ? fontBold : font,
      )) {
        page.drawText(part, {
          x,
          y: cy,
  ...
  private referenceTaxLines(item: POItem): string[] {
    ...
    if (cgst || sgst)
      return [`CGST ${this.fmtNum(cgst)}%`, `SGST ${this.fmtNum(sgst)}%`];
    return ["-"];
  }
  ...
  private async ensureSpace(
    ...
    if (repeatTableHdr && cols && tw)
      s = this.drawTableHeader(s, cols, tw, ctx.fontBold);
    return s;
  }
  ...
  private drawAddressBlocks(state: PageState, ctx: any): PageState {
    ...
    const { font, fontBold, data } = ctx;
    const gap = 10;
    const vendorColW = Math.floor((USABLE - gap) * 0.6); // ~313
    ...
    const vLines = [
      ...
      data.vendorContactPerson
        ? `Contact : ${formatVendorContactName(data.vendorSalutation, data.vendorContactPerson)}`
        : "",
    ].filter(Boolean);
    ...
    const rLines = [
      ...
      data.deliveryDate
        ? `Delivery Date : ${this.fmtDate(data.deliveryDate)}`
        : "",
  ...
  private drawDeliveryBar(state: PageState, ctx: any): PageState {
    ...
    const wrapped = this.wrapText(
      delText || "Same as company address",
      USABLE - 16,
      7.5,
      font,
    );
    ...
    for (let i = 0; i < data.items.length; i++) {
      ...
      const gstLines: string[] =
        igstRate > 0
          ...
          : [
              `CGST ${this.fmtNum(cgstRate)}%`,
              `SGST ${this.fmtNum(sgstRate)}%`,
            ];
      const rowH = Math.max(
        18,
        Math.max(descLines.length, gstLines.length) * 9 + 4,
      );
  ...
  private async drawSummaryBlock(
    state: PageState,
    ctx: any,
  ): Promise<PageState> {
    const { font, fontBold, data } = ctx;
    ...
    if (data.freightGstAmount)
      rows.push([
        `Freight GST @ ${this.fmtNum(data.freightGstPercent || 0)}%`,
        data.freightGstAmount,
      ]);
    if (data.customsDuty) rows.push(["Customs Duty", data.customsDuty]);
    if (data.additionalExpenses)
      rows.push(["Additional Expenses", data.additionalExpenses]);
    if (Math.abs(this.safeNumber(data.roundOff)) >= 0.01)
      rows.push(["Rounding", data.roundOff]);
    ...
    state.page.drawText("Amount Summary", {
      x: rx + 8,
      y: top - 12,
      size: 7.5,
      font: fontBold,
  ...
  private async drawShortTerms(state: PageState, ctx: any): Promise<PageState> {
    ...
    const statLines = [`GST No : ${data.companyGSTIN || "-"}`].filter(Boolean);
  ...
  private async drawGstSummary(state: PageState, ctx: any): Promise<PageState> {
    ...
    state.page.drawText("GST Summary", {
      x: MX + 8,
      y: top - 12,
      size: 7.5,
      font: fontBold,
    ...
    for (const col of cols) {
      ...
      state.page.drawLine({
        start: { x: headerX, y: currentY },
        end: { x: headerX, y: currentY - rowH },
        thickness: 0.4,
        color: BORDER,
      });
      headerX += col.width;
    ...
    for (const row of rows) {
      ...
      const values = [
        row.rateLabel,
        this.fmtCur(row.taxableAmount),
        this.fmtCur(row.taxAmount),
      ];
    ...
    state.page.drawLine({
      start: { x: MX, y: currentY },
      end: { x: MX + tw, y: currentY },
      thickness: 0.4,
      color: BORDER,
    });
  ...
  private async drawGeneralTerms(
    state: PageState,
    ctx: any,
  ): Promise<PageState> {
    const { font, fontBold, data } = ctx;
  ...
  private async drawSignatureBlock(
    ...
    for (let i = 0; i < signers.length; i++) {
      ...
      state.page.drawText(this.fitText(value, boxW - 16, fontItalic, 7), {
        x: x + 8,
        y: top - boxH + 10,
        size: 7,
        font: fontItalic,
  ...
  private drawFooters(
    ...
    for (let i = 0; i < pages.length; i++) {
      ...
      const right =
        branding.footerText ||
        "Computer-generated PO - no physical signature required.";
      ...
      page.drawLine({
        start: { x: MX, y: MY_BOT + 10 },
        end: { x: PAGE_W - MX, y: MY_BOT + 10 },
        thickness: 0.5,
        color: BORDER,
      });
      page.drawText(left, {
      ...
      const fittedRight = this.fitText(
  ...
  private drawWrappedLines(
    ...
    let cy = y;
    for (const line of lines) {
      page.drawText(line, { x, y: cy, size: _fs, font, color });
      cy -= lh;
    }
  }

  private drawJustifiedLines(
    page: PDFPage,
    lines: string[],
    x: number,
    y: number,
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (isLast || line.trim() === "") {
        ...
        if (words.length <= 1) {
          ...
          const totalTextW = words.reduce(
            (s, w) => s + font.widthOfTextAtSize(w, fs),
            0,
          );
  ...
  private wrapText(
    ...
    for (const w of words) {
      ...
      if (font.widthOfTextAtSize(cand, fontSize) <= width) {
        cur = cand;
        continue;
      }
      if (cur) lines.push(cur);
      if (font.widthOfTextAtSize(w, fontSize) <= width) {
        cur = w;
        continue;
      }
      // Break long word
      let seg = "";
      for (const ch of w) {
        if (font.widthOfTextAtSize(seg + ch, fontSize) <= width) {
          seg += ch;
        } else {
  ...
  private fitText(
    ...
    let t = "";
    for (const ch of s) {
      if (font.widthOfTextAtSize(t + ch + "...", fontSize) > width) break;
      t += ch;
    }
    return t ? t + "..." : "";
  }

  private countWrapped(
    lines: string[],
    w: number,
    fs: number,
    font: PDFFont,
  ...
  private buildGstSummaryRows(
    items: POItem[],
  ): Array<{ rateLabel: string; taxableAmount: number; taxAmount: number }> {
  ...
  private composeDescription(item: POItem): string {
    ...
    const deduped = parts
      .filter(
        (p, i) =>
          parts.findIndex((c) => c.toLowerCase() === p.toLowerCase()) === i,
      )
      .join(" | ");
  ...
  private makePdfValueSafe(value: unknown, font: PDFFont): unknown {
    if (typeof value === "string") return makePdfTextSafe(value, font);
    if (Array.isArray(value))
      return value.map((entry) => this.makePdfValueSafe(entry, font));
    if (value && typeof value === "object") {
      return Object.fromEntries(
        Object.entries(value).map(([key, entry]) => [
          key,
          this.makePdfValueSafe(entry, font),
        ]),
      );
    }
    return value;
  ...
  private withRoundedGrandTotal(data: POPdfData): POPdfData {
    ...
    const roundOff =
      Math.abs(suppliedRoundOff) >= 0.01
