...
describe('buildSapGrnControls', () => {
  it('blocks empty GRNs from QC, three-way match, and Accounts Payable', () => {
    ...
    expect(controls.toleranceStatus).toBe('BLOCKED');
    expect(controls.threeWayMatchStatus).toBe('BLOCKED');
    expect(controls.grIrStatus).toBe('PENDING_INVOICE_APPROVAL');
    expect(controls.messages.join(' ')).toContain('no material lines');
  });
