...
export type ListTableProps<T> = {
  ...
  className?: string;
  /** Concise description announced before the table or mobile record list. */
  ariaLabel?: string;
...
export function ListTable<T>(props: ListTableProps<T>) {
  ...
  return (
    <div className={`overflow-hidden rounded-md border border-[#E8DCC4] bg-white ${className}`}>
      ...
      {/* Mobile record list */}
      <div className="divide-y divide-[#E8DCC4] md:hidden" role="list" aria-label={ariaLabel}>
        {pagedRows.length === 0 ? (
          <div className="px-4 py-10 text-center text-sm text-gray-500" role="status">
          ...
          pagedRows.map((row) => {
            ...
            return (
              <article key={rowId} className="bg-white px-3 py-3" role="listitem">
            ...
            {pagedRows.length === 0 ? (
              <tr>
                <td colSpan={(visibleColumns.length || 1) + (selectable ? 1 : 0)} className="px-6 py-10 text-center text-gray-500">
                  <div role="status">{emptyState || 'No results'}</div>
                </td>
              </tr>
            ) : (
              pagedRows.map((row) => (
