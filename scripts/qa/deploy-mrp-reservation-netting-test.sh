...
stage=/tmp/mizantra-mrp-reservation-netting-20260830
backup=/root/sak-deploy-backups/mizantra-mrp-reservation-netting-$(date +%Y%m%d-%H%M%S)
...
if [ "$verified" != true ]; then
  ...
  pm2 restart sak-api-test sak-web-test
  echo 'MRP reservation-netting deployment failed health checks; previous test runtime restored.' >&2
  exit 1
fi

echo "Mizantra TEST MRP reservation netting deployed and verified. Backup: $backup"
