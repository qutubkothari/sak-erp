...
Object.defineProperty(exports, "JobOrderService", {
    enumerable: true,
    get: function() {
...
let JobOrderService = class JobOrderService {
    async create(tenantId, userId, dto) {
        // Get item details
        const { data: item } = await this.supabase.from('items').select('code, name').eq('id', dto.itemId).single();
        if (!item) throw new _common.NotFoundException('Item not found');
        ...
        if (dto.materials && dto.materials.length > 0) {
            ...
            if (!availability.available) {
                throw new _common.BadRequestException(`Insufficient materials:\n${availability.shortages.map((s)=>`${s.itemCode} - ${s.itemName}: Need ${s.required}, Available ${s.available}, Short ${s.shortage}`).join('\n')}`);
            }
        }
        // Create job order
        const { data: jobOrder, error } = await this.supabase.from('production_job_orders').insert({
            tenant_id: tenantId,
            item_id: dto.itemId,
            item_code: item.code,
            item_name: item.name,
        ...
        if (dto.operations && dto.operations.length > 0) {
            const operations = await Promise.all(dto.operations.map(async (op)=>{
                const { data: workstation } = await this.supabase.from('workstations').select('name').eq('id', op.workstationId).single();
                let assignedUserName = null;
                if (op.assignedUserId) {
                    const { data: user } = await this.supabase.from('users').select('full_name').eq('id', op.assignedUserId).single();
        ...
        if (dto.materials && dto.materials.length > 0) {
            const materials = await Promise.all(dto.materials.map(async (mat)=>{
                const { data: matItem } = await this.supabase.from('items').select('code, name').eq('id', mat.itemId).single();
                let warehouseName = null;
                if (mat.warehouseId) {
                    const { data: wh } = await this.supabase.from('warehouses').select('name').eq('id', mat.warehouseId).single();
                ...
                return {
                    ...
                    selected_variant_id: mat.selectedVariantId || null,
                    variant_notes: mat.variantNotes || null
                };
            }));
    ...
    async findAll(tenantId, filters) {
        let query = this.supabase.from('production_job_orders').select('*').eq('tenant_id', tenantId).order('created_at', {
            ascending: false
        });
        if (filters?.status) {
    ...
    async findOne(tenantId, id) {
        const { data: jobOrder, error } = await this.supabase.from('production_job_orders').select('*').eq('tenant_id', tenantId).eq('id', id).single();
        if (error) throw new _common.NotFoundException('Job order not found');
        // Get operations
        const { data: operations } = await this.supabase.from('job_order_operations').select('*').eq('job_order_id', id).order('sequence_number', {
            ascending: true
        });
        // Get materials
        const { data: materials } = await this.supabase.from('job_order_materials').select('*').eq('job_order_id', id);
        return {
            ...jobOrder,
    ...
    async update(tenantId, id, dto) {
        const { error } = await this.supabase.from('production_job_orders').update(dto).eq('tenant_id', tenantId).eq('id', id);
        if (error) throw new _common.BadRequestException(error.message);
    ...
    async updateStatus(tenantId, id, status) {
        ...
        const { error } = await this.supabase.from('production_job_orders').update(updates).eq('tenant_id', tenantId).eq('id', id);
        if (error) throw new _common.BadRequestException(error.message);
        return this.findOne(tenantId, id);
    }
    async updateOperation(tenantId, jobOrderId, operationId, dto) {
        // Verify job order belongs to tenant
        const { data: jobOrder } = await this.supabase.from('production_job_orders').select('id').eq('tenant_id', tenantId).eq('id', jobOrderId).single();
        if (!jobOrder) throw new _common.NotFoundException('Job order not found');
        const { error } = await this.supabase.from('job_order_operations').update(dto).eq('id', operationId).eq('job_order_id', jobOrderId);
        if (error) throw new _common.BadRequestException(error.message);
        return this.findOne(tenantId, jobOrderId);
    }
    async delete(tenantId, id) {
        const { error } = await this.supabase.from('production_job_orders').delete().eq('tenant_id', tenantId).eq('id', id);
        if (error) throw new _common.BadRequestException(error.message);
        return {
            message: 'Job order deleted successfully'
        };
    }
    async createFromBOM(tenantId, userId, itemId, bomId, quantity, startDate) {
        // Get BOM details
        const { data: bom } = await this.supabase.from('bom_headers').select(`
        *,
        bom_items(*, items(code, name)),
        bom_routing(*, workstations(id, name))
      `).eq('tenant_id', tenantId).eq('id', bomId).single();
        if (!bom) throw new _common.NotFoundException('BOM not found');
        // Create operations from routing
        const operations = (bom.bom_routing || []).map((route, idx)=>({
    ...
    async createFromBOMWithVariantSelections(tenantId, userId, args) {
        const { data: bom } = await this.supabase.from('bom_headers').select(`
        *,
        bom_items(*, items(code, name)),
        bom_routing(*, workstations(id, name))
      `).eq('tenant_id', tenantId).eq('id', args.bomId).single();
        if (!bom) throw new _common.NotFoundException('BOM not found');
        const operations = (bom.bom_routing || []).map((route, idx)=>({
                sequenceNumber: route.operation_sequence || (idx + 1) * 10,
                operationName: route.operation_name,
        ...
        const materials = (bom.bom_items || []).map((item)=>{
            ...
            return {
                ...
                selectedVariantId: shouldApplyVariant && selectedVariantId && selectedVariantId !== baseItemId ? selectedVariantId : undefined
            };
        });
        return this.create(tenantId, userId, {
            itemId: args.itemId,
    ...
    async checkMaterialAvailability(tenantId, materials, jobQuantity) {
        ...
        for (const material of materials){
            ...
            if (available < required) {
                // Fetch item details
                const { data: item } = await this.supabase.from('items').select('code, name').eq('id', itemIdToCheck).single();
                shortages.push({
    ...
    async completeJobOrder(tenantId, jobOrderId, userId) {
        // Get job order with materials
        const { data: jobOrder } = await this.supabase.from('production_job_orders').select('*, job_order_materials(*)').eq('tenant_id', tenantId).eq('id', jobOrderId).single();
        if (!jobOrder) throw new _common.NotFoundException('Job order not found');
        if (jobOrder.status !== 'IN_PROGRESS') {
            throw new _common.BadRequestException('Job order must be IN_PROGRESS to complete');
        }
        ...
        try {
            ...
            for (const material of jobOrder.job_order_materials){
                ...
                // Get item details
                const { data: item } = await this.supabase.from('items').select('code, name, category').eq('id', itemIdToConsume).single();
                // Get available stock entries for this material (use variant if selected)
                const { data: stockEntries } = await this.supabase.from('stock_entries').select('*').eq('tenant_id', tenantId).eq('item_id', itemIdToConsume).gt('available_quantity', 0).order('created_at', {
                    ascending: true
                });
                if (!stockEntries || stockEntries.length === 0) {
                    throw new _common.BadRequestException(`Failed to consume ${item?.code}: Item not found in inventory`);
                }
                ...
                if (totalAvailable < consumeQty) {
                    throw new _common.BadRequestException(`Failed to consume ${item?.code}: Insufficient stock. Need ${consumeQty}, have ${totalAvailable}`);
                }
                ...
                for (const entry of stockEntries){
                    ...
                    if (updateError) {
                        ...
                        throw new _common.BadRequestException(`Failed to consume ${item?.code}: ${updateError.message}`);
                    }
                    remainingToConsume -= toConsumeFromEntry;
                }
                // Update material issued quantity
                await this.supabase.from('job_order_materials').update({
            ...
            const { data: warehouses } = await this.supabase.from('warehouses').select('id').eq('tenant_id', tenantId).limit(1);
            if (!warehouses || warehouses.length === 0) {
                throw new _common.BadRequestException('No warehouse configured. Please create a warehouse first.');
            }
            const warehouseId = warehouses[0].id;
            // Get finished item details for UID generation
            const { data: finishedItem } = await this.supabase.from('items').select('id, code, name, category').eq('id', jobOrder.item_id).single();
            if (!finishedItem) {
                throw new _common.BadRequestException('Finished item not found');
            }
            ...
            for(let i = 0; i < quantityProduced; i++){
                // Generate UID
                const uid = await this.uidService.generateUID('SAIF', 'MFG', entityType);
            ...
            const { error: addError } = await this.supabase.from('stock_entries').insert({
                tenant_id: tenantId,
            ...
            if (addError) {
                ...
                throw new _common.BadRequestException(`Failed to add finished goods: ${addError.message}`);
            }
            // 3. Update job order status
            const { error: updateError } = await this.supabase.from('production_job_orders').update({
                status: 'COMPLETED',
                actual_end_date: new Date().toISOString(),
                completed_quantity: jobOrder.quantity
            }).eq('id', jobOrderId);
            if (updateError) throw new _common.BadRequestException(updateError.message);
            return this.findOne(tenantId, jobOrderId);
        } catch (error) {
    ...
    async getCompletionPreview(tenantId, jobOrderId) {
        // Get job order with materials and finished item
        const { data: jobOrder } = await this.supabase.from('production_job_orders').select(`
        *,
        job_order_materials(*, item:items(code, name)),
        finished_item:items!production_job_orders_item_id_fkey(code, name)
      `).eq('tenant_id', tenantId).eq('id', jobOrderId).single();
        if (!jobOrder) throw new _common.NotFoundException('Job order not found');
        // Get current stock for finished item from stock_entries
        const { data: finishedStockEntries } = await this.supabase.from('stock_entries').select('available_quantity').eq('tenant_id', tenantId).eq('item_id', jobOrder.item_id);
        ...
        const materialsWithStock = await Promise.all(jobOrder.job_order_materials.map(async (material)=>{
            const { data: stockEntries } = await this.supabase.from('stock_entries').select('available_quantity, allocated_quantity').eq('tenant_id', tenantId).eq('item_id', material.item_id);
    ...
    async getItemBasic(itemId) {
        const { data } = await this.supabase.from('items').select('id, code, name, category').eq('id', itemId).single();
        return data || null;
    }
    async getAvailableStock(tenantId, itemId) {
    ...
    async getActiveBomForItem(tenantId, itemId) {
        // Prefer active BOM. If is_active is not present or no active exists, fall back to latest version.
        const { data: active } = await this.supabase.from('bom_headers').select('*').eq('tenant_id', tenantId).eq('item_id', itemId).eq('is_active', true).order('version', {
            ascending: false
        }).limit(1);
        if (Array.isArray(active) && active.length > 0) return active[0];
        const { data: latest } = await this.supabase.from('bom_headers').select('*').eq('tenant_id', tenantId).eq('item_id', itemId).order('version', {
            ascending: false
        }).limit(1);
    ...
    async getBomById(tenantId, bomId) {
        const { data } = await this.supabase.from('bom_headers').select('*').eq('tenant_id', tenantId).eq('id', bomId).single();
        return data || null;
    }
    async getBomItems(bomId) {
        const { data } = await this.supabase.from('bom_items').select('*').eq('bom_id', bomId).order('sequence', {
            ascending: true
        });
        return Array.isArray(data) ? data : [];
    }
    async buildSmartExplosion(tenantId, bomId, multiplier, level, visitedBomIds, caches) {
        if (multiplier <= 0) return {
            nodes: [],
            subAssemblies: []
        };
        if (visitedBomIds.has(bomId)) {
            throw new _common.BadRequestException('BOM cycle detected. Please check BOM hierarchy.');
        }
        ...
        for (const bi of bomItems){
            ...
            const childBomId = bi.child_bom_id || bi.child_bomId || null;
            const itemId = bi.item_id || bi.itemId || null;
            if (childBomId) {
                let childBom = caches.bomById.get(childBomId);
                if (!childBom) {
                    childBom = await this.getBomById(tenantId, childBomId);
                    if (!childBom) throw new _common.NotFoundException('Child BOM not found');
                ...
                if (!subItem) {
                    ...
                    if (!subItem) throw new _common.NotFoundException('Item not found');
                ...
                if (toMakeQuantity > 0) {
                    ...
                    const childResult = await this.buildSmartExplosion(tenantId, childBomId, toMakeQuantity, level + 1, new Set(visitedBomIds), caches);
                ...
                if (isSubassembly) {
                    ...
                    if (subBom) {
                        ...
                        if (toMakeQuantity > 0) {
                            ...
                            const childResult = await this.buildSmartExplosion(tenantId, subBom.id, toMakeQuantity, level + 1, new Set(visitedBomIds), caches);
                            nodes.push(...childResult.nodes);
                            subAssemblies.push(...childResult.subAssemblies);
                        }
                        continue;
    ...
    async getSmartJobOrderPreview(tenantId, req) {
        if (!req?.itemId) throw new _common.BadRequestException('itemId is required');
        if (!req?.quantity || Number(req.quantity) <= 0) throw new _common.BadRequestException('quantity must be > 0');
        const finishedItem = await this.getItemBasic(req.itemId);
        if (!finishedItem) throw new _common.NotFoundException('Item not found');
        const topBom = await this.getActiveBomForItem(tenantId, req.itemId);
        if (!topBom) {
            throw new _common.BadRequestException('No BOM found for this item');
        }
        const caches = {
            itemById: new Map([
                [
        ...
        const { nodes, subAssemblies } = await this.buildSmartExplosion(tenantId, topBom.id, Number(req.quantity), 0, new Set(), caches);
    ...
    async createSmartJobOrder(tenantId, userId, req) {
        if (!req?.itemId) throw new _common.BadRequestException('itemId is required');
        if (!req?.quantity || Number(req.quantity) <= 0) throw new _common.BadRequestException('quantity must be > 0');
        ...
        for (const sa of preview.subAssembliesToMake){
            ...
            await this.supabase.from('production_job_orders').update({
                status: 'IN_PROGRESS',
        ...
        const mainNotesParts = [
            'Created via Smart Job Order'
        ];
...
//# sourceMappingURL=job-order.service.js.map