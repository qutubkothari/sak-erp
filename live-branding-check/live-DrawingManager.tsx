...
export default function DrawingManager({ itemId, itemCode, itemName, onClose, onChanged, mandatory = false }: DrawingManagerProps) {
  ...
  const canDeleteDrawings = getUserRoleNames(readStoredUser()).some((name) =>
    String(name).trim().toUpperCase().replace(/[\s_-]+/g, '') === 'SUPERADMIN',
  );
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      const blockedExtensions = new Set([
        '.apk', '.app', '.bat', '.cmd', '.com', '.cpl', '.dll', '.dmg',
        '.exe', '.hta', '.html', '.htm', '.jar', '.js', '.jse', '.msi',
        '.msp', '.php', '.ps1', '.scr', '.sh', '.vbs', '.wsf',
      ]);
      ...
      setFileRole(
        ['step','stp','dwg','dxf','iges','igs','ifc','stl','rvt','sldprt','sldasm','ipt','iam'].includes(extensionName)
          ? 'NATIVE_CAD'
          : ['pdf','png','jpg','jpeg','tif','tiff','webp'].includes(extensionName) ? 'CONTROLLED_2D' : 'SUPPORTING',
      );
  ...
  const uploadDirectlyToSupabase = (signedUrl: string, file: File): Promise<void> =>
    new Promise((resolve, reject) => {
      ...
      request.onerror = () => reject(new Error('Storage upload was interrupted'));
      request.onload = () => {
  ...
  const parsedSharedItemCodes = () =>
    Array.from(new Set(sharedItemCodes
      .split(/[;,\n]/)
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (!prepareResponse.ok) {
        throw new Error(uploadTarget?.message || 'Unable to prepare secure drawing upload');
      }

      if (uploadTarget?.deduplicated && uploadTarget?.existingDrawingId) {
        const reuseResponse = await fetch(`/api/v1/inventory/items/${itemId}/drawings`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
          body: JSON.stringify({
  ...
  const handleTransition = async (drawing: Drawing, action: 'SUBMIT' | 'APPROVE' | 'REJECT') => {
    try {
      ...
      if (!response.ok) throw new Error(result?.message || `Unable to ${action.toLowerCase()} drawing package`);
      await fetchDrawings();
    } catch (error: any) {
      alert(error?.message || 'Unable to change drawing package status');
    } finally {
      setUpdatingActiveId(null);
    }
  };
  ...
  const handleDeleteDrawing = async (drawing: Drawing) => {
    ...
    const linkedItems = Array.isArray(drawing.linked_item_codes) ? drawing.linked_item_codes.filter(Boolean) : [];
    ...
    const mandatoryWarning = mandatory && drawings.length === 1
      ? "\n\nThis is the item's last drawing. A replacement will be required before drawing-controlled workflows can proceed."
      : '';
    if (!window.confirm(
    ...
    try {
      ...
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(result?.message || 'Unable to delete drawing');
      await fetchDrawings();
  ...
  const canClose = !mandatory || drawings.some((drawing) => drawing.lifecycle_status === 'APPROVED' && drawing.is_active);

  const activeDrawing = drawings.find(d => d.lifecycle_status === 'APPROVED' && d.is_active) || null;
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-gray-500">
                  CAD and engineering formats such as DWG, DXF, STEP, IGES,
                  IFC, STL, RVT and native vendor files are supported.
                </p>
              </div>
              ...
              <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 space-y-2">
                ...
                <select value="" onChange={(event) => {
                  ...
                  if (existing) { setDrawingNumber(existing.drawing_number || itemCode); setRevisionCode(existing.revision_code || `R${existing.version}`); }
                  ...
                  {drawings.map((drawing) => <option key={drawing.id} value={drawing.id}>{drawing.drawing_number || itemCode} — {drawing.revision_code || `R${drawing.version}`}</option>)}
                </select>
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                  <input value={drawingNumber} onChange={(event) => setDrawingNumber(event.target.value.toUpperCase())} placeholder="Drawing number" className="rounded border border-amber-200 bg-white px-3 py-2 text-sm" />
                  <input value={revisionCode} onChange={(event) => setRevisionCode(event.target.value.toUpperCase())} placeholder="Revision" className="rounded border border-amber-200 bg-white px-3 py-2 text-sm" />
                  <select value={fileRole} onChange={(event) => setFileRole(event.target.value)} className="rounded border border-amber-200 bg-white px-3 py-2 text-sm">
                    <option value="NATIVE_CAD">Native CAD / 3D</option><option value="CONTROLLED_2D">Controlled PDF / 2D</option><option value="SUPPORTING">Supporting document</option>
                  </select>
                </div>
                <p className="text-xs text-amber-900">Files with the same drawing number and revision form one controlled package.</p>
              </div>
          ...
          <div>
            ...
            {activeDrawing && (
              <div className="mb-3 text-sm bg-green-50 border border-green-200 rounded px-3 py-2 text-green-900">
                Latest approved revision: <span className="font-semibold">{activeDrawing.drawing_number || `v${activeDrawing.version}`} / {activeDrawing.revision_code || `R${activeDrawing.version}`}</span>
              </div>
            )}
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        <div className="mt-2 text-sm text-gray-600 grid grid-cols-2 gap-x-4">
                          ...
                          <div className="col-span-2 mt-1">Drawing: {drawing.drawing_number || '-'} / Revision: {drawing.revision_code || `R${drawing.version}`} / Role: {drawing.file_role || '-'}</div>
                        ...
                        {Array.isArray(drawing.linked_item_codes) && drawing.linked_item_codes.length > 0 && (
                      ...
                      <div className="flex gap-2">
                        ...
                        {(drawing.lifecycle_status || 'DRAFT') === 'DRAFT' && (
                          ...
                          >
                            {updatingActiveId === drawing.id ? 'Submitting...' : 'Submit package'}
                          </button>
                        )}
                        {drawing.lifecycle_status === 'SUBMITTED' && (
                          <><button type="button" onClick={() => handleTransition(drawing, 'APPROVE')} disabled={updatingActiveId === drawing.id} className="bg-green-100 text-green-800 px-4 py-2 rounded text-sm font-medium disabled:opacity-50">Approve</button>
                          <button type="button" onClick={() => handleTransition(drawing, 'REJECT')} disabled={updatingActiveId === drawing.id} className="bg-red-100 text-red-800 px-4 py-2 rounded text-sm font-medium disabled:opacity-50">Reject</button></>
                        )}
                      </div>
                    </div>
                  </div>
