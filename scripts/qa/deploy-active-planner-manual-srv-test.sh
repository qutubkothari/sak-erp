...
stage=/tmp/mizantra-active-planner-manual-srv-20260830
backup=/root/sak-deploy-backups/mizantra-active-planner-manual-srv-$(date +%Y%m%d-%H%M%S)
...
paths=(
  ...
  scripts/qa/deploy-active-planner-manual-srv-test.sh
  scripts/qa/active-planner-manual-srv-preview.cjs
)
...
if [ "$verified" != true ]; then
  ...
  pm2 restart sak-api-test
  echo 'Governed manual SRV deployment failed; previous Mizantra test API restored.' >&2
  exit 1
fi

pm2 save
echo "Mizantra TEST governed manual SRV prompt deployed and verified. Backup: $backup"
