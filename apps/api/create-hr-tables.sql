...
DO $$ BEGIN
    CREATE TYPE salary_component_type AS ENUM ('BASIC', 'HRA', 'ALLOWANCE', 'BONUS', 'DEDUCTION', 'PF', 'ESI', 'TAX', 'CTC');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;
...
-- Existing installations may already have the HR tables. Keep their schema
-- compatible with travel/per-diem payroll without requiring a data reset.
