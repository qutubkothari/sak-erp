        ...
        <div className="bg-white rounded-md border border-[#E8DCC4] overflow-hidden">
          {loading ? (
            ...
            <ListTable
              ...
              isRowSelectable={(invoice) => !invoice.invoice_approved}
              selectedRowIds={Array.from(selectedInvoiceIds).filter((id) => sanctionableInvoices.some((invoice) => invoice.id === id))}
              onSelectionChange={(ids) => {
                const sanctionableIds = new Set(sanctionableInvoices.map((invoice) => invoice.id));
                setSelectedInvoiceIds(new Set(ids.filter((id) => sanctionableIds.has(id))));
              }}
            />
          )}
        </div>
