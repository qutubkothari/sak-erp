...
function isCompletedJobOrder(jobOrder: JobOrder): boolean {
  ...
  return (
    status === "QC_COMPLETED" ||
    ((status === "STORE_ISSUED" || status === "COMPLETED") &&
      workflowStatus === "QC_COMPLETED") ||
    (status === "COMPLETED" && srvQty > 0 && srvPendingQty <= 0)
  );
}

function materialIssueStage(jobOrder: JobOrder) {
  const materials = Array.isArray(jobOrder.materials) ? jobOrder.materials : [];
  const required = materials.reduce(
    (sum, material) =>
      sum + Math.max(0, Number(material.requiredQuantity || 0)),
    0,
  );
  const issuedByLines = materials.reduce(
    (sum, material) => sum + Math.max(0, Number(material.issuedQuantity || 0)),
    0,
  );
  const summary = jobOrder.sivSummary || {};
  const issued = Math.max(issuedByLines, Number(summary.quantity || 0));
  const approved = Number(summary.approvedQuantity || 0);
  const pending = Number(summary.pendingQuantity || 0);

  if (required <= 0 && issued <= 0) return "Pending";
  if (issued <= 0) return "Pending";
  if (required > 0 && issued + 1e-9 < required) return "Partially Issued";
  if (pending > 0 || approved + 1e-9 < issued) return "Pending Approval";
  return "Completed";
}
...
export default function JobOrdersPage() {
  return (
    <Suspense
      fallback={
        <div className="p-6 text-sm text-gray-500">Loading job orders...</div>
      }
    >
...
function JobOrdersPageContent() {
  ...
  const storedUserId =
    typeof window !== "undefined"
      ? String(localStorage.getItem("userId") || "").trim()
      : "";
  ...
  const canApproveInventoryJobs =
    ...
    hasScreenPermission(
      currentUser as any,
      "/dashboard/inventory/siv",
      "approve",
    ) ||
    hasScreenPermission(
      currentUser as any,
      "/dashboard/inventory/srv",
      "approve",
    ) ||
    hasScreenPermission(
      currentUser as any,
      "/dashboard/inventory/store-vouchers",
      "approve",
    );
  ...
  const [jobOrderToSchedule, setJobOrderToSchedule] = useState<JobOrder | null>(
    null,
  );
  ...
  const [jobOrderToPartialComplete, setJobOrderToPartialComplete] =
  ...
  const [completionJobOrderId, setCompletionJobOrderId] = useState<
    string | null
  >(null);
  ...
  const [stockErrorModal, setStockErrorModal] = useState<{
    show: boolean;
    shortages: any[];
  }>({ show: false, shortages: [] });
  const [itemStockSummaryById, setItemStockSummaryById] = useState<
    Record<string, ItemStockSummary>
  >({});
  ...
  const [factoryReadiness, setFactoryReadiness] = useState<any>(null);
  const [scheduleForm, setScheduleForm] = useState({
    assignedTo: "",
    startDate: `${getTodayDateInputValue()}T09:00`,
  });
  const [dispatchOperation, setDispatchOperation] =
  ...
  const [baseMaterialQuantities, setBaseMaterialQuantities] = useState<{
    [key: string]: number;
  }>({});

  const resolveAssignedUserName = (
    jobOrder: Pick<JobOrder, "assignedTo" | "assignedToName">,
  ) => {
    ...
    const matchedUser = users.find(
      (user) => String(user.id || "").trim() === assignedUserId,
    );
  ...
  const mapJobOrderFromApi = (jo: any): JobOrder => {
    ...
    return {
      ...
      linkedPrWorkflowStatus:
        jo.linked_pr_workflow_status ?? jo.linkedPrWorkflowStatus,
      linkedPurchaseFlow:
        jo.linked_purchase_flow ?? jo.linkedPurchaseFlow ?? null,
      ...
      expectedDurationHours:
        jo.expected_duration_hours ?? jo.expectedDurationHours,
      notes: jo.notes,
      createdAt: jo.created_at || jo.createdAt,
      operations: operationsRaw.map((op: any) => ({
        id: op.id,
        routingId: op.routing_id ?? op.routingId,
        ...
        expectedDurationHours:
          op.expected_duration_hours ?? op.expectedDurationHours,
        setupTimeHours: op.setup_time_hours ?? op.setupTimeHours,
        acceptedVariationPercent:
          op.accepted_variation_percent ?? op.acceptedVariationPercent,
        status: op.status,
        notes: op.notes,
        eligibleWorkstations: (op.eligible_workstations || []).map(
          (station: any) => ({
            id: station.id,
            name: station.name,
            type: station.type,
            primary: Boolean(station.primary),
          }),
        ),
        dispatches: (op.dispatches || []).map((dispatch: any) => ({
          id: dispatch.id,
          dispatchNumber:
            dispatch.dispatch_number ?? dispatch.dispatchNumber,
          workStationId:
            dispatch.work_station_id ?? dispatch.workStationId,
          workStationName:
            dispatch.work_station?.station_name ??
            dispatch.workStationName ??
            "Machine",
          assignedQuantity: Number(
            dispatch.assigned_quantity ?? dispatch.assignedQuantity ?? 0,
          ),
          completedQuantity: Number(
            dispatch.completed_quantity ?? dispatch.completedQuantity ?? 0,
          ),
          rejectedQuantity: Number(
            dispatch.rejected_quantity ?? dispatch.rejectedQuantity ?? 0,
          ),
          plannedStart: dispatch.planned_start ?? dispatch.plannedStart,
          plannedEnd: dispatch.planned_end ?? dispatch.plannedEnd,
          assignedOperatorId:
            dispatch.assigned_operator_id ?? dispatch.assignedOperatorId,
          assignedOperatorName:
            dispatch.assigned_operator_name ?? dispatch.assignedOperatorName,
  ...
  const openJobOrderDetails = async (jo: JobOrder) => {
    ...
    setQcSummary(null);
    setFactoryReadiness(null);
    try {
      const [detailsResult, qcSummaryResult, readinessResult] = await Promise.allSettled([
        apiClient.get(`/job-orders/${jo.id}`),
        apiClient.get<JobOrderQcSummary>(`/job-orders/${jo.id}/qc-summary`),
        apiClient.get(`/job-orders/${jo.id}/factory-readiness`),
      ]);
      ...
      if (readinessResult.status === "fulfilled") {
        setFactoryReadiness(readinessResult.value ?? null);
      }
    } catch (error) {
  ...
  useEffect(() => {
    const openJobOrderId = new URLSearchParams(window.location.search)
      .get("open")
      ?.trim();
    if (
      !openJobOrderId ||
      handledDeepLinkRef.current === openJobOrderId ||
      jobOrders.length === 0
    ) {
      return;
    }
    const matchingJobOrder = jobOrders.find((jo) => jo.id === openJobOrderId);
    if (!matchingJobOrder) return;
    handledDeepLinkRef.current = openJobOrderId;
    void openJobOrderDetails(matchingJobOrder);
  ...
  useEffect(() => {
    const ids = Array.from(
      new Set(
        materials.map((m) => String(m.itemId || "").trim()).filter(Boolean),
      ),
    );
    ids.forEach((id) => {
  ...
  const fetchJobOrders = async (options?: { silent?: boolean }) => {
    try {
      ...
      const data = await apiClient.get(
        "/job-orders",
        restrictToAssignedJobs ? { myAssigned: "true" } : undefined,
      );
  ...
  const refreshSelectedJobOrder = async (jobOrderId: string) => {
    try {
      const [detailsResult, qcSummaryResult, readinessResult] = await Promise.allSettled([
        ...
        apiClient.get<JobOrderQcSummary>(
          `/job-orders/${jobOrderId}/qc-summary`,
        ),
        apiClient.get(`/job-orders/${jobOrderId}/factory-readiness`),
      ]);
  ...
  const handlePrintSelectedJobOrder = async () => {
    ...
    printWindow.document.write(
      '<!doctype html><html><head><title>Preparing Job Order...</title></head><body style="font-family: Arial, sans-serif; padding: 16px;">Preparing Job Order...</body></html>',
    );
    ...
    const joDateOptions: Intl.DateTimeFormatOptions = {
      day: "2-digit",
      month: "short",
      year: "numeric",
      hour: "2-digit",
    ...
    const formatDate = (value?: string | null) => {
      ...
      return Number.isNaN(parsed.getTime())
        ? "-"
        : parsed.toLocaleString("en-IN", joDateOptions);
    };
    ...
    const workflowRows = [
      ...
      ["Material Requisition", materialIssueStage(selectedJobOrder)],
      [
        "Purchase",
        selectedJobOrder.linkedPrNumber
          ? mapPurchaseWorkflowStatus(
              selectedJobOrder.linkedPrWorkflowStatus,
            ) || "Requisition Issued"
          : "Not Linked",
      ],
      [
        "Linked PR",
        linkedFlow?.prNumber || selectedJobOrder.linkedPrNumber || "-",
      ],
      [
        "Linked PO(s)",
        Array.isArray(linkedFlow?.poNumbers) && linkedFlow.poNumbers.length
          ? linkedFlow.poNumbers.join(", ")
          : "-",
      ],
      [
        "Linked GRN(s)",
        Array.isArray(linkedFlow?.grnNumbers) && linkedFlow.grnNumbers.length
          ? linkedFlow.grnNumbers.join(", ")
          : "-",
      ],
      [
        "Purchase Qty",
        linkedFlow
          ? `Ordered ${formatQty(linkedFlow.orderedQty)} / Received ${formatQty(linkedFlow.receivedQty)}`
          : "-",
      ],
      [
        "SIV / Material Issue",
        `Issued ${formatQty(sivSummary.quantity)} / Approved ${formatQty(sivSummary.approvedQuantity)} / Pending ${formatQty(sivSummary.pendingQuantity)}`,
      ],
      [
        "SRV / Finished Goods Receipt",
        `Received ${formatQty(srvSummary.quantity)} / Released ${formatQty(srvSummary.approvedQuantity)} / QC Hold ${formatQty(srvSummary.pendingQuantity)}`,
      ],
      [
        "QC",
        qcSummary?.isQcApplied
          ? `Applied (${qcSummary?.passedUidsCount ?? 0} passed / ${qcSummary?.rejectedUidsCount ?? 0} on-hold)`
          : "Pending",
      ],
    ];
      ...
      .map(
        (operation, index) => `
          <tr>
            ...
            <td>${(operation.dispatches || []).filter((row) => row.status !== "CANCELLED").map((row) => `${escapeHtml(row.dispatchNumber)} · ${escapeHtml(row.workStationName || "Machine")} · ${escapeHtml(String(row.assignedQuantity))}`).join("<br>") || "Not dispatched"}</td>
          </tr>
        `,
      )
      .join("");
    ...
    const html = `
      ...
      <body>
        <div class="page">
          ...
          <div class="title-row">
            <div>
              <div class="title">Production Sheet / Job Order</div>
              <div class="subtitle">Consolidated shop-floor production packet</div>
            </div>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th>Machine allocations / Job Cards</th>
              </tr>
            </thead>
            <tbody>
              ${operationsHtml || '<tr><td colspan="7">No operations available.</td></tr>'}
            </tbody>
          </table>
  ...
  const openDispatchOperation = (operation: Operation) => {
    if (!selectedJobOrder || !operation.id) return;
    const allocated = (operation.dispatches || [])
      .filter((row) => row.status !== "CANCELLED")
      .reduce((sum, row) => sum + Number(row.assignedQuantity || 0), 0);
    const remaining = Math.max(0, Number(selectedJobOrder.quantity || 0) - allocated);
    const primary = (operation.eligibleWorkstations || []).find(
      (station) => station.primary,
    );
    setDispatchOperation(operation);
    setDispatchForm({
      workStationId: primary?.id || operation.eligibleWorkstations?.[0]?.id || operation.workstationId || "",
      assignedQuantity: remaining ? String(remaining) : "",
      plannedStart: formatDateTimeLocalValue(operation.startDatetime || selectedJobOrder.startDate, ""),
      plannedEnd: formatDateTimeLocalValue(operation.endDatetime || selectedJobOrder.endDate, ""),
      assignedOperatorId: operation.assignedUserId || "",
      notes: "",
    });
  };

  const saveOperationDispatch = async () => {
    if (!selectedJobOrder || !dispatchOperation?.id) return;
    const operator = users.find(
      (row) => String(row.id) === String(dispatchForm.assignedOperatorId),
    );
    setDispatchBusy(true);
    try {
      await apiClient.post(
        `/job-orders/${selectedJobOrder.id}/operations/${dispatchOperation.id}/dispatches`,
        {
          workStationId: dispatchForm.workStationId,
          assignedQuantity: Number(dispatchForm.assignedQuantity),
          plannedStart: dispatchForm.plannedStart
            ? new Date(dispatchForm.plannedStart).toISOString()
            : null,
          plannedEnd: dispatchForm.plannedEnd
            ? new Date(dispatchForm.plannedEnd).toISOString()
            : null,
          assignedOperatorId: dispatchForm.assignedOperatorId || null,
          assignedOperatorName: operator?.displayName || null,
          notes: dispatchForm.notes || null,
        },
      );
      setDispatchOperation(null);
      await refreshSelectedJobOrder(selectedJobOrder.id);
    } catch (error: any) {
      await confirmDialog({
        title: "Unable to dispatch operation",
        message: error?.message || "The machine allocation could not be saved.",
        confirmLabel: "OK",
        cancelLabel: "Close",
        variant: "warning",
      });
  ...
  const cancelOperationDispatch = async (dispatch: OperationDispatch) => {
    if (!selectedJobOrder) return;
    const confirmed = await confirmDialog({
      title: "Cancel Job Card allocation",
      message: `Cancel ${dispatch.dispatchNumber}? This is allowed only before production starts.`,
    ...
    try {
      await apiClient.delete(
        `/job-orders/${selectedJobOrder.id}/dispatches/${dispatch.id}`,
      );
      await refreshSelectedJobOrder(selectedJobOrder.id);
    } catch (error: any) {
      await confirmDialog({
        title: "Unable to cancel",
        message: error?.message || "The Job Card allocation could not be cancelled.",
        confirmLabel: "OK",
        cancelLabel: "Close",
        variant: "warning",
      });
  ...
  const printJobCards = async (onlyDispatch?: OperationDispatch) => {
    if (!selectedJobOrder) return;
    ...
    const cards = (selectedJobOrder.operations || []).flatMap((operation) =>
      (operation.dispatches || [])
        .filter(
          (dispatch) =>
            dispatch.status !== "CANCELLED" &&
            (!onlyDispatch || dispatch.id === onlyDispatch.id),
        )
        .map((dispatch) => ({ operation, dispatch })),
    );
    if (!cards.length) {
      await confirmDialog({
        title: "No Job Cards",
        message: "Allocate at least one operation to a machine before printing Job Cards.",
        confirmLabel: "OK",
        cancelLabel: "Close",
        variant: "warning",
      });
    ...
    const pages = await Promise.all(
      cards.map(async ({ operation, dispatch }) => {
        const qr = await QRCode.toDataURL(
          `MIZANTRA:JOB-CARD:${dispatch.id}`,
          { margin: 1, width: 180, errorCorrectionLevel: "M" },
        );
        return `<section class="card">
          ${renderStandardLetterheadHtml(branding, new Date().toLocaleString("en-IN"))}
          <div class="head"><div><h1>OPERATION JOB CARD</h1><p>Machine-specific production authorization</p></div><img src="${qr}" /></div>
          <div class="grid">
            <div><label>Job Card</label><b>${escapeHtml(dispatch.dispatchNumber)}</b></div>
            <div><label>Job Order</label><b>${escapeHtml(selectedJobOrder.jobOrderNumber)}</b></div>
            <div><label>Product</label><b>${escapeHtml(`${selectedJobOrder.itemCode} - ${selectedJobOrder.itemName}`)}</b></div>
            <div><label>Operation</label><b>${escapeHtml(`${operation.sequenceNumber}. ${operation.operationName}`)}</b></div>
            <div><label>Machine</label><b>${escapeHtml(dispatch.workStationName || "-")}</b></div>
            <div><label>Operator</label><b>${escapeHtml(dispatch.assignedOperatorName || "Floor assignment")}</b></div>
            <div><label>Assigned Quantity</label><b>${escapeHtml(String(dispatch.assignedQuantity))}</b></div>
            <div><label>Status</label><b>${escapeHtml(dispatch.status)}</b></div>
            <div><label>Planned Start</label><b>${escapeHtml(formatCardDate(dispatch.plannedStart))}</b></div>
            <div><label>Planned End</label><b>${escapeHtml(formatCardDate(dispatch.plannedEnd))}</b></div>
          </div>
          <table><thead><tr><th>Good Qty</th><th>Rejected Qty</th><th>Rework Qty</th><th>Start</th><th>Finish</th></tr></thead><tbody><tr><td></td><td></td><td></td><td></td><td></td></tr></tbody></table>
          <div class="notes"><b>Instructions / notes:</b> ${escapeHtml(dispatch.notes || operation.notes || "-")}</div>
          <div class="sign"><div>Operator</div><div>Supervisor</div><div>QC</div></div>
        </section>`;
      }),
    );
    printWindow.document.open();
    printWindow.document.write(`<!doctype html><html><head><title>Job Cards - ${escapeHtml(selectedJobOrder.jobOrderNumber)}</title><style>
      @page{size:A4;margin:10mm}*{box-sizing:border-box}body{font:11px Arial;color:#172033;margin:0}.card{page-break-after:always}.card:last-child{page-break-after:auto}.head{display:flex;justify-content:space-between;align-items:center;border-bottom:3px solid #234f85;padding:8px 0}.head h1{margin:0;color:#234f85;font-size:22px}.head p{margin:4px 0;color:#667085}.head img{width:92px;height:92px}.grid{display:grid;grid-template-columns:1fr 1fr;border:1px solid #bcc7d7;margin:14px 0}.grid div{padding:9px;border:1px solid #d5dbe7}.grid label{display:block;font-size:9px;text-transform:uppercase;color:#667085;margin-bottom:4px}table{width:100%;border-collapse:collapse;margin-top:14px}th,td{border:1px solid #aeb9c8;padding:10px;text-align:left}th{background:#eef3f9}.notes{border:1px solid #aeb9c8;min-height:70px;padding:10px;margin-top:14px}.sign{display:flex;gap:30px;margin-top:55px}.sign div{flex:1;border-top:1px solid #111;padding-top:6px;text-align:center}
  ...
  const fetchSalesOrderItemsWithBlocked = async (salesOrderId: string) => {
    ...
    try {
      ...
      (Array.isArray(mappedJobOrders) ? mappedJobOrders : []).forEach(
        (jo: any) => {
          const status = String(jo?.status || "").toUpperCase();
          if (status === "COMPLETED" || status === "CANCELLED") return;
          const soItemId = String(
            jo?.sales_order_item_id || jo?.salesOrderItemId || "",
          ).trim();
          if (!soItemId) return;
          blockedBySoItemId.set(
            soItemId,
            (blockedBySoItemId.get(soItemId) || 0) +
              (Number(jo?.quantity || 0) || 0),
          );
        },
      );
      ...
      const mappedItems = (Array.isArray(items) ? items : [])
        .map((row: any) => {
          ...
          const remainingQty = Math.max(
            0,
            orderedQty - dispatchedQty - blockedQty,
          );
          ...
          return {
            ...
            itemLabel:
              [itemCode, itemDescription].filter(Boolean).join(" - ") || id,
            orderedQty,
            dispatchedQty,
            blockedQty,
            remainingQty,
  ...
  useEffect(() => {
    ...
    const selected = salesOrderItems.find(
      (row) => row.id === formData.salesOrderItemId,
    );
    if (!selected) return;
    ...
    setFormData((prev) => {
      ...
      if (!prev.quantity || prev.quantity <= 1)
        next.quantity = Math.max(1, Math.floor(selected.remainingQty || 1));
      return next;
    });
  ...
  const fetchBOMData = async (bomId: string) => {
    ...
    try {
      ...
      const materialsWithVariantsRaw = await Promise.all(
          ...
          try {
            ...
            if (variants && variants.length > 0) {
              const defaultVariant =
                variants.find((v: any) => v.is_default_variant) || variants[0];
              selectedVariantId = defaultVariant.id;
              selectedVariantName =
                defaultVariant.variant_name || defaultVariant.name;
            }
          } catch (error) {}
      ...
      const routing = await apiClient.get(
        `/production/routing/bom/${bomId}?withStations=true`,
      );
  ...
  const updateMaterialVariant = (index: number, variantId: string) => {
    ...
    const variant = updated[index].variants?.find(
  ...
  const changeMaterialItem = async (index: number, nextItemId: string) => {
    ...
    setMaterials((prev) => {
      const next = [...prev];
      const current =
        next[index] || ({ itemId: "", requiredQuantity: 0 } as Material);
    ...
    try {
      ...
      if (formData.bomId) {
        ...
        if (linkedPrNumber) {
          ...
          if (openPr) {
            router.push(
              `/dashboard/purchase/requisitions?search=${encodeURIComponent(linkedPrNumber)}`,
            );
          }
        } else {
          const openSiv = await confirmDialog({
            title: "Smart Job Order Created",
            message:
              "BOM planning is complete. Open the SIV screen now to print and issue available materials?",
          ...
          if (openSiv) {
            router.push(
              `/dashboard/inventory/siv?jobId=${encodeURIComponent(createdJobOrder.id)}&joNumber=${encodeURIComponent(createdJobOrder.jobOrderNumber)}`,
            );
          }
        }
        return;
      ...
      const payload: any = {
        ...
        expectedDurationHours: formData.expectedDurationHours
          ? Number(formData.expectedDurationHours)
          : undefined,
      ...
      if (operations.length > 0) {
        payload.operations = operations.map((op) => ({
          routingId: op.routingId || undefined,
      ...
      const linkedPrNumber = String(
        (response as any)?.linked_pr_number ||
          (response as any)?.linkedPrNumber ||
          "",
      ).trim();
      const sivReady = Boolean(
        (response as any)?.siv_ready ??
        (response as any)?.sivReady ??
        createdJobOrder.sivReady,
      );
      ...
      if (linkedPrNumber) {
        ...
        const openSiv = await confirmDialog({
          title: "Job Order Created",
          message:
            "Materials are available. Open the SIV screen now to print and issue materials?",
          confirmLabel: "Open SIV Screen",
          variant: "warning",
        });
      ...
      if (errorMessage.includes("Insufficient materials")) {
        ...
        const shortageLines = lines
          .slice(1)
          .filter((line: string) => line.trim()); // Skip the first line and empty lines

        const shortages = shortageLines
          .map((line: string) => {
            // Parse lines like "SG1 - L80 GPS: Need 1, Available 0, Short 1"
            const match = line.match(
              /^(.+?):\s*Need\s+(\d+(?:\.\d+)?),\s*Available\s+(\d+(?:\.\d+)?),\s*Short\s+(\d+(?:\.\d+)?)$/,
            );
            if (match) {
              return {
        ...
        await confirmDialog({
          ...
          message:
            errorMessage ||
            "Something went wrong while creating the job order.",
          confirmLabel: "OK",
          cancelLabel: "Close",
          variant: "warning",
        });
  ...
  const handleUpdateStatus = async (id: string, status: string) => {
    ...
    try {
        ...
        try {
          ...
          if (!readiness.ready && readiness.blockers.length > 0) {
            const lines = readiness.blockers
              .map((b) => {
                const jo = b.pendingSubJoNumber
                  ? ` (JO: ${b.pendingSubJoNumber})`
                  : "";
          ...
          const errMsg = String(
            (readinessErr as any)?.response?.data?.message ||
              (readinessErr as any)?.message ||
              "network error",
          );
          await confirmDialog({
      ...
      fetchJobOrders();
      if (status === "IN_PROGRESS") {
        const jobOrder = jobOrders.find((row) => row.id === id);
        router.push(
          `/dashboard/shop-floor?jobId=${encodeURIComponent(id)}${
            jobOrder?.jobOrderNumber
              ? `&joNumber=${encodeURIComponent(jobOrder.jobOrderNumber)}`
              : ""
          }`,
        );
        return;
      ...
      if (blockedByStoreIssue) {
        const goToStore = await confirmDialog({
          title: "SIV Pending",
          message:
            "Cannot start this Job Order yet because SIV (material issue) is pending. Open Inventory SIV screen now?",
          confirmLabel: "Open SIV Screen",
          variant: "warning",
        });
        if (goToStore) {
          router.push(
            `/dashboard/inventory/siv?jobId=${encodeURIComponent(id)}`,
          );
        }
        return;
      }
  ...
  const openEditModal = (jo: JobOrder) => {
    ...
    setFormData({
      ...
      startDate: jo.startDate
        ? formatDateTimeLocalValue(jo.startDate)
        : `${getTodayDateInputValue()}T09:00`,
      endDate: jo.endDate ? formatDateTimeLocalValue(jo.endDate) : "",
      priority: jo.priority,
      assignedTo: jo.assignedTo || "",
      expectedDurationHours:
        jo.expectedDurationHours !== undefined
          ? String(jo.expectedDurationHours)
          : "",
      notes: jo.notes || "",
    });
  ...
  const handleStopJobOrder = async () => {
    ...
    try {
      ...
      const producedQuantity =
        Number.isFinite(producedQty) && producedQty > 0
          ? producedQty
          : undefined;
      const result = await apiClient.post(
        `/job-orders/${jobOrderToStop.id}/stop`,
        {
      ...
      if ((result as any)?.srvPending) {
        ...
        if (openSrv) {
          router.push(
            `/dashboard/inventory/srv?jobId=${encodeURIComponent(stoppedJob.id)}&joNumber=${encodeURIComponent(stoppedJob.jobOrderNumber)}`,
          );
        }
      } else {
        await confirmDialog({
          title: "Job Order Stopped",
          message:
            (result as any)?.message || "Job Order stopped successfully.",
          confirmLabel: "OK",
          cancelLabel: "Close",
          variant: "info",
        });
  ...
  const handleCompleteJobOrder = async (id: string) => {
    ...
    try {
      ...
      const preview = await apiClient.get(
        `/job-orders/${id}/completion-preview`,
      );
  ...
  const confirmPartialCompleteJobOrder = async () => {
    ...
    try {
      const result = await apiClient.post(
        `/job-orders/${jo.id}/complete-partial`,
        {
          producedQuantity,
        },
      );
      ...
      const nextCompleted = Number(
        (result as any)?.completedQuantity ?? already + producedQuantity,
      );
      const openSrv = await confirmDialog({
      ...
      if (openSrv) {
        router.push(
          `/dashboard/inventory/srv?jobId=${encodeURIComponent(jo.id)}&joNumber=${encodeURIComponent(jo.jobOrderNumber)}`,
        );
      }
    } catch (error: any) {
      const errorMsg =
        error.response?.data?.message ||
        error.message ||
        "Failed to record partial completion";
      await confirmDialog({
  ...
  const calculateJobOrderDays = (
    jo: Pick<JobOrder, "actualStartDate" | "actualEndDate" | "status">,
  ) => {
  ...
  const getJobOrderWorklistStage = (jo: JobOrder): JobOrderTab => {
    ...
    const linkedPurchaseKey = normalizeStatusKey(
      jo.linkedPurchaseFlow?.status || jo.linkedPrWorkflowStatus,
    );
    const hasLinkedPr = Boolean(
      String(jo.linkedPrNumber || jo.linkedPurchaseFlow?.prNumber || "").trim(),
    );
    const purchaseDone = ["GOODS_RCVD", "GOODS_RECEIVED"].includes(
      linkedPurchaseKey,
    );
    ...
    const plannedQuantity = Number(jo.quantity || 0);
    const completedQuantity = Number(jo.completedQuantity || 0);
    if (
      baseKey === "IN_PROGRESS" &&
      plannedQuantity > 0 &&
      completedQuantity + 1e-9 < plannedQuantity
    ) {
      // A partial SRV/QC receipt does not close the unproduced balance. Keep
      // the Job Order in Production until the plan is met or it is closed short.
      return "production";
    }
  ...
  const getJobOrderDisplayStatus = (
    jo: JobOrder | null,
    summary: JobOrderQcSummary | null,
  ) => {
    if (!jo) return "-";
  ...
  const jobOrdersTableColumns: Array<ListTableColumn<JobOrder>> = [
    ...
    },
    {
      ...
      cell: (jo) => (
        <div className="min-w-0 max-w-full overflow-hidden">
          <div className="font-medium leading-5 text-gray-900 [overflow-wrap:anywhere]">
            {jo.itemCode}
          </div>
          <div
            className="text-sm leading-5 text-gray-500 [overflow-wrap:anywhere]"
            title={jo.itemName}
          >
            {jo.itemName}
          </div>
    ...
    },
    {
      ...
      cell: (jo) => (
        <div className="text-right">
          <div className="font-semibold text-[#4A3426]">
            {formatQuantityValue(jo.quantity)}
          </div>
          {Number(jo.completedQuantity || 0) > 0 ? (
            <div className="text-[11px] text-emerald-700">
              Done {formatQuantityValue(jo.completedQuantity)}
            </div>
          ) : null}
        </div>
      ),
    },
    {
      ...
      sortAccessor: (jo) =>
        jo.startDate ? new Date(jo.startDate).getTime() : 0,
      cell: (jo) => (
        <span className="text-xs">{formatDisplayDateTime(jo.startDate)}</span>
      ),
    },
    {
      id: "actualStartDate",
      ...
      sortAccessor: (jo) =>
        jo.actualStartDate ? new Date(jo.actualStartDate).getTime() : 0,
      cell: (jo) => (
        <span className="text-xs">{formatDisplayDate(jo.actualStartDate)}</span>
      ),
    },
    {
      id: "days",
      ...
      cell: (jo) => {
        ...
        return days !== null ? (
          <span
            className={`text-xs font-medium ${jo.status === "IN_PROGRESS" ? "text-blue-600" : ""}`}
          >
    ...
    },
    {
      ...
      cell: (jo) => (
        <ErpStatusBadge
          ...
          tone={
            jo.priority === "URGENT" || jo.priority === "HIGH"
              ? "danger"
              : jo.priority === "LOW"
                ? "neutral"
                : "warning"
    ...
    },
    {
      ...
      cell: (jo) => {
        ...
        const status = mapPurchaseWorkflowStatus(
          jo.linkedPrWorkflowStatus || flow?.status,
        );
        const isMissingPr =
          normalizeStatusKey(jo.linkedPrWorkflowStatus || flow?.status) ===
          "PR_NOT_FOUND";
        return (
          <div className="min-w-[190px] space-y-1">
            <button
              ...
              onClick={(event) => {
                ...
                router.push(
                  `/dashboard/purchase/requisitions?search=${encodeURIComponent(prNumber)}`,
                );
              }}
            ...
            <div className="flex flex-wrap items-center gap-1">
              <button
                ...
                onClick={(event) => {
                  event.stopPropagation();
                  router.push(
                    `/dashboard/inventory/siv?jobId=${encodeURIComponent(jo.id)}&joNumber=${encodeURIComponent(jo.jobOrderNumber)}`,
                  );
                }}
              ...
              {sivPendingQty > 0 ? (
                <span
                  ...
                  title="Pending approval quantity"
                >
                  Pending {sivPendingQty}
                </span>
              ) : null}
            </div>
            <div className="flex flex-wrap items-center gap-1">
              ...
              <span title="Received finished goods quantity">
                Recv {srvQty}
              </span>
              <span
                className="text-emerald-700"
                title="Released to stock quantity"
              >
                OK {srvApprovedQty}
              </span>
              {srvPendingQty > 0 ? (
                <span className="text-amber-700" title="QC hold quantity">
                  QC hold {srvPendingQty}
                </span>
              ) : null}
            </div>
          </div>
    ...
    },
    {
      ...
      cell: (jo) => {
        const isAssignedToCurrentUser =
          !!currentUserId &&
          String(jo.assignedTo || "").trim() === currentUserId;
        const canOperateAssignedJob =
          restrictToAssignedJobs && isAssignedToCurrentUser;
        ...
        return (
          <div className="flex flex-wrap items-center justify-end gap-1.5 text-sm">
            ...
            {["DRAFT", "SCHEDULED", "IN_PROGRESS"].includes(jo.status) &&
              canEdit && (
                <ErpButton
                  type="button"
            ...
            {jo.status === "IN_PROGRESS" &&
              (canApprove || canOperateAssignedJob) && (
                <>
  ...
  const countByStage = (stage: JobOrderTab) =>
    jobOrders.filter((jo) => getJobOrderWorklistStage(jo) === stage).length;
  ...
  const runningJobOrderCount = jobOrders.filter(
    (jo) => normalizeStatusKey(jo.status) === "IN_PROGRESS",
  ).length;

  const isLateJobOrder = (jo: JobOrder) => {
    const due = new Date(jo.endDate || jo.startDate || "");
    return (
      !Number.isNaN(due.getTime()) &&
      due.getTime() < Date.now() &&
      !isCompletedJobOrder(jo) &&
      !isStoppedJobOrder(jo)
    );
  };

  const supervisorPriority = (jo: JobOrder) => {
    const stage = getJobOrderWorklistStage(jo);
    const priority = normalizeStatusKey(jo.priority);
    return (
      (isLateJobOrder(jo) ? 1000 : 0) +
      (["URGENT", "HIGH"].includes(priority) ? 300 : 0) +
      (
        {
          all_open: 0,
          purchase: 220,
          ...
          completed: 0,
          stopped: 0,
        } satisfies Record<JobOrderTab, number>
      )[stage]
    );
  };

  const supervisorQueue = jobOrders
    .filter((jo) => {
      const stage = getJobOrderWorklistStage(jo);
      return stage !== "completed" && stage !== "stopped";
    })
    .sort((left, right) => supervisorPriority(right) - supervisorPriority(left))
    .slice(0, 8);

  const getSupervisorDecision = (jo: JobOrder) => {
    const stage = getJobOrderWorklistStage(jo);
    const planned = Number(jo.quantity || 0);
    const completed = Number(jo.completedQuantity || 0);
    const remaining = Math.max(0, planned - completed);
    if (stage === "purchase") {
      return {
        label: "Follow purchase",
        reason: "Required material is still moving through PR, PO or GRN.",
      };
    }
    if (stage === "siv") {
      return {
        label: "Issue material",
        reason: "Material is waiting for Stores issue through SIV.",
      };
    }
    if (stage === "srv_qc") {
      return {
        label: "Receive / QC",
        reason:
          "Produced quantity is waiting for Stores receipt or QC release.",
      };
    }
    if (normalizeStatusKey(jo.status) === "DRAFT") {
      return {
        label: "Schedule",
        reason: "Assign an operator and planned start time.",
      };
    }
    if (normalizeStatusKey(jo.status) === "SCHEDULED") {
      return {
        label: "Start production",
        reason: "The order is scheduled and ready for its material check.",
      };
    }
    if (normalizeStatusKey(jo.status) === "IN_PROGRESS") {
      if (completed > 0 && remaining > 0) {
        return {
          label: "Produce remaining",
          reason: `${formatQuantityValue(remaining)} pieces remain. Continue production or close the Job Order short.`,
        };
      }
      return {
        label: "Open shop floor",
        reason: "Production is running; record output, rejection and downtime.",
      };
    }
    return {
      label: "Review order",
      reason: "Review the order's current state.",
    };
  };

  const openRemainingProduction = async (jo: JobOrder) => {
    setLoading(true);
    try {
      await apiClient.post(`/job-orders/${jo.id}/resume-remaining`, {});
      router.push(
        `/dashboard/shop-floor?jobId=${encodeURIComponent(jo.id)}&joNumber=${encodeURIComponent(jo.jobOrderNumber)}`,
      );
    } catch (error: any) {
      await confirmDialog({
        title: "Could Not Reopen Production",
        message:
          error?.response?.data?.message ||
          error?.message ||
          "The remaining production quantity could not be reopened.",
        confirmLabel: "OK",
        cancelLabel: "Close",
        variant: "warning",
      });
  ...
  const handleSupervisorDecision = (jo: JobOrder) => {
    const stage = getJobOrderWorklistStage(jo);
    if (stage === "purchase") {
      const prNumber = jo.linkedPrNumber || jo.linkedPurchaseFlow?.prNumber;
      router.push(
        `/dashboard/purchase/requisitions${prNumber ? `?search=${encodeURIComponent(prNumber)}` : ""}`,
      );
      return;
    }
    if (stage === "siv") {
    ...
    if (normalizeStatusKey(jo.status) === "DRAFT") {
      openScheduleModal(jo);
      return;
    }
    if (normalizeStatusKey(jo.status) === "SCHEDULED") {
      void handleUpdateStatus(jo.id, "IN_PROGRESS");
      return;
    }
    if (normalizeStatusKey(jo.status) === "IN_PROGRESS") {
      const planned = Number(jo.quantity || 0);
      const completed = Number(jo.completedQuantity || 0);
      if (completed > 0 && completed + 1e-9 < planned) {
        void openRemainingProduction(jo);
      } else {
        router.push(
          `/dashboard/shop-floor?jobId=${encodeURIComponent(jo.id)}&joNumber=${encodeURIComponent(jo.jobOrderNumber)}`,
        );
    ...
    void openJobOrderDetails(jo);
  };

  const jobOrderTabs: Array<{
    id: JobOrderTab;
    label: string;
    count: number;
    tone: "brown" | "amber" | "blue" | "cyan" | "green" | "red";
  }> = [
    {
      id: "all_open",
      label: "All Open",
      count: activeJobOrderCount,
      tone: "brown",
    },
    {
      id: "purchase",
    ...
    {
      id: "siv",
      label: "SIV / Material Issue",
      count: awaitingMaterialCount,
      tone: "blue",
    },
    {
      id: "production",
    ...
    { id: "srv_qc", label: "SRV / QC", count: awaitingQcCount, tone: "cyan" },
    {
      id: "completed",
      label: "QC Completed",
      count: completedJobOrderCount,
      tone: "green",
    },
    {
      id: "stopped",
      label: "Stopped / Cancelled",
      count: stoppedJobOrderCount,
      tone: "red",
    },
  ];
  const activeTabLabel =
    jobOrderTabs.find((tab) => tab.id === activeTab)?.label.toLowerCase() ||
    "job";
  const tabClassName = (tab: {
    ...
    if (active) {
      const activeClasses: Record<
        "brown" | "amber" | "blue" | "cyan" | "green" | "red",
        string
      > = {
    ...
    <div className="min-h-screen bg-[#FAF9F6] p-6">
      <div className="space-y-4">
        <ErpPageHeader
          ...
          description="See what should run now, what is blocked and the one next action needed from your team."
          actions={
            <div className="flex flex-wrap gap-2">
              <ErpButton
                variant="secondary"
                onClick={() => setShowFullRegister((value) => !value)}
              >
                {showFullRegister
        ...
        <ErpMetricStrip
          ...
          metrics={[
            {
              label: "Open orders",
              value: activeJobOrderCount,
              tone: "warning",
            },
            {
              label: "Running now",
              value: runningJobOrderCount,
              tone: runningJobOrderCount > 0 ? "success" : "neutral",
            },
            {
              label: "Material blocked",
              value: purchasePendingCount + awaitingMaterialCount,
              tone:
                purchasePendingCount + awaitingMaterialCount > 0
                  ? "warning"
                  : "neutral",
            },
            {
              label: "Waiting receipt / QC",
              value: awaitingQcCount,
              tone: awaitingQcCount > 0 ? "warning" : "neutral",
            },
          ]}
        />

        {!showFullRegister ? (
          <section className="rounded-2xl border border-[#E8DCC4] bg-white p-4 shadow-sm sm:p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-[#7A6555]">
                  Mizantra ranks late work, shortages and high-priority orders
                  first.
                </p>
              </div>
              <span className="rounded-full bg-[#F3EBDD] px-3 py-1 text-xs font-bold text-[#6F4E37]">
                {supervisorQueue.length} shown
              </span>
            </div>

            {supervisorQueue.length === 0 && !loading ? (
              <div className="mt-4 flex items-start gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-900">
                ...
                <div>
                  <p className="font-bold">No open production decision</p>
                  <p className="mt-1 text-sm">
                    There is no job order requiring supervisor action right now.
                  </p>
                </div>
              </div>
            ) : null}
            ...
            <div className="mt-4 grid gap-3 xl:grid-cols-2">
              {supervisorQueue.map((jo) => {
                const decision = getSupervisorDecision(jo);
                const late = isLateJobOrder(jo);
                const stage = getJobOrderWorklistStage(jo);
                const completed = Number(jo.completedQuantity || 0);
                const progress =
                  jo.quantity > 0
                    ? Math.min(100, (completed / jo.quantity) * 100)
                    : 0;
                const hasOpenProductionBalance =
                  normalizeStatusKey(jo.status) === "IN_PROGRESS" &&
                  completed > 0 &&
                  completed + 1e-9 < Number(jo.quantity || 0);
                return (
                  <article
                    key={jo.id}
                    className={`rounded-xl border p-4 ${late ? "border-red-200 bg-red-50" : "border-[#E8DCC4] bg-[#FFFCF7]"}`}
                  >
                      ...
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          <button
                            type="button"
                            onClick={() => openJobOrderDetails(jo)}
                            className="font-bold text-[#3E2A1F] hover:underline"
                          >
                            {jo.jobOrderNumber}
                          </button>
                          {late ? (
                          ...
                          <span className="rounded-full bg-white px-2 py-0.5 text-[11px] font-semibold uppercase text-[#80613D]">
                            {stage.replace("_", " / ")}
                          </span>
                        </div>
                        <p className="mt-1 truncate text-sm font-semibold text-[#4A3426]">
                          {jo.itemCode} · {jo.itemName}
                        </p>
                        <p className="mt-1 text-xs text-[#7A6555]">
                          Plan {formatQuantityValue(jo.quantity)} · Done{" "}
                          {formatQuantityValue(completed)} · Start{" "}
                          {formatDisplayDate(jo.startDate)}
                        </p>
                      </div>
                      {stage === "production" ? (
                        <Factory className="h-5 w-5 shrink-0 text-[#8B6F47]" />
                      ) : stage === "purchase" || stage === "siv" ? (
                        <PackageSearch className="h-5 w-5 shrink-0 text-amber-700" />
                      ) : (
                        <AlertTriangle className="h-5 w-5 shrink-0 text-cyan-700" />
                      )}
                    </div>
                    <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-[#E9E0D3]">
                      <div
                    ...
                    <div className="mt-3 flex items-end justify-between gap-3">
                      ...
                      <div className="flex shrink-0 flex-wrap justify-end gap-2">
                        {hasOpenProductionBalance && canEdit ? (
                          <button
                            type="button"
                            onClick={() => openStopModal(jo)}
                            disabled={loading}
                        ...
                        <button
                          type="button"
                          onClick={() => handleSupervisorDecision(jo)}
                          disabled={loading}
                          className="inline-flex items-center gap-1 rounded-lg bg-[#4A3526] px-3 py-2 text-xs font-bold text-white disabled:opacity-50"
                        >
                          {decision.label}
        ...
        {showFullRegister ? (
          <>
              ...
              <div className="grid gap-px bg-[#E8DCC4] text-sm md:grid-cols-5">
                {[
                  [
                    "1",
                    "Create JO",
                    "Create even when raw/sub-assembly stock is short.",
                  ],
                  [
                    "2",
                    "Shortage PR",
                    "Auto PR follows approval, RFQ, PO, GRN.",
                  ],
                  [
                    "3",
                    "SIV Issue",
                    "Stores issues available material to production.",
                  ],
                  ...
                  <div key={step} className="bg-[#FFFDF8] p-3">
                    <div className="mb-1 flex items-center gap-2">
                      <span className="flex h-6 w-6 items-center justify-center rounded-full bg-[#8B6F47] text-xs font-bold text-white">
                        {step}
                      </span>
            ...
            <div className="rounded-xl border border-[#E8DCC4] bg-white p-3 shadow-sm">
              <label className="block text-xs font-bold uppercase tracking-wide text-[#7A6555]">
                Worklist by next action
              </label>
              <div className="mt-2 flex flex-wrap items-center gap-3">
                <select
                  value={activeTab}
                  onChange={(event) =>
                    setActiveTab(event.target.value as JobOrderTab)
                  }
                  className="min-w-64 rounded-md border border-[#D8C8AA] bg-white px-3 py-2 text-sm font-semibold text-[#4A3426]"
                >
                ...
                <span className="text-xs text-[#7A6555]">
                  Choose the stage that needs action; counts remain visible in
                  the summary above.
                </span>
              </div>
            </div>
            ...
            <div className="mb-4 flex items-start justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {jobOrderToSchedule.jobOrderNumber} ·{" "}
                  {jobOrderToSchedule.itemCode}
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
                ...
                <div className="space-y-2">
                    ...
                    {boms.map((bom) => {
                      ...
                      return (
                        <option key={bom.id} value={bom.id}>
                          BOM for: {itemCode} - {itemName} (v{version}) |{" "}
                          {status}
                        </option>
                      );
                    })}
                  ...
                  {!bomSearchTerm && allBoms.length === 0 && (
                    <p className="text-xs text-red-600">
                      No BOMs found in system. Create a BOM first.
                    </p>
                  )}
                  <div className="flex items-center gap-2 pt-1">
                    <button
                      type="button"
                      onClick={() =>
                        formData.bomId && fetchBOMData(formData.bomId)
                      }
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Item is automatically set when you select a BOM
                </p>
              </div>

              <div>
                <label className="block text-sm font-medium mb-1">
                  Sales Order Mapping (Optional)
                </label>
                <select
                ...
                >
                  ...
                  {openSalesOrders.map((so) => (
                    <option key={so.id} value={so.id}>
                      {so.soNumber} - {so.customerName || "Customer"} (
                      {so.status})
                    </option>
                  ))}
                </select>
                ...
                >
                  ...
                  {salesOrderItems.map((row) => (
                    <option key={row.id} value={row.id}>
                      {row.itemLabel} | Ordered {row.orderedQty} | Dispatched{" "}
                      {row.dispatchedQty} | Blocked {row.blockedQty} | Open{" "}
                      {row.remainingQty}
                    </option>
                  ))}
                </select>
                  ...
                  onChange={(e) => {
                    ...
                    if (Object.keys(baseMaterialQuantities).length > 0) {
                      const updatedMaterials = materials.map((mat) => ({
                        ...mat,
                        requiredQuantity:
                          baseMaterialQuantities[mat.itemId] * newQuantity,
                      }));
            ...
            <div className="mb-6">
              ...
              <div className="space-y-3">
                {operations.map((op, idx) => (
                  <div
                    key={idx}
                    className="border border-[#E8DCC4] rounded p-4 bg-white"
                  >
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      ...
                      <div className="col-span-2">
                        ...
                        <input
                          ...
                          value={op.notes || ""}
                          onChange={(e) =>
                            updateOperation(idx, "notes", e.target.value)
                          }
                          className="w-full border rounded px-2 py-1 text-sm"
              ...
              <div className="space-y-2">
                {materials.length === 0 && (
                  <p className="text-sm text-gray-500 italic">
                    No materials added. Click &quot;+ Add Material&quot; or
                    select an item with BOM to auto-load materials.
                  </p>
                )}
                {materials.map((mat, idx) => {
                  // Find item details including current stock
                  const selectedItem = items.find(
                    (i) => String(i.id) === String(mat.itemId),
                  );
                  const summary =
                    itemStockSummaryById[String(mat.itemId || "").trim()];
                  const stockInHand = summary?.total_quantity ?? 0;
                  const stockAvailable =
                    summary?.available_quantity ??
                    selectedItem?.total_stock ??
                    0;
                  ...
                  return (
                    <div
                      key={idx}
                      className="flex gap-3 items-center border border-[#E8DCC4] rounded p-3 bg-white"
                    >
                      ...
                      <div className="w-32 text-sm">
                        ...
                        <div
                          className={`font-semibold ${isShort ? "text-red-600" : "text-green-600"}`}
                        >
                          ...
                          {isShort && (
                            <span className="text-xs block text-red-500">
                              Short: {mat.requiredQuantity - stockAvailable}
                            </span>
                          )}
                        </div>
                      </div>
          ...
          <div className="bg-white rounded-lg p-6 max-w-6xl w-full mx-4 my-8 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold">
                Job Order: {selectedJobOrder.jobOrderNumber}
              </h2>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6 p-4 bg-gray-50 rounded">
              {selectedJobOrderLoading ? (
                <div className="col-span-2 text-sm text-gray-600">
                  Loading materials & operations...
                </div>
              ) : null}
              <div>
                <strong>Item:</strong> {selectedJobOrder.itemCode} -{" "}
                {selectedJobOrder.itemName}
              </div>
              <div>
              ...
              <div>
                ...
                {formatDisplayDateTime(selectedJobOrder.startDate)}
              </div>
              <div>
              ...
              <div>
                ...
                {resolveAssignedUserName(selectedJobOrder) || (
                  ...
                  {(() => {
                    ...
                    return (
                      <>
                        ...
                        <span
                          className={`px-2 py-1 text-xs rounded ${getStatusColor(purchaseStatusLabel)}`}
                        >
                          {purchaseStatusLabel}
                        </span>
                      </>
              ...
              <div>
                ...
                {(() => {
                  const displayStatus = getJobOrderDisplayStatus(
                    selectedJobOrder,
                    qcSummary,
                  );
                  ...
                  return (
                    <span
                      className={`px-2 py-1 text-xs rounded ${getStatusColor(displayStatus)}`}
                    >
                      {formatStatusLabel(displayStatus)}
                      {rawStatus && displayStatus !== rawStatus
                        ? ` (${rawStatus})`
                        : ""}
                    </span>
                  );
              ...
              <div>
                ...
                {(() => {
                  const mats = Array.isArray(selectedJobOrder.materials)
                    ? selectedJobOrder.materials
                    : [];
                  ...
                  const status = materialIssueStage(selectedJobOrder);
                  return (
                    <span
              ...
              {(() => {
                const isAssignedToCurrentUser =
                  !!currentUserId &&
                  String(selectedJobOrder.assignedTo || "").trim() ===
                    currentUserId;
                const canOperateAssignedJob =
                  restrictToAssignedJobs && isAssignedToCurrentUser;
                const canCompleteSelectedJob =
                ...
                return (
                  <div className="col-span-2 flex justify-end">
                    {canCompleteSelectedJob && !canOperateAssignedJob && (
                      <button
                        onClick={() =>
                          openPartialCompleteModal(selectedJobOrder)
                        }
                        disabled={loading}
                    ...
                    {canCompleteSelectedJob && (
                      <button
                        onClick={() =>
                          handleCompleteJobOrder(selectedJobOrder.id)
                        }
                        disabled={loading}
              ...
              <div className="col-span-2 text-xs text-gray-600">
                Smart Job Orders create a Material Requisition. Issue materials
                via <strong>SIV</strong> before completing. Completion consumes
                any remaining and adds finished goods.
              </div>

              {factoryReadiness ? (
                <div className={`col-span-2 rounded-lg border p-3 ${factoryReadiness.ready_to_release ? "border-emerald-200 bg-emerald-50" : "border-amber-300 bg-amber-50"}`}>
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div>
                      <div className="text-xs font-bold uppercase tracking-wide">Factory readiness</div>
                      <div className="font-semibold">
                        {factoryReadiness.ready_to_release ? "Ready for controlled release" : "Action required before release"}
                      </div>
                    </div>
                    <div className="text-sm">
                      Reserved: <strong>{Number(factoryReadiness.material_summary?.reserved_quantity || 0)}</strong>
                      {" · "}Uncovered: <strong>{Number(factoryReadiness.material_summary?.uncovered_quantity || 0)}</strong>
                    </div>
                  </div>
                  {(factoryReadiness.blockers || []).length ? (
                    <div className="mt-3 space-y-1 text-sm">
                      {(factoryReadiness.blockers || []).map((blocker: any, index: number) => (
                        <div key={`${blocker.blocker_type}-${blocker.item_id}-${index}`} className="rounded border border-amber-200 bg-white px-3 py-2">
                          <strong>{String(blocker.blocker_code || blocker.blocker_type).replace(/_/g, " ")}</strong>
                          {": "}{blocker.detail || "Supply is not yet available"}
                        </div>
                      ))}
                    </div>
                  ) : null}
                  {(factoryReadiness.supply_actions || []).length ? (
                    <div className="mt-2 text-xs text-gray-700">
                      Supply worklist: {(factoryReadiness.supply_actions || []).map((action: any) => `${action.action_type} ${action.required_quantity}`).join(" · ")}
                    </div>
                  ) : null}
                </div>
              ) : null}

              {selectedJobOrder.linkedPurchaseFlow ||
              selectedJobOrder.linkedPrNumber ? (
                <div className="col-span-2 rounded-lg border border-amber-200 bg-amber-50/50 p-3">
                  <div className="mb-3 flex items-center justify-between gap-2">
                    <div className="text-xs font-bold uppercase tracking-wide text-amber-900">
                      Linked purchase trail
                    </div>
                    <span
                      className={`px-2 py-1 text-xs rounded ${getStatusColor(mapPurchaseWorkflowStatus(selectedJobOrder.linkedPurchaseFlow?.status || selectedJobOrder.linkedPrWorkflowStatus))}`}
                    >
                      {mapPurchaseWorkflowStatus(
                        selectedJobOrder.linkedPurchaseFlow?.status ||
                          selectedJobOrder.linkedPrWorkflowStatus,
                      ) || "Requisition Issued"}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-2 text-xs">
                    <div className="rounded border border-amber-100 bg-white p-2">
                      ...
                      <div className="font-semibold text-gray-900">
                        {selectedJobOrder.linkedPurchaseFlow?.prNumber ||
                          selectedJobOrder.linkedPrNumber ||
                          "-"}
                      </div>
                      <div className="text-gray-500">
                        {formatStatusLabel(
                          selectedJobOrder.linkedPurchaseFlow?.prStatus ||
                            selectedJobOrder.linkedPrWorkflowStatus ||
                            "",
                        )}
                      </div>
                    ...
                    <div className="rounded border border-amber-100 bg-white p-2">
                      ...
                      <div className="font-semibold text-gray-900">
                        {Number(
                          selectedJobOrder.linkedPurchaseFlow?.rfqSentCount ||
                            0,
                        )}{" "}
                        sent /{" "}
                        {Number(
                    ...
                    <div className="rounded border border-amber-100 bg-white p-2">
                      ...
                      <div className="font-semibold text-gray-900">
                        {Number(
                          selectedJobOrder.linkedPurchaseFlow?.poCount || 0,
                        )}{" "}
                        created
                      </div>
                      <div
                        ...
                        title={(
                          selectedJobOrder.linkedPurchaseFlow?.poNumbers || []
                        ).join(", ")}
                      >
                        {(selectedJobOrder.linkedPurchaseFlow?.poNumbers || [])
                          .slice(0, 3)
                    ...
                    <div className="rounded border border-amber-100 bg-white p-2">
                      ...
                      <div className="font-semibold text-gray-900">
                        {Number(
                          selectedJobOrder.linkedPurchaseFlow?.grnCount || 0,
                        )}{" "}
                        received
                      </div>
                      <div
                        className="truncate text-gray-600"
                        title={(
                          selectedJobOrder.linkedPurchaseFlow?.grnNumbers || []
                        ).join(", ")}
                      >
                        {(selectedJobOrder.linkedPurchaseFlow?.grnNumbers || [])
                          .slice(0, 3)
                    ...
                    <div className="rounded border border-amber-100 bg-white p-2">
                      ...
                      <div className="font-semibold text-gray-900">
                        {Number(
                          selectedJobOrder.linkedPurchaseFlow?.receivedQty || 0,
                        )}{" "}
                        /{" "}
                        {Number(
                          selectedJobOrder.linkedPurchaseFlow?.orderedQty || 0,
                        )}
                      </div>
                        ...
                        <div className="flex items-center justify-between gap-2">
                          <div>
                            ...
                            <div className="text-slate-500">
                              Store issues raw material to production
                            </div>
                          </div>
                          <span
                      ...
                      <div className="rounded-lg border border-cyan-100 bg-cyan-50/70 p-3">
                        <div className="flex items-center justify-between gap-2">
                          <div>
                            ...
                            <div className="text-slate-500">
                              Stores receives production output for QC/stock
                            </div>
                          </div>
                          <span
                            className={`px-2 py-1 rounded ${getStatusColor(status)}`}
              ...
              <div className="col-span-2 rounded-lg border border-[#E8DCC4] bg-[#FFFDF7] p-3">
                <div className="mb-2 text-xs font-bold uppercase tracking-wide text-[#7A542F]">
                  Document flow shortcuts
                </div>
                <div className="flex flex-wrap gap-2">
                  {selectedJobOrder.linkedPrNumber
                    ? (() => {
                        const isMissingLinkedPr =
                          normalizeStatusKey(
                            selectedJobOrder.linkedPurchaseFlow?.status ||
                              selectedJobOrder.linkedPrWorkflowStatus,
                          ) === "PR_NOT_FOUND";
                        return isMissingLinkedPr ? (
                          <span className="rounded border border-red-200 bg-red-50 px-3 py-1.5 text-xs font-semibold text-red-800">
                            Linked PR {selectedJobOrder.linkedPrNumber} missing
                            from register
                          </span>
                        ) : (
                          <button
                            ...
                            onClick={() =>
                              router.push(
                                `/dashboard/purchase/requisitions?search=${encodeURIComponent(selectedJobOrder.linkedPrNumber || "")}`,
                              )
                            }
                  ...
                  {(selectedJobOrder.linkedPurchaseFlow?.poNumbers || [])
                    ...
                    <button
                      ...
                      onClick={() =>
                        router.push(
                          `/dashboard/purchase/orders?search=${encodeURIComponent(selectedJobOrder.linkedPurchaseFlow?.poNumbers?.[0] || "")}`,
                        )
                      }
                  ...
                  {(selectedJobOrder.linkedPurchaseFlow?.grnNumbers || [])
                    ...
                    <button
                      ...
                      onClick={() =>
                        router.push(
                          `/dashboard/purchase/grn?search=${encodeURIComponent(selectedJobOrder.linkedPurchaseFlow?.grnNumbers?.[0] || "")}`,
                        )
                      }
                  ...
                  <button
                    ...
                    onClick={() =>
                      router.push(
                        `/dashboard/inventory/siv?jobId=${encodeURIComponent(selectedJobOrder.id)}&joNumber=${encodeURIComponent(selectedJobOrder.jobOrderNumber)}`,
                      )
                    }
                  ...
                  <button
                    ...
                    onClick={() =>
                      router.push(
                        `/dashboard/inventory/srv?jobId=${encodeURIComponent(selectedJobOrder.id)}&joNumber=${encodeURIComponent(selectedJobOrder.jobOrderNumber)}`,
                      )
                    }
                  ...
                  <button
                    ...
                    onClick={() =>
                      router.push(
                        `/dashboard/quality?jobId=${encodeURIComponent(selectedJobOrder.id)}&joNumber=${encodeURIComponent(selectedJobOrder.jobOrderNumber)}`,
                      )
                    }
            ...
            <div className="mb-6">
                  ...
                  <tbody>
                    {(() => {
                      ...
                      const completedQty = Number(
                        selectedJobOrder.completedQuantity || 0,
                      );
                      const hasProductionCompleted = [
                        "STORE_ISSUED",
                        "COMPLETED",
                        "QC_COMPLETED",
                      ].includes(baseKey);
                      ...
                      const srvApprovedQty = Number(srv.approvedQuantity || 0);
                      const srvPendingQty = Number(srv.pendingQuantity || 0);
                      const mrStatus = materialIssueStage(selectedJobOrder);
                      const hasLinkedPurchaseRequisition = Boolean(
                        String(selectedJobOrder.linkedPrNumber || "").trim(),
                      );
                      const isLinkedPrMissing =
                        normalizeStatusKey(
                          selectedJobOrder.linkedPurchaseFlow?.status ||
                            selectedJobOrder.linkedPrWorkflowStatus,
                        ) === "PR_NOT_FOUND";
                      ...
                      const isQcCompleted =
                        isCompletedJobOrder(selectedJobOrder);
                      const productionStatus =
                        baseKey === "STOPPED"
                          ? "Stopped"
                          : isQcCompleted
                            ? "QC Completed"
                            : ["STORE_ISSUED", "SENT_TO_STORE"].includes(
                                  baseKey,
                                )
                      ...
                      const rows: Array<{
                      ...
                      }> = [
                        {
                          action: "Job Created",
                          status: "Completed",
                          detail: selectedJobOrder.jobOrderNumber,
                        },
                        ...(hasLinkedPurchaseRequisition
                          ...
                          : []),
                        {
                          action: "SIV / Material Issue",
                          status: mrStatus,
                          detail: `Issued ${sivQty}; approved ${sivApprovedQty}; pending ${sivPendingQty}`,
                        },
                        {
                          ...
                          detail:
                            plannedQty > 0
                              ? `Produced ${completedQty || (hasProductionCompleted ? plannedQty : 0)} of ${plannedQty}`
                              : "-",
                        },
                        {
                          action: "SRV / Finished Goods Receipt",
                          status: srvStatus,
                          detail: `Received ${srvQty}; released ${srvApprovedQty}; QC hold ${srvPendingQty}`,
                        },
                        {
                          action: "QC Release",
                          status: qcStatus,
              ...
              <div className="mt-2 text-xs text-gray-600">
                {qcSummary?.totalUidsCount != null ? (
                  <span>
                    QC: total {qcSummary.totalUidsCount} | passed{" "}
                    {qcSummary.passedUidsCount ?? 0} | on-hold{" "}
                    {qcSummary.rejectedUidsCount ?? 0} | pending{" "}
                    {qcSummary.pendingUidsCount ?? 0}
                  </span>
                ) : (
                  <span>
                    QC summary loads automatically when viewing a job order.
                  </span>
                )}
              </div>
            </div>
            ...
            {/* Operations */}
            {selectedJobOrder.operations &&
              selectedJobOrder.operations.length > 0 && (
                <div className="mb-6">
                    ...
                    <tbody>
                      {selectedJobOrder.operations.map((op) => (
                        <tr key={op.id}>
                          ...
                          <td className="border px-3 py-2 text-sm">
                            {op.assignedUserName || "Unassigned"}
                          </td>
                          <td className="border px-3 py-2 text-sm">
                            {op.expectedDurationHours}h
                          </td>
                          <td className="border px-3 py-2 text-sm">
                            <span
                              className={`px-2 py-1 text-xs rounded ${getStatusColor(op.status || "NOT_STARTED")}`}
                            >
                          ...
                          <td className="border px-3 py-2 text-sm">
                            {selectedJobOrder.status === "IN_PROGRESS" ||
                            selectedJobOrder.status === "STORE_ISSUED" ? (
                              <button
                                type="button"
                                onClick={() =>
                                  router.push(
                                    `/dashboard/shop-floor?jobId=${encodeURIComponent(selectedJobOrder.id)}&joNumber=${encodeURIComponent(selectedJobOrder.jobOrderNumber)}`,
                                  )
                                }
                              ...
                              <span className="text-xs text-gray-500">
                                Start the Job Order after SIV
                              </span>
                            )}
                          </td>
                          <td className="border px-3 py-2 text-sm min-w-[280px]">
                            <div className="space-y-2">
                              {(op.dispatches || [])
                                ...
                                .map((dispatch) => (
                                  ...
                                  >
                                    ...
                                    <div>
                                      {dispatch.workStationName}: {dispatch.assignedQuantity} units
                                    </div>
                                    <div className="text-gray-600">
                                      {dispatch.assignedOperatorName || "Operator assigned on floor"} · {dispatch.status.replaceAll("_", " ")}
                                    </div>
                                    <div className="mt-1 flex gap-2">
                                      <button
                                        type="button"
                                        onClick={() => void printJobCards(dispatch)}
                                        className="font-semibold text-blue-700"
                                      >
                                        Print card
                                      </button>
                                      {canEdit && dispatch.status === "DISPATCHED" ? (
                                        <button
                                          type="button"
                                          onClick={() => void cancelOperationDispatch(dispatch)}
                              ...
                              {canEdit ? (
                                <button
                                  type="button"
                                  onClick={() => openDispatchOperation(op)}
                                  className="rounded border border-[#8B6844] px-2 py-1 font-semibold text-[#5E3B27]"
                                >
                                  + Allocate machine
                                </button>
                              ) : null}
            ...
            {/* Materials */}
            {selectedJobOrder.materials &&
              selectedJobOrder.materials.length > 0 && (
                <div className="mb-6">
      ...
      {dispatchOperation && selectedJobOrder && (
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h2 className="text-xl font-bold text-gray-950">
                  {dispatchOperation.sequenceNumber}. {dispatchOperation.operationName}
                </h2>
                <p className="mt-1 text-sm text-gray-600">
                  {selectedJobOrder.jobOrderNumber} · Split the operation across eligible machines.
                </p>
              </div>
              <button type="button" onClick={() => setDispatchOperation(null)} className="text-2xl text-gray-500">×</button>
            </div>
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              <label className="text-sm font-semibold text-gray-700">
                Machine *
                <select
                  value={dispatchForm.workStationId}
                  onChange={(event) => setDispatchForm({ ...dispatchForm, workStationId: event.target.value })}
                  className="mt-1 w-full rounded-lg border px-3 py-2"
                >
                  <option value="">Select eligible machine</option>
                  {(dispatchOperation.eligibleWorkstations || []).map((station) => (
                    <option key={station.id} value={station.id}>
                      {station.name}{station.primary ? " (Primary)" : ""}
                    </option>
                  ))}
                </select>
              </label>
              <label className="text-sm font-semibold text-gray-700">
                Quantity for this machine *
                <input
                  type="number"
                  min="0.0001"
                  step="0.0001"
                  value={dispatchForm.assignedQuantity}
                  onChange={(event) => setDispatchForm({ ...dispatchForm, assignedQuantity: event.target.value })}
                  className="mt-1 w-full rounded-lg border px-3 py-2"
                />
              </label>
              <label className="text-sm font-semibold text-gray-700">
                ...
                <input
                  type="datetime-local"
                  value={dispatchForm.plannedStart}
                  onChange={(event) => setDispatchForm({ ...dispatchForm, plannedStart: event.target.value })}
                  className="mt-1 w-full rounded-lg border px-3 py-2"
                />
              </label>
              <label className="text-sm font-semibold text-gray-700">
                ...
                <input
                  type="datetime-local"
                  value={dispatchForm.plannedEnd}
                  onChange={(event) => setDispatchForm({ ...dispatchForm, plannedEnd: event.target.value })}
                  className="mt-1 w-full rounded-lg border px-3 py-2"
                />
              </label>
              <label className="text-sm font-semibold text-gray-700 sm:col-span-2">
                Operator (optional)
                <select
                  value={dispatchForm.assignedOperatorId}
                  onChange={(event) => setDispatchForm({ ...dispatchForm, assignedOperatorId: event.target.value })}
                  className="mt-1 w-full rounded-lg border px-3 py-2"
                >
                  <option value="">Assign operator on shop floor</option>
                  {users.map((user) => (
              ...
              <label className="text-sm font-semibold text-gray-700 sm:col-span-2">
                ...
                <textarea
                  ...
                  onChange={(event) => setDispatchForm({ ...dispatchForm, notes: event.target.value })}
                  className="mt-1 min-h-20 w-full rounded-lg border px-3 py-2"
                />
              </label>
            </div>
            <div className="mt-5 flex justify-end gap-3">
              <button type="button" onClick={() => setDispatchOperation(null)} className="rounded-lg border px-4 py-2">Cancel</button>
              <button
                type="button"
                disabled={dispatchBusy || !dispatchForm.workStationId || !(Number(dispatchForm.assignedQuantity) > 0)}
                onClick={() => void saveOperationDispatch()}
                className="rounded-lg bg-emerald-700 px-5 py-2 font-semibold text-white disabled:opacity-50"
              >
                {dispatchBusy ? "Dispatching..." : "Create Job Card"}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b">
              <h2 className="text-2xl font-bold text-gray-900">
                Complete Job Order - SRV Handover Preview
              </h2>
              <p className="text-gray-600 mt-1">
                Job Order: {completionPreview.jobOrderNumber}
              </p>
            </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <h3 className="text-lg font-semibold text-green-900 mb-3">
                  Finished Product to Add
                </h3>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-sm text-gray-600">Product</p>
                    <p className="text-base font-semibold text-gray-900">
                      {completionPreview.finishedProduct.itemCode} -{" "}
                      {completionPreview.finishedProduct.itemName}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">
                      Quantity to Receive in SRV
                    </p>
                    <p className="text-2xl font-bold text-green-600">
                      +{completionPreview.finishedProduct.quantityToAdd}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">Current Stock</p>
                    <p className="text-lg font-medium text-gray-700">
                      {completionPreview.finishedProduct.currentStock}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">
                      Stock After QC Release
                    </p>
                    <p className="text-lg font-bold text-green-700">
                      {completionPreview.finishedProduct.newStock}
                    </p>
                  </div>
                </div>
              </div>

              {/* Materials to Backflush Section */}
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-3">
                  Issued WIP to Backflush
                </h3>
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200 border">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {completionPreview.materialsToConsume.map(
                        (material: any, index: number) => (
                          <tr
                            key={index}
                            className={material.sufficient ? "" : "bg-red-50"}
                          >
                              ...
                              {material.toConsume > 0 ? (
                                <div className="space-y-1">
                                  <div className="text-gray-900">
                                    {material.itemCode} will be posted from
                                    issued WIP to actual consumption.
                                  </div>
                                  {!material.sufficient ? (
                                    <div className="text-xs font-medium text-red-700">
                                      SIV pending: {material.pendingIssue} must
                                      still be issued.
                                    </div>
                                  ) : null}
                                </div>
              ...
              {!completionPreview.canComplete && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                  <h3 className="text-lg font-semibold text-red-900 mb-2">
                    Cannot Complete Job Order
                  </h3>
                  <p className="text-sm text-red-700 mb-3">
                    The following materials have not been fully issued through
                    SIV:
                  </p>
                  <ul className="list-disc list-inside text-sm text-red-700 space-y-1">
                    {completionPreview.insufficientMaterials.map(
                      (mat: any, idx: number) => (
                        <li key={idx}>
                          {mat.itemCode} - {mat.itemName}: Required{" "}
                          {mat.requiredQty}, Issued {mat.alreadyIssued}
                        </li>
                      ),
                    )}
                  </ul>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <p className="text-sm text-blue-800">
                  <strong>Note:</strong> Completing this job order posts
                  immutable production-consumption evidence against material
                  already issued through SIV, then hands over the finished
                  quantity for SRV receipt. Finished goods stay under SRV / QC
                  control first; they become available stock only after QC
                  release. This action cannot be undone.
                </p>
              </div>
            </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    Cannot create job order due to material shortages
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="p-6">
              <div className="mb-4">
                <p className="text-sm text-gray-700 mb-4">
                  The following materials are not available in sufficient
                  quantities:
                </p>
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {stockErrorModal.shortages.map(
                        (shortage: any, idx: number) => (
                ...
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-800 mt-1">
                      Please add inventory for these materials before creating
                      the job order. You can do this from the Inventory module.
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <div className="mb-5">
              ...
              <p className="text-sm text-gray-500">
                {jobOrderToPartialComplete.jobOrderNumber}
              </p>
            </div>
            ...
            <div className="space-y-4">
              <div className="rounded-lg border border-[#E8DCC4] bg-[#FAF9F6] p-3 text-sm">
                <div className="font-semibold text-[#4A3426]">
                  {jobOrderToPartialComplete.itemCode}
                </div>
                <div className="text-gray-600">
                  {jobOrderToPartialComplete.itemName}
                </div>
                <div className="mt-3 grid grid-cols-3 gap-2 text-center">
                  <div className="rounded bg-white p-2">
                    ...
                    <div className="font-bold">
                      {formatQuantityValue(jobOrderToPartialComplete.quantity)}
                    </div>
                  </div>
                  <div className="rounded bg-white p-2">
                    <div className="text-[11px] uppercase text-gray-500">
                    ...
                    <div className="font-bold">
                      {formatQuantityValue(
                        jobOrderToPartialComplete.completedQuantity || 0,
                      )}
                    </div>
                  </div>
                  <div className="rounded bg-white p-2">
                    ...
                    <div className="font-bold">
                      {formatQuantityValue(
                        Math.max(
                          0,
                          Number(jobOrderToPartialComplete.quantity || 0) -
                            Number(
                              jobOrderToPartialComplete.completedQuantity || 0,
                            ),
                        ),
              ...
              <label className="block">
                <span className="mb-1 block text-sm font-medium text-gray-700">
                  Produced quantity to receive through SRV
                </span>
                <input
                  type="number"
                  min="0"
              ...
              <div className="rounded border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-900">
                This records only the produced quantity. Stores will receive it
                in SRV and QC will release it to stock.
              </div>
            </div>
      ...
      {showStopModal && jobOrderToStop && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <div className="mb-4">
              <p className="text-sm text-gray-700 mb-4">
                Are you sure you want to stop this job order? This action cannot
                be undone.
              </p>
              <div className="bg-gray-50 p-3 rounded mb-4">
                <p className="text-sm">
                  <strong>Item:</strong> {jobOrderToStop.itemCode}
                </p>
                <p className="text-sm">
                  <strong>Quantity:</strong> {jobOrderToStop.quantity}
                </p>
                <p className="text-sm">
                  <strong>Already Produced:</strong>{" "}
                  {formatQuantityValue(jobOrderToStop.completedQuantity || 0)}
                </p>
                <p className="text-sm">
                  <strong>Status:</strong> {jobOrderToStop.status}
                </p>
              </div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
              ...
              <div className="mb-3 rounded border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900">
                If any quantity was produced before stopping, enter it here. It
                will appear in SRV for stores receipt and QC; blank/0 means
