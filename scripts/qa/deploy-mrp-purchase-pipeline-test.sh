...
stage=/tmp/mizantra-mrp-purchase-pipeline-20260829
backup=/root/sak-deploy-backups/mizantra-mrp-purchase-pipeline-$(date +%Y%m%d-%H%M%S)
...
if [ "$verified" != true ]; then
  ...
  pm2 restart sak-api-test sak-web-test
  echo 'MRP purchase-pipeline deployment failed health checks; previous test runtime restored.' >&2
  exit 1
fi

echo "Mizantra TEST MRP purchase pipeline deployed and verified. Backup: $backup"
