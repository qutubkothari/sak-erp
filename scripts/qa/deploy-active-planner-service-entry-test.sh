...
stage=/tmp/mizantra-active-planner-service-entry-20260830
backup=/root/sak-deploy-backups/mizantra-active-planner-service-entry-$(date +%Y%m%d-%H%M%S)
...
paths=(
  ...
  scripts/qa/deploy-active-planner-service-entry-test.sh
)
...
if [ "$verified" != true ]; then
  ...
  pm2 restart sak-api-test
  echo 'Service-entry prompt deployment failed; previous Mizantra test API restored.' >&2
  exit 1
fi

pm2 save
echo "Mizantra TEST service-entry prompt deployed and verified. Backup: $backup"
