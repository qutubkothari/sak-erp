...
export class GrnService {
  ...
  /**
   * Expose the commercial basis used by GRN/AP calculations to read-only
   * consumers such as the QC screen. Existing stored GRN amounts are
   * authoritative so historical rounding is preserved.
   */
  private async attachGrnLinePricing(grn: any): Promise<any> {
    const lines = Array.isArray(grn?.grn_items) ? grn.grn_items : [];
    const pricing = await this.getPoItemPricingMap(
      lines.map((line: any) => String(line?.po_item_id || "").trim()),
    );

    return {
      ...grn,
      grn_items: lines.map((line: any) => {
        const poPrice = pricing.get(String(line?.po_item_id || "").trim());
        const grossUnitPrice = Math.max(
          0,
          poPrice?.rate ?? this.toNumber(line?.rate ?? line?.unit_price),
        );
        const discountPercent = Math.max(0, poPrice?.discountPercent ?? 0);
        const receivedQty = Math.max(
          0,
          this.toNumber(line?.received_qty ?? line?.received_quantity),
        );
        const calculatedNetAmount = this.roundMoney(
          this.calculateDiscountedLineAmount(
            receivedQty,
            grossUnitPrice,
            discountPercent,
        ...
        const netAmount = hasStoredAmount
          ? this.roundMoney(Math.max(0, this.toNumber(line.amount)))
          : calculatedNetAmount;
        const netUnitPrice =
          receivedQty > 0
            ? this.roundMoney(netAmount / receivedQty)
            : this.roundMoney(
                grossUnitPrice * Math.max(0, 1 - discountPercent / 100),
              );

        return {
          ...line,
          gross_unit_price: this.roundMoney(grossUnitPrice),
    ...
    if (data.items && data.items.length > 0) {
      ...
      const items = data.items
        ...
        .map((item: any) => {
          ...
          // GRN receiving quantity can differ from the PO, but the commercial
          // rate is owned by the approved PO. Never trust a browser-submitted
          // rate here: it would allow a receiver to change inventory/AP value.
          const rate = this.toNumber(poPricing?.rate);
  ...
  async findAll(tenantId: string, filters?: any) {
    ...
    const compact = String(filters?.compact || "").toLowerCase() === "true";
  ...
  async findOne(tenantId: string, id: string) {
    ...
    const pricedData = await this.attachGrnLinePricing(data);
    pricedData.sap_controls = await this.getSapControlsForGrn(tenantId, id);
    return this.attachGrnPaymentCalculation(tenantId, pricedData);
  }

  async update(tenantId: string, id: string, data: any, userId?: string) {
    if (Array.isArray(data?.items) && data.items.length === 0) {
      throw new BadRequestException(
        "A GRN cannot be saved without at least one material line.",
      );
    }
    ...
    if (!Array.isArray(data?.items) && (existingItems ?? []).length === 0) {
      throw new BadRequestException(
        "This GRN has no material lines. Add valid PO items or cancel the orphaned GRN.",
      );
    }

    if (Array.isArray(data?.items)) {
      const invalidLine = data.items.find(
        (item: any) =>
          !this.isUuid(item?.poItemId) ||
          !this.isUuid(item?.itemId) ||
          this.toNumber(item?.receivedQuantity ?? item?.receivedQty) <= 0,
      );
      if (invalidLine) {
        throw new BadRequestException(
  ...
  async submit(tenantId: string, id: string, userId: string) {
    ...
    if (materialLines.length === 0) {
      throw new BadRequestException(
        "Cannot complete GRN: at least one material line is required.",
      );
    }
  ...
  async updateStatus(
    ...
    if (
      ...
      throw new BadRequestException(
        "Cannot complete or approve GRN: at least one material line is required.",
      );
    }
  ...
  async qcAccept(tenantId: string, grnId: string, userId: string, body: any) {
    ...
    try {
      ...
      if (submittedItems.length === 0) {
        throw new BadRequestException(
          "QC cannot be completed because this GRN has no material lines.",
        );
      }
      ...
      const { data: grnHeader, error: grnHeaderError } = await this.supabase
        ...
        .eq("id", grnId)
        .maybeSingle();
      if (grnHeaderError || !grnHeader) {
        throw new NotFoundException("GRN not found");
      }
      if (
        String((grnHeader as any).status || "").toUpperCase() !== "DRAFT" ||
        (grnHeader as any).qc_completed === true
      ) {
        throw new BadRequestException(
          "QC can be completed only once while the GRN is in DRAFT status.",
        );
      }
      ...
      const { data: storedItems, error: storedItemsError } = await this.supabase
        ...
        .eq("grn_id", grnId);
      if (storedItemsError) {
        throw new BadRequestException(storedItemsError.message);
      }
      ...
      const submittedIds = submittedItems.map((item: any) =>
        String(item?.itemId || "").trim(),
      );
      if (
        submittedIds.some((itemId: string) => !this.isUuid(itemId)) ||
        new Set(submittedIds).size !== submittedIds.length ||
        submittedIds.length !== storedItems.length
      ) {
        throw new BadRequestException(
          "QC must include every GRN material line exactly once.",
        );
      }

      const submittedById = new Map(
        submittedItems.map((item: any) => [String(item.itemId).trim(), item]),
      );
      for (const storedItem of storedItems) {
        const item = submittedById.get(String((storedItem as any).id));
        if (!item) {
          throw new BadRequestException(
            "QC must include every GRN material line exactly once.",
          );
        ...
        const receivedQty = this.toNumber((storedItem as any).received_qty);
        if (
          !Number.isFinite(acceptedQty) ||
          !Number.isFinite(rejectedQty) ||
          acceptedQty < 0 ||
          rejectedQty < 0 ||
          Math.abs(acceptedQty + rejectedQty - receivedQty) > 0.000001
        ) {
          throw new BadRequestException(
            "For every GRN line, accepted plus rejected quantity must equal the received quantity.",
          );
        }
      ...
      if (allItemsError) {
        throw new BadRequestException(allItemsError.message);
      }
  ...
  async approveInvoice(
    ...
    const { data: currentGrn, error: fetchError } = await this.supabase
      .from("grns")
      .select(
        "id, grn_number, status, qc_completed, invoice_approved, received_by, net_payable_amount, grn_items(id)",
      )
      .eq("id", grnId)
      .eq("tenant_id", tenantId)
      .single();
    ...
    if (
      !Array.isArray((currentGrn as any).grn_items) ||
      (currentGrn as any).grn_items.length === 0
    ) {
      throw new BadRequestException(
        "Supplier invoice cannot be sanctioned because the GRN has no material lines.",
      );
    }
