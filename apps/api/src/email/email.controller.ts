...
export class EmailController {
  ...
  async getStatus() {
    ...
    const gmailOAuthConfigured = this.gmailOAuth2Service.isConfigured();
  ...
  async testConnection(@Request() req: any) {
    const recipient = (await this.emailConfigService.getEmailAsync('noreply')).trim();
    if (!recipient) {
      throw new BadRequestException('No authenticated email account is configured');
    }

    const result = await this.emailService.sendEmail({
      to: recipient,
      subject: 'SAK ERP email configuration test',
      html: `<p>Email delivery is configured and working.</p><p>Verified at ${new Date().toISOString()}.</p>`,
      from: 'noreply',
    ...
    return {
      success: true,
      recipient: domain ? `${maskedLocal}@${domain}` : maskedLocal,
      transport: this.gmailOAuth2Service.isConfigured() ? 'Gmail OAuth2' : 'SMTP',
