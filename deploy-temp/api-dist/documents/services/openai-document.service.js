...
Object.defineProperty(exports, "OpenAIDocumentService", {
    enumerable: true,
    get: function() {
        return OpenAIDocumentService;
    }
});
...
let OpenAIDocumentService = class OpenAIDocumentService {
    /**
   ...
   */ async extractTextFromPdfBuffer(pdfBuffer) {
        try {
            const parsed = await (0, _pdfparse.default)(pdfBuffer);
...
For business documents (quotation/invoice/PO), extract:
{
   ...
   */ async analyzeDocumentFromText(text, fileName) {
        if (!this.enabled) {
            throw new Error('OpenAI not configured');
        }
...
${clippedText}`
   ...
   */ async qualityCheckFromText(text, fileName) {
        if (!this.enabled) {
            throw new Error('OpenAI not configured');
        }
...
Return JSON only in this shape:
{
   ...
   */ async searchBySimilarity(query, documents) {
   ...
   */ estimateCost(pageCount) {
    ...
    constructor(configService){
        ...
        this.logger = new _common.Logger(OpenAIDocumentService.name);
...
OpenAIDocumentService = _ts_decorate([
...
//# sourceMappingURL=openai-document.service.js.map