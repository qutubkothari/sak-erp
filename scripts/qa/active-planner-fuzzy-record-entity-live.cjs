...
async function main() {
  ...
  const prompt = process.env.QA_PROMPT || "Which is the latest purchase order already recorded for supplier Macfso?";
  ...
  try {
    ...
    const warnings = data?.analytics?.warnings || [];
    const pass =
      response.ok &&
      data?.status === "READY_WITH_ANALYTICS" &&
      data?.analytics?.kind === "SEMANTIC_QUERY" &&
      rows.length === 1 &&
      /macfos/i.test(String(rows[0]?.name || "")) &&
      warnings.some((warning) => /closest governed master value/i.test(warning));
    console.log(JSON.stringify({
      ...
      questions: data?.questions || data?.analytics?.questions || [],
      semantic_plan: data?.analytics?.semantic_plan || null,
