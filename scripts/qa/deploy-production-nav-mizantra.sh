#!/usr/bin/env bash
set -Eeuo pipefail
app=/var/www/sak-erp-test; stage=/tmp/mizantra-production-nav-20260906-v1; release=production-nav-20260906; page=apps/web/src/components/Sidebar.tsx; backup="$app/backups/$release-$(date +%Y%m%d-%H%M%S)"
...
for _ in $(seq 1 40); do status=$(curl -s -o /dev/null -w '%{http_code}' http://127.0.0.1:3001/dashboard/production || true); [[ "$status" == 200 ]] && break; sleep 2; done; [[ "$status" == 200 ]]; pm2 describe sak-web-test | grep -q online; trap - ERR; rm -rf -- "$stage"; printf '{"result":"deployed","backup":"%s","http":%s}\n' "$backup" "$status"
