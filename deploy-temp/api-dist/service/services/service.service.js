...
let ServiceService = class ServiceService {
    async uploadServiceAttachments(tenantId, userId, files) {
        if (!files || files.length === 0) {
            throw new _common.BadRequestException('No files uploaded');
        }
        ...
        const uploadsRoot = process.env.UPLOAD_ROOT_DIR || (0, _path.resolve)(process.cwd(), '..', '..', 'uploads');
        const today = new Date().toISOString().split('T')[0];
        const uploads = [];
        for (const file of files){
            const isAllowedType = file.mimetype?.startsWith('image/') || file.mimetype?.startsWith('video/');
            if (!isAllowedType) {
                throw new _common.BadRequestException(`Unsupported file type: ${file.mimetype || 'unknown'}`);
            }
            if (file.size > maxSizeBytes) {
                throw new _common.BadRequestException(`File too large: ${file.originalname} exceeds 50MB`);
            }
            const extensionFromName = (0, _path.extname)(file.originalname || '').toLowerCase();
            const safeExtension = extensionFromName && extensionFromName.length <= 10 ? extensionFromName : '';
            ...
            await _fs.promises.writeFile(targetPath, file.buffer);
    ...
    async createServiceTicket(tenantId, userId, data) {
        ...
        const { data: ticket, error } = await this.supabase.from('service_tickets').insert(ticketData).select().single();
        if (error) throw new _common.BadRequestException(error.message);
        return ticket;
    }
    async getServiceTickets(tenantId, filters) {
        let query = this.supabase.from('service_tickets').select('*').eq('tenant_id', tenantId).order('created_at', {
            ascending: false
        });
        if (filters?.status) {
        ...
        if (tickets && tickets.length > 0) {
            const customerIds = [
                ...new Set(tickets.map((t)=>t.customer_id).filter(Boolean))
            ];
            const warrantyIds = [
                ...new Set(tickets.map((t)=>t.warranty_id).filter(Boolean))
            ];
            // Fetch customers
            let customersMap = {};
            if (customerIds.length > 0) {
                const { data: customers } = await this.supabase.from('customers').select('id, customer_name, customer_code').in('id', customerIds);
                if (customers) {
                    customersMap = Object.fromEntries(customers.map((c)=>[
                            c.id,
                            c
                        ]));
                }
            ...
            if (warrantyIds.length > 0) {
                const { data: warranties } = await this.supabase.from('warranties').select('id, warranty_number, warranty_end_date').in('id', warrantyIds);
                if (warranties) {
                    warrantiesMap = Object.fromEntries(warranties.map((w)=>[
                            w.id,
                            w
                        ]));
                }
    ...
    async getServiceTicketById(tenantId, ticketId) {
        const { data, error } = await this.supabase.from('service_tickets').select(`
        *,
        customer:customers(*),
        warranty:warranties(*),
      ...
      `).eq('tenant_id', tenantId).eq('id', ticketId).single();
        if (error) throw new _common.NotFoundException('Service ticket not found');
        return data;
    }
    async updateServiceTicket(tenantId, ticketId, data) {
        const { data: ticket, error } = await this.supabase.from('service_tickets').update({
            ...data,
            updated_at: new Date().toISOString()
        }).eq('tenant_id', tenantId).eq('id', ticketId).select().single();
        if (error) throw new _common.BadRequestException(error.message);
        return ticket;
    }
    async closeServiceTicket(tenantId, ticketId, userId, data) {
    ...
    // ==================== Warranty Validation ====================
    async validateWarrantyForUID(tenantId, uid) {
        const { data: warranty, error } = await this.supabase.from('warranties').select('*').eq('tenant_id', tenantId).eq('uid', uid).eq('status', 'ACTIVE').single();
        if (error || !warranty) {
            // Fallback: deployment tracking (supports part-number based workflows)
            const { data: deploymentStatus } = await this.supabase.from('v_uid_deployment_status').select('warranty_expiry_date').eq('tenant_id', tenantId).eq('uid', uid).single();
            ...
            return {
                ...
                message: isValid ? 'Warranty is valid' : `Warranty expired on ${warrantyExpiryDate}`,
                days_remaining: isValid ? Math.ceil((warrantyEndDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24)) : 0
            };
        }
        const today = new Date();
        ...
        return {
            is_valid: isValid,
            warranty: warranty,
            message: isValid ? 'Warranty is valid' : `Warranty expired on ${warranty.warranty_end_date}`,
            days_remaining: isValid ? Math.ceil((warrantyEndDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24)) : 0
        };
    }
    // ==================== Technicians ====================
    async createTechnician(tenantId, data) {
        ...
        const { data: technician, error } = await this.supabase.from('technicians').insert(technicianData).select().single();
        if (error) throw new _common.BadRequestException(error.message);
        return technician;
    }
    async getTechnicians(tenantId, activeOnly = true) {
        let query = this.supabase.from('technicians').select('*').eq('tenant_id', tenantId).order('technician_name');
        if (activeOnly) {
    ...
    // ==================== Service Assignments ====================
    async assignTechnician(tenantId, userId, data) {
        // Check if ticket exists
        const { data: ticket, error: ticketError } = await this.supabase.from('service_tickets').select('*').eq('tenant_id', tenantId).eq('id', data.service_ticket_id).single();
        if (ticketError) throw new _common.NotFoundException('Service ticket not found');
        // Create assignment
        ...
        const { data: assignment, error } = await this.supabase.from('service_assignments').insert(assignmentData).select().single();
        if (error) throw new _common.BadRequestException(error.message);
    ...
    async getAssignmentsByTechnician(technicianId, status) {
        let query = this.supabase.from('service_assignments').select(`
        *,
        service_ticket:service_tickets(
          *,
          customer:customers(customer_name, contact_person, mobile)
        )
      `).eq('technician_id', technicianId).order('assigned_date', {
            ascending: false
        });
        if (status) {
    ...
    async updateAssignment(assignmentId, data) {
        const { data: assignment, error } = await this.supabase.from('service_assignments').update({
            ...data,
            updated_at: new Date().toISOString()
    ...
    // ==================== Service Parts Used ====================
    async addServicePart(tenantId, data) {
        ...
        const { data: part, error } = await this.supabase.from('service_parts_used').insert({
            ...partData,
            replacement_warranty_end: replacementWarrantyEnd
        }).select().single();
        if (error) throw new _common.BadRequestException(error.message);
    ...
    async getServicePartsByTicket(ticketId) {
        const { data, error } = await this.supabase.from('service_parts_used').select('*').eq('service_ticket_id', ticketId).order('used_date', {
            ascending: false
        });
        if (error) throw new _common.BadRequestException(error.message);
        return data;
    ...
    // ==================== Service History ====================
    async createServiceHistoryEntry(tenantId, ticket) {
        const historyData = {
            tenant_id: tenantId,
            uid: ticket.uid,
        ...
        const { data, error } = await this.supabase.from('service_history').insert(historyData).select().single();
        if (error) throw new _common.BadRequestException(error.message);
        return data;
    }
    async getServiceHistoryByUID(tenantId, uid) {
        const { data, error } = await this.supabase.from('service_history').select(`
        *,
        service_ticket:service_tickets(ticket_number, status),
        customer:customers(customer_name),
        technician:technicians(technician_name)
      `).eq('tenant_id', tenantId).eq('uid', uid).order('service_date', {
            ascending: false
        });
        if (error) throw new _common.BadRequestException(error.message);
        return data;
    ...
    // ==================== Reports ====================
    async getServiceReports(tenantId, filters) {
        // Get open vs closed tickets
        const { data: statusCounts, error: statusError } = await this.supabase.from('service_tickets').select('status').eq('tenant_id', tenantId);
        if (statusError) throw new _common.BadRequestException(statusError.message);
        const openCount = statusCounts.filter((t)=>[
                'OPEN',
                'ASSIGNED',
                'IN_PROGRESS',
                'PARTS_PENDING'
            ].includes(t.status)).length;
        const closedCount = statusCounts.filter((t)=>[
                'COMPLETED',
                'CLOSED'
            ].includes(t.status)).length;
        // Get warranty claim costs
        const { data: warrantyClaims, error: claimsError } = await this.supabase.from('service_tickets').select('actual_cost, parts_cost, labor_cost').eq('tenant_id', tenantId).eq('service_type', 'WARRANTY');
        if (claimsError) throw new _common.BadRequestException(claimsError.message);
        ...
        const { data: tickets, error: ticketsError } = await this.supabase.from('service_tickets').select('product_name, uid').eq('tenant_id', tenantId);
        if (ticketsError) throw new _common.BadRequestException(ticketsError.message);
        const productIssues = {};
        tickets.forEach((t)=>{
            if (t.product_name) {
        ...
        return {
            ...
            product_reliability: Object.entries(productIssues).map(([product, count])=>({
                    product,
                    issue_count: count
                })).sort((a, b)=>b.issue_count - a.issue_count).slice(0, 10)
        };
    }
    // ==================== Helper Methods ====================
    async generateTicketNumber(tenantId) {
        const { count } = await this.supabase.from('service_tickets').select('*', {
            count: 'exact',
            head: true
    ...
    async generateTechnicianCode(tenantId) {
        const { count } = await this.supabase.from('technicians').select('*', {
            count: 'exact',
            head: true
        }).eq('tenant_id', tenantId);
        const nextNumber = (count || 0) + 1;
