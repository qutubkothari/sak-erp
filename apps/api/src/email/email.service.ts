...
export class EmailService {
  ...
  public async applyFromAndReplyTo(
    ...
    // The OAuth mailbox is transport-only. Never expose it as the sender in
    // documents, previews, or delivered messages; use the configured
    // departmental identity (normally a Workspace alias) instead.
  ...
  async sendEmail(options: { 
    ...
    html: string; 
    attachments?: any[];
    from?: 'admin' | 'sales' | 'support' | 'technical' | 'purchase' | 'production' | 'accounts' | 'reminders' | 'quality' | 'documents' | 'hr' | 'noreply';
    tenantId?: string;
  }) {
