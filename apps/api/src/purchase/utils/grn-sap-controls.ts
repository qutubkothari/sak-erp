...
export function buildSapGrnControls(input: SapGrnInput): SapGrnControl {
  ...
  const hasMaterialLines = items.length > 0;
  const itemToleranceStatus = hasMaterialLines
    ? maxStatus(items.map((item) => item.toleranceStatus))
    : 'BLOCKED';
  const messages = items.flatMap((item) =>
    item.toleranceMessages.map((message) => `${item.itemCode || item.itemId || 'Item'}: ${message}`),
  );
  if (!hasMaterialLines) {
    messages.push('GRN has no material lines and cannot proceed to QC, stock posting, or Accounts Payable.');
  }
  ...
  const threeWayMatchStatus: SapToleranceStatus =
    !hasMaterialLines
      ? 'BLOCKED'
      : itemToleranceStatus === 'OK' && qcGateStatus !== 'PENDING_INSPECTION'
        ? 'OK'
        : itemToleranceStatus;
  ...
  return {
    ...
    grIrStatus:
      ...
      qcGateStatus !== 'PENDING_INSPECTION'
        ? 'READY_FOR_AP'
