...
import { buildUidLabelPrintHtml } from "@/lib/uid-label-print";
...
export default function SrvPage() {
  ...
  const [activeSrvView, setActiveSrvView] = useState<"open" | "history">(
    "open",
  );
  const [focusJobId, setFocusJobId] = useState("");
  const [focusJoNumber, setFocusJoNumber] = useState("");
  const [showAllOpenSrvs, setShowAllOpenSrvs] = useState(false);
  ...
  const [manualSrvItems, setManualSrvItems] = useState<
    Array<{ id: string; code: string; name: string }>
  >([]);
  const [manualSrvForm, setManualSrvForm] = useState({
    itemId: "",
    quantity: "",
    warehouseId: "",
    receiverName: "",
  ...
  const [manualSrvAlert, setManualSrvAlert] = useState<{
    type: "error" | "success";
    message: string;
  } | null>(null);
  ...
  const [selectedRow, setSelectedRow] = useState<ReceiptVoucherRow | null>(
    null,
  );
  ...
  const loadAll = useCallback(async (options?: { silent?: boolean }) => {
    ...
    try {
      const [open, hist] = await Promise.all([
        apiClient.get<ReceiptVoucherRow[]>(
          "/job-orders/store/receipt-vouchers/open",
        ),
        apiClient.get<ReceiptVoucherRow[]>(
          "/job-orders/store/receipt-vouchers/history",
        ),
      ]);
      setOpenSrvs(open || []);
      setSrvHistory(hist || []);
  ...
  const handleViewInvoice = useCallback(
    (invoiceFileUrl: string, invoiceFileName?: string) => {
  ...
  const handleQCFileSelect = useCallback(async (file: File, index: number) => {
    const validTypes = [
      "image/png",
      "image/jpeg",
      "image/jpg",
    ...
    try {
      ...
      setQcFormData((prev) => {
        ...
        next[index] = {
          ...
          qcFileName:
            existingFiles.length === 0 ? newFile.name : next[index].qcFileName,
          qcFileType:
            existingFiles.length === 0 ? newFile.type : next[index].qcFileType,
          qcFileSize:
            existingFiles.length === 0 ? newFile.size : next[index].qcFileSize,
        };
        return next;
      });
    } catch (e) {
  ...
  const handleRemoveQCFile = useCallback(
    (itemIndex: number, fileIndex: number) => {
      setQcFormData((prev) => {
        const next = [...prev];
  ...
  const openView = useCallback(
    async (row: ReceiptVoucherRow) => {
      ...
      const receiverNamePrefill =
        receivedByNameRaw && isUuidLike(receivedByNameRaw)
          ? ""
          : receivedByNameRaw;
      setReceiverName(receiverNamePrefill);
      setReceiverPhone(
        String(
          (receiptRow as any)?.received_by_phone ||
            receiptRow.received_by_phone ||
            "",
        ) || "",
      );
  ...
  const receiveSrv = useCallback(
    async (row: ReceiptVoucherRow, qty: number) => {
      ...
      await apiClient.post(
        `/job-orders/store/receipt-vouchers/${jobOrderId}/receive`,
        {
          receiverName,
          receiverPhone,
  ...
  const approveSrv = useCallback(async (row: ReceiptVoucherRow) => {
    ...
    await apiClient.put(
      `/job-orders/store/receipt-vouchers/${entryOrJobOrderId}/approve`,
      {},
    );
  }, []);

  const approveSrvWithFeedback = useCallback(
    async (row: ReceiptVoucherRow) => {
  ...
  const approveAllPending = useCallback(async () => {
    ...
    try {
      ...
      for (const row of pending) {
        try {
          await apiClient.put(
            `/job-orders/store/receipt-vouchers/${String(row.entry_id || row.id || "").trim()}/approve`,
            {},
          );
          succeeded++;
        } catch {
  ...
  const qcAcceptSrv = useCallback(
    async (
      ...
      return apiClient.post<QcApproveResponse>(
        `/job-orders/${jobOrderId}/qc-approve`,
        {
          acceptedQuantity,
          rejectedQuantity,
          ...(extra || {}),
  ...
  const deleteSrv = useCallback(
    async (movementId: string) => {
      const confirmed = await confirmDialog({
        ...
        message:
          "Reverse this SRV goods receipt? Stock and UID movement will be reversed.",
      ...
      try {
        await apiClient.delete(
          `/job-orders/store/receipt-vouchers/${movementId}`,
        );
        await loadAll();
        await confirmDialog({
          title: "SRV Reversed",
          message:
            "SRV goods receipt has been reversed and stock/UID movement has been adjusted.",
          confirmLabel: "OK",
          cancelLabel: "Close",
        });
      } catch (err: any) {
  ...
  const printSrv = useCallback(
    async (row: ReceiptVoucherRow) => {
      const receivedAt = row.movement_date
      ...
      const receivedByLabel = resolveEmployeeLabel(
        row.received_by_name || row.received_by,
      );
      ...
      printWindow.document.write(
        '<!doctype html><html><head><title>Preparing SRV...</title></head><body style="font-family: Arial, sans-serif; padding: 16px;">Preparing SRV...</body></html>',
      );
  ...
  const printGeneratedUids = useCallback(
    async (
      payload: GeneratedUidPrintPayload,
      preOpenedWindow?: Window | null,
    ) => {
      const generatedUids = Array.isArray(payload.generatedUids)
        ? payload.generatedUids
      ...
      printWindow.document.open();
      printWindow.document.write(
        '<!doctype html><html><head><title>Preparing UID print...</title></head><body style="font-family: Arial, sans-serif; padding: 16px;">Preparing UID print...</body></html>',
      );
      printWindow.document.close();
      ...
      const qcDate = escapePrintHtml(
        payload.qcDate
          ? new Date(payload.qcDate).toLocaleDateString()
    ...
    `;
      try {
        const labelHtml = await buildUidLabelPrintHtml(
          generatedUids.map((uid) => ({ uid, itemCode: payload.itemCode })),
          `UID Labels - ${payload.jobOrderNumber || "Production receipt"}`,
        );
        printWindow.document.open();
        ...
        printWindow.close();
        await confirmDialog({
          title: "UID Label Error",
          message: error?.message || "Unable to generate UID labels. The accepted UIDs remain saved.",
          confirmLabel: "OK",
          cancelLabel: "Close",
          variant: "warning",
        });
    ...
    if (manualSrvItems.length === 0) {
      ...
      try {
        ...
        setManualSrvItems(
          Array.isArray(data)
            ? data
                .map((i: any) => ({ id: i.id, code: i.code, name: i.name }))
  ...
  const pendingApprovalCount = srvHistory.filter(
    (row) => !row.approved_by,
  ).length;
  const approvedCount = srvHistory.length - pendingApprovalCount;
  const uidTrackedCount = [...openSrvs, ...srvHistory].filter((row) =>
    String(row.uid || "").trim(),
  ).length;
  const qcPendingCount = openSrvs.length;
  const totalReceivedQty = srvHistory.reduce(
    (sum, row) => sum + (Number(row.quantity) || 0),
    0,
  );
  const focusedOpenSrv = openSrvs.find(
    (row) =>
      (focusJobId && String(row.job_order_id || "").trim() === focusJobId) ||
      (focusJoNumber &&
        String(row.job_order_number || "").trim() === focusJoNumber),
  );
  const focusedReceiptMode = Boolean(
    focusedOpenSrv && (focusJobId || focusJoNumber),
  );
  const displayedOpenSrvs: ReceiptVoucherRow[] =
    focusedReceiptMode && !showAllOpenSrvs && focusedOpenSrv
      ? [focusedOpenSrv]
      : openSrvs;
    ...
    <div className="min-h-screen bg-[#FAF9F6] px-4 py-4 text-[#2F241D] lg:px-6">
      <div className="flex min-h-[calc(100vh-2rem)] flex-col gap-4">
        <ErpPageHeader
          ...
          description="Receive finished output, complete QC, and let Mizantra post accepted stock with full traceability."
          actions={
            <div className="flex flex-wrap gap-2">
              {canCreate && (
                <ErpButton
                  variant="primary"
                  onClick={() => void openManualSrv()}
                >
          ...
          metrics={[
            {
              ...
              tone: qcPendingCount > 0 ? "warning" : "success",
            },
            {
              label: "Posted Receipts",
              value: srvHistory.length,
              tone: "success",
            },
            { label: "Received Qty", value: totalReceivedQty },
            {
              label: "UID Tracked",
              value: uidTrackedCount,
              tone: uidTrackedCount > 0 ? "success" : "neutral",
            },
            {
        ...
        <div className="grid gap-3 lg:grid-cols-4">
          ...
          <div className="rounded-md border border-[#E8DCC4] bg-white p-3">
            ...
            <div className="text-xs text-[#7A6555]">
              Production receipts move through QC before final acceptance
            </div>
          </div>
          <div className="rounded-md border border-[#E8DCC4] bg-white p-3">
            <div className="text-xs font-semibold uppercase text-[#7A6555]">
            ...
            <div className="text-xs text-[#7A6555]">
              Accepted quantity controls stock and UID generation
            </div>
          </div>
          <div className="rounded-md border border-[#E8DCC4] bg-white p-3">
            <div className="text-xs font-semibold uppercase text-[#7A6555]">
            ...
            <div className="text-xs text-[#7A6555]">
              Reversal restores stock and UID movement
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-col gap-3 border-b border-[#E8DCC4] px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
            ...
            {activeSrvView === "history" && canApprove && (
              ...
              >
                ...
                {bulkApproving
                  ? "Approving..."
                  : `Approve Pending (${pendingApprovalCount})`}
              </ErpButton>
            )}
          </div>
        </div>
              ...
              <div className="flex flex-col gap-3 rounded-xl border border-blue-200 bg-blue-50 p-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <div className="mt-1 font-bold text-blue-950">
                    {focusedOpenSrv?.job_order_number} ·{" "}
                    {focusedOpenSrv?.item_code} - {focusedOpenSrv?.item_name}
                  </div>
                  <p className="mt-1 text-sm text-blue-900">
                    Open this receipt, confirm the received quantity, then
                    complete QC. Only accepted output posts to stock.
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => setShowAllOpenSrvs((value) => !value)}
                ...
                >
                  {showAllOpenSrvs
                    ...
                    : `Show all QC receipts (${openSrvs.length})`}
                </button>
              </div>
            ) : null}
            <div className="rounded-md border border-[#E8DCC4] bg-white">
              <div className="border-b border-[#E8DCC4] px-4 py-3">
                <h2 className="text-base font-bold text-[#4A3426]">
                  Receipts Awaiting QC
                </h2>
                <p className="mt-0.5 text-xs text-[#7A6555]">
                  Complete QC acceptance before final stock/UID posting.
                </p>
              </div>
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-[#E8DCC4]">
                  ...
                  <tbody className="divide-y divide-[#E8DCC4] bg-white">
                    {displayedOpenSrvs.length === 0 && (
                      <tr>
                        <td
                          colSpan={9}
                          className="px-6 py-8 text-center text-[#7A6555]"
                        >
                          No pending SRVs.
                        </td>
                      </tr>
                    )}
                    {displayedOpenSrvs.map((row) => (
                      <tr
                        key={getSrvRowIdentity(row)}
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <div className="flex justify-end gap-2">
                            ...
                            <ErpButton onClick={() => openView(row)} size="sm">
                              Receive / QC
                            </ErpButton>
                            {canDelete && (
                              <ErpButton
                                onClick={() =>
                                  deleteSrv(
                                    String(row.entry_id || row.id || "").trim(),
                                  )
                                }
                                variant="danger"
                                size="sm"
        ...
        {activeSrvView === "history" && (
          <div className="min-h-0 flex-1 rounded-md border border-[#E8DCC4] bg-white">
            <div className="border-b border-[#E8DCC4] px-4 py-3">
              ...
              <p className="mt-0.5 text-xs text-[#7A6555]">
                Posted receipts, approvals, UID traceability, and reversal
                controls.
              </p>
            </div>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-[#E8DCC4]">
                ...
                <tbody className="divide-y divide-[#E8DCC4] bg-white">
                  ...
                  {[...srvHistory]
                    .sort((a, b) => {
                      const aTs = a.movement_date
                        ? new Date(a.movement_date).getTime() || 0
                        : 0;
                      const bTs = b.movement_date
                        ? new Date(b.movement_date).getTime() || 0
                        : 0;
                      return bTs - aTs;
                    })
                    .map((row) => (
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg w-[95vw] max-w-7xl max-h-[92vh] overflow-y-auto">
              ...
              {(() => {
                ...
                const statusLabel = isManualSrv
                  ? "AUTO-APPROVED"
                  : receiptRow?.approved_by
                    ? "APPROVED"
                    : "DRAFT";
                const receivedAt =
                  (receiptRow as any)?.received_at ||
                  receiptRow?.movement_date ||
                  null;
                const approvedAt = receiptRow?.approved_at || null;

                const receivedQtyDisplay =
                  Number(receiptRow?.quantity ?? 0) || 0;
                const availableQtyDisplay =
                  Number((receiptRow as any)?.available_quantity ?? 0) || 0;
                    ...
                    <div className="p-6 space-y-6">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-sm font-medium text-gray-700">
                            {isManualSrv ? "Source" : "Job Order"}
                          </label>
                          <p className="mt-1 text-gray-900 font-semibold">
                            {isManualSrv
                              ? "Manual SRV"
                              : selectedRow.job_order_number ||
                                selectedRow.job_order_id ||
                                selectedRow.id}
                          </p>
                        </div>
                        ...
                        <div>
                          ...
                          <p className="mt-1 text-gray-900">
                            {resolveWarehouseLabel(
                              receiptRow?.to_warehouse_id ||
                                selectedRow.to_warehouse_id,
                            )}
                          </p>
                        </div>
                      </div>
                        ...
                        <table className="min-w-full">
                          ...
                          <tbody>
                            <tr className="border-t">
                              ...
                              <td className="px-4 py-2 text-sm text-green-700 text-center">
                                {availableQtyDisplay}
                              </td>
                              <td className="px-4 py-2 text-sm text-red-700 text-center">
                                -
                              </td>
                            </tr>
                          </tbody>
                      ...
                      {isManualSrv ? (
                        <div className="text-sm text-green-800 bg-green-50 border border-green-200 rounded-lg p-3">
                          OK: This is a <strong>Manual SRV</strong> - stock and
                          UIDs were auto-approved at creation. No QC step
                          required.
                        </div>
                      ) : (
                        <>
                          <div className="grid grid-cols-3 gap-4">
                            <div>
                              ...
                              <input
                                ...
                                value={receivedQty}
                                onChange={(e) =>
                                  setReceivedQty(Number(e.target.value || 0))
                                }
                          ...
                          <div className="text-sm text-gray-600 bg-amber-50 border border-amber-200 rounded-lg p-3">
                            UIDs will NOT be generated at SRV receipt. UIDs will
                            be generated only after QC is completed.
                          </div>
                        </>
                      )}
                    </div>
                            ...
                            onClick={async () => {
                              ...
                              if (qcCompleted) {
                                await confirmDialog({
                                  ...
                                  message:
                                    "QC is already completed for this SRV.",
                                  confirmLabel: "OK",
                                  cancelLabel: "Close",
                                  variant: "info",
                                });
                                ...
                                setQcFormData([
                                  {
                                    ...
                                    receivedQty:
                                      Number(receivedQtyDisplay || qty) || qty,
                                    acceptedQty:
                                      Number(receivedQtyDisplay || qty) || qty,
                                    rejectedQty: 0,
                                    qcNotes: "",
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <button
                                onClick={async () => {
                                  ...
                                  try {
                                    const hasRejectedWithoutReason =
                                      qcFormData.some(
                                        (it) =>
                                          it.rejectedQty > 0 &&
                                    ...
                                    const accepted = qcFormData.reduce(
                                      (sum, it) =>
                                        sum + (Number(it.acceptedQty) || 0),
                                      0,
                                    );
                                    const rejected = qcFormData.reduce(
                                      (sum, it) =>
                                        sum + (Number(it.rejectedQty) || 0),
                                      0,
                                    );
                                    const received = qcFormData.reduce(
                                      (sum, it) =>
                                        sum + (Number(it.receivedQty) || 0),
                                      0,
                                    );
                                    ...
                                    if (
                                      ...
                                      await confirmDialog({
                                        ...
                                        message:
                                          "Accepted Qty must be greater than 0.",
                                        confirmLabel: "OK",
                                        cancelLabel: "Close",
                                        variant: "warning",
                                      });
                                    ...
                                    if (
                                      ...
                                      await confirmDialog({
                                        ...
                                        message:
                                          "Rejected Qty must be greater than or equal to 0.",
                                        confirmLabel: "OK",
                                        cancelLabel: "Close",
                                        variant: "warning",
                                      });
                                    ...
                                    if (uidPrintWindow) {
                                      uidPrintWindow.document.write(
                                        '<!doctype html><html><head><title>Preparing UID print...</title></head><body style="font-family:Arial,sans-serif;padding:16px">Preparing UID print...</body></html>',
                                      );
                                      uidPrintWindow.document.close();
                                    }

                                    const qcResult = await qcAcceptSrv(
                                      jobOrderId,
                                      accepted,
                                      rejected,
                                      {
                                    ...
                                    const generatedUids = Array.isArray(
                                      qcResult?.generatedUids,
                                    )
                                      ? qcResult.generatedUids
                                          .map((uid) =>
                                            String(uid || "").trim(),
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg">
            <div className="flex items-center justify-between p-5 border-b">
              ...
              <button
                onClick={() => setShowManualSrvModal(false)}
            ...
            <div className="p-5 space-y-4">
              {manualSrvAlert && (
                <div
                  className={`rounded-lg px-4 py-3 text-sm font-medium ${manualSrvAlert.type === "error" ? "bg-red-50 text-red-800" : "bg-green-50 text-green-800"}`}
                >
            ...
            <div className="flex justify-end gap-3 p-5 border-t">
              ...
              <button
                disabled={
                  manualSrvSaving ||
                  !manualSrvForm.itemId ||
                  !manualSrvForm.quantity
                }
                onClick={async () => {
                  setManualSrvAlert(null);
                  ...
                  try {
                    ...
                    const res = await fetch(
                      "/api/v1/job-orders/store/receipt-vouchers/manual",
                      {
                        method: "POST",
                        headers: {
                          "Content-Type": "application/json",
                        ...
                        body: JSON.stringify({
                          ...
                          receiverPhone:
                            manualSrvForm.receiverPhone || undefined,
                    ...
                    if (!res.ok) {
                      uidPrintWin?.close();
                      throw new Error(
                        data?.message || "Failed to create manual SRV",
                      );
                    }

                    const generatedUids: string[] = Array.isArray(data?.uids)
                      ? data.uids
                          .map((u: any) => String(u || "").trim())
                          .filter(Boolean)
                      : [];
                    const selectedItem = manualSrvItems.find(
                      (i) => i.id === manualSrvForm.itemId,
                    );
                    ...
                    if (generatedUids.length > 0) {
                      ...
                      setManualSrvAlert({
                        type: "success",
                        message: `${data.message || "Manual SRV created!"} ${generatedUids.length} UID label(s) sent to print.`,
                      });
                    } else {
                      uidPrintWin?.close();
                      setManualSrvAlert({
                        type: "success",
                        message:
                          data.message || "Manual SRV created successfully!",
                      });
                    }
                    void loadAll();
                    ...
                    setManualSrvAlert({
                      type: "error",
                      message: err?.message || "Failed to create manual SRV",
                    });
                  } finally {
