...
Object.defineProperty(exports, "GmailWebhookService", {
    enumerable: true,
    get: function() {
...
let GmailWebhookService = class GmailWebhookService {
    ...
    async loadLastHistoryId() {
        try {
            const result = await this.databaseService.executeQuery('SELECT last_message_id FROM email_sync_status WHERE email_account = $1', [
    ...
    async saveHistoryId(historyId) {
        try {
            await this.databaseService.executeQuery(`INSERT INTO email_sync_status (email_account, last_message_id, last_sync_date, sync_status)
         VALUES ($1, $2, NOW(), 'idle')
    ...
    async processNotification(payload) {
        ...
        try {
            ...
            const messageData = JSON.parse(Buffer.from(payload.message.data, 'base64').toString('utf-8'));
    ...
    async fetchRecentMessages() {
        ...
        try {
            const list = await this.gmailOAuth2Service.listMessages(10, [
                'INBOX'
            ]);
    ...
    isSpam(from, subject, listUnsubscribe) {
    ...
    async storeEmail(emailData) {
        try {
            await this.databaseService.executeQuery(`INSERT INTO email_inbox (
          message_id,
          thread_id,
          from_address,
          from_name,
    ...
    parseAddressList(value) {
        if (!value) return [];
        return value.split(',').map((part)=>this.parseAddress(part).email).map((email)=>email.trim()).filter((email)=>email.length > 0);
    }
    constructor(gmailOAuth2Service, databaseService, configService){
        this.gmailOAuth2Service = gmailOAuth2Service;
        this.databaseService = databaseService;
        this.configService = configService;
        this.logger = new _common.Logger(GmailWebhookService.name);
...
GmailWebhookService = _ts_decorate([
...
//# sourceMappingURL=gmail-webhook.service.js.map