...
export class IntelligenceController {
  ...
  briefHistory(@Req() request: any, @Query('period') period?: string) {
    return this.intelligence.briefHistory(request.user.tenantId, period || 'WEEK');
  }

  @Get('root-cause-brief')
  rootCauseBrief(@Req() request: any, @Query('period') period?: string) {
    return this.intelligence.historicalRootCauseBrief(request.user.tenantId, period || 'WEEK');
  }

  @Post('ask')
  ask(@Req() request: any, @Body() body: { question?: string }) {
    return this.intelligence.ask(request.user.tenantId, request.user, body?.question || '', request);
  }

  @Post('reports/query')
  report(@Req() request: any, @Body() body: { question?: string }) {
    return this.intelligence.naturalLanguageReport(request.user.tenantId, request.user, body?.question || '', request);
  }

  @Post('actions')
  action(@Req() request: any, @Body() body: { insight_id?: string; action_code?: string; due_date?: string; payload?: Record<string, any> }) {
    return this.intelligence.executeControlledAction(request.user.tenantId, request.user, body || {}, request);
  }

  @Get('action-requests')
  actionRequests(@Req() request: any, @Query('status') status?: string) {
    return this.governedActions.list(request.user.tenantId, request.user, status);
  }

  @Post('action-requests')
  requestAction(@Req() request: any, @Body() body: { action_code?: string; payload?: Record<string, any> }) {
    return this.governedActions.request(request.user.tenantId, request.user, body?.action_code || '', body?.payload || {}, request);
  }

  @Patch('action-requests/:id/approve')
  approveAction(@Req() request: any, @Param('id') id: string) {
    return this.governedActions.approve(request.user.tenantId, request.user, id, request);
  }

  @Patch('action-requests/:id/reject')
  rejectAction(@Req() request: any, @Param('id') id: string, @Body() body: { reason?: string }) {
    return this.governedActions.reject(request.user.tenantId, request.user, id, body?.reason || '', request);
  }

  @Post('action-requests/:id/execute')
  executeAction(@Req() request: any, @Param('id') id: string) {
    return this.governedActions.execute(request.user.tenantId, request.user, id, request);
  }
  ...
  draftWorkflow(@Req() request: any, @Body() body: { instruction?: string }) {
    return this.intelligence.draftWorkflow(request.user.tenantId, request.user, body?.instruction || '', request);
  }

  @Post('workflows/:id/execute')
  executeWorkflow(@Req() request: any, @Param('id') id: string) {
    return this.intelligence.executeWorkflowDraft(request.user.tenantId, request.user, id, request);
  }

  @Get('events')
  events(@Req() request: any, @Query('limit') limit?: string, @Query('correlation_id') correlationId?: string) {
    return this.intelligence.recentEvents(request.user.tenantId, Number(limit) || 25, correlationId);
  }
  ...
  memory(@Req() request: any, @Query('limit') limit?: string) {
    return this.intelligence.businessMemory(request.user.tenantId, Number(limit) || 100);
  }

  @Post('knowledge-graph/refresh')
  refreshKnowledgeGraph(@Req() request: any) { return this.knowledgeGraph.refresh(request.user.tenantId); }

  @Get('knowledge-graph')
  knowledgeGraphView(@Req() request: any, @Query('limit') limit?: string) { return this.knowledgeGraph.graph(request.user.tenantId, Number(limit) || 500); }

  @Get('knowledge-graph/path')
  knowledgeGraphPath(@Req() request: any, @Query('from') from?: string, @Query('to') to?: string) { return this.knowledgeGraph.path(request.user.tenantId, from || '', to || ''); }
  ...
  analyseOnboarding(@Req() request: any, @Body() body: any) { return this.onboardingIntelligence.analyse(request.user.tenantId, request.user.userId || request.user.id, body || {}); }
  ...
  @Get('agents')
  agentDashboard(@Req() request: any) { return this.agents.dashboard(request.user.tenantId); }

  @Post('agents/policies')
  createAgentPolicy(@Req() request: any, @Body() body: any) { return this.agents.createPolicy(request.user.tenantId, request.user, body || {}); }

  @Patch('agents/policies/:id/approve')
  approveAgentPolicy(@Req() request: any, @Param('id') id: string) { return this.agents.approvePolicy(request.user.tenantId, request.user, id); }

  @Post('agents/policies/:id/run')
  runAgentPolicy(@Req() request: any, @Param('id') id: string) { return this.agents.run(request.user.tenantId, request.user, id); }
