...
-- STEP 2: Delete excess UIDs for RAD-TRR-QX71W915 (keep first 7)
WITH to_delete AS (
    SELECT 
        ur.id,
        ur.uid,
...
WHERE id IN (SELECT id FROM to_delete WHERE rn > 7)
RETURNING id, uid;

-- STEP 3: Delete excess UIDs for RAD-TRR-R9MI1W915 (keep first 7)
WITH to_delete AS (
    SELECT 
        ur.id,
        ur.uid,
...
-- STEP 4: Update grn_items uid_count to match reality
UPDATE grn_items gi
SET uid_count = 7
WHERE grn_id = (SELECT id FROM grns WHERE grn_number = 'GRN-2025-12-001')
    AND accepted_qty = 7
RETURNING item_code, uid_count;
