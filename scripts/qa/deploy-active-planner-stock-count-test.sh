...
stage=/tmp/mizantra-active-planner-stock-count-20260830
backup=/root/sak-deploy-backups/mizantra-active-planner-stock-count-$(date +%Y%m%d-%H%M%S)
...
paths=(
  ...
  scripts/qa/deploy-active-planner-stock-count-test.sh
)
...
if [ "$verified" != true ]; then
  ...
  pm2 restart sak-api-test
  echo 'Stock-count prompt deployment failed; previous Mizantra test API restored.' >&2
  exit 1
fi

pm2 save
echo "Mizantra TEST stock-count prompt deployed and verified. Backup: $backup"
