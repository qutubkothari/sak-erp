...
stage=/tmp/mizantra-active-planner-grn-20260830
backup=/root/sak-deploy-backups/mizantra-active-planner-grn-$(date +%Y%m%d-%H%M%S)
...
paths=(
  ...
  scripts/qa/deploy-active-planner-grn-test.sh
)
...
if [ "$verified" != true ]; then
  ...
  pm2 restart sak-api-test
  echo 'Governed GRN prompt deployment failed; previous Mizantra test API restored.' >&2
  exit 1
fi

pm2 save
echo "Mizantra TEST governed GRN prompt deployed and verified. Backup: $backup"
