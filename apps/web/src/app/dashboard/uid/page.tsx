...
export default function UIDTrackingPage() {
  ...
  const printBulkLabels = async (records: UIDRecord[]) => {
    if (records.length === 0) {
    ...
    try {
      const html = await buildUidLabelPrintHtml(
        records.map((record) => ({
          uid: record.uid,
          partNumber: getDisplayPartNumber(record),
          itemCode: record.items?.code || record.itemCode,
        })),
      );
      ...
      printWindow.close();
      alert(error?.message || 'Unable to generate UID labels. Please try again.');
    }
  };
  ...
  const printQrLabel = useCallback(async (record: UIDRecord) => {
    ...
    try {
      ...
      printWindow.close();
      alert(error?.message || 'Unable to generate this UID label. Please try again.');
    }
  }, []);
