...
export default function Sidebar({ collapsed, onToggle }: SidebarProps) {
  ...
  return (
    <aside 
      className={`fixed left-0 top-0 h-screen bg-[#4A3426] border-r-2 border-[#6F4E37] z-50 flex flex-col transition-all duration-300 ${
        collapsed ? 'w-16' : 'w-56'
      }`}
    >
      {/* Header */}
      <div className={`h-14 flex items-center border-b-2 border-[#8B6F47]/40 ${collapsed ? 'justify-center px-2' : 'justify-between px-3'}`}>
        {!collapsed && (
          <Link href={homeHref} className="flex items-center gap-2">
            ...
            <span className="font-bold text-sm text-[#FFFDF8] truncate max-w-[132px]" title={appBranding.companyName}>{appBranding.companyName}</span>
          </Link>
        )}
        <button
          onClick={onToggle}
      ...
      {!collapsed && (
        <div className="px-3 pt-2 pb-1">
          ...
          >
            ...
            <kbd className="hidden sm:inline text-[10px] bg-white/90 border border-[#D8C8AA] rounded px-1.5 py-0.5 font-mono text-[#6F4E37]">⌘K</kbd>
          </button>
        </div>
      )}
      {collapsed && (
      ...
      {/* User section */}
      <div className={`border-t-2 border-[#8B6F47]/40 p-2 ${collapsed ? 'flex flex-col items-center gap-1' : ''}`}>
        {/* Dark mode toggle */}
        <button
