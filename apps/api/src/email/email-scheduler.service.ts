...
export class EmailSchedulerService implements OnModuleInit {
  ...
  async fetchEmailsJob() {
    ...
    try {
      ...
      if (gmailOAuthReady) {
        const fetched = await this.gmailWebhookService.syncRecentMessages(50);
        this.logger.log(`Gmail OAuth sync inspected ${fetched} recent inbox messages`);
        return;
      }

      // IMAP remains available for non-Gmail providers.
      const emails = await this.fetchService.fetchEmails({ limit: 50, unseen: true });
  ...
  async parseEmailsJob() {
    ...
    // CRM enquiry classification is handled by CrmEmailIntakeScheduler using
    // the governed AI taxonomy. Do not run the legacy rule parser automatically.
    if (this.configService.get<string>('CRM_AI_EMAIL_INTAKE', 'true') === 'true') {
      return;
    }
