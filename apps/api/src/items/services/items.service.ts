...
export class ItemsService {
  ...
  async uploadDrawing(
    ...
    if (deactivateError) {
      throw new Error(
        `Failed to finalize drawing upload: ${deactivateError.message}`,
      );
    }
  ...
  async updateDrawing(
    ...
    if (drawingData.isActive === true) {
      ...
      if (deactivateError) {
        throw new Error(
          `Failed to activate drawing: ${deactivateError.message}`,
        );
      }
    }
  ...
  async getStockTrail(tenantId: string, itemId: string) {
    ...
    for (const t of trails) {
      ...
      stockByWarehouse.set(
        warehouse,
        (stockByWarehouse.get(warehouse) || 0) +
          Number(t.qty_in || 0) -
          Number(t.qty_out || 0),
      );
    }

    const currentStock = Array.from(stockByWarehouse.entries()).map(
