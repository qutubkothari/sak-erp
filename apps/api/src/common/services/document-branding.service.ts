...
export class DocumentBrandingService {
  ...
  async preparePdfBrandingAssets(pdfDoc: PDFDocument, branding?: DocumentBranding): Promise<PdfBrandingAssets> {
    ...
    // Prefer the tenant's own configured logo so documents don't cross-brand between tenants.
    if (branding?.logoUrl) {
      assets.markLogo = await this.tryEmbedImage(pdfDoc, branding.logoUrl, 'tenant logo');
    }

    if (!assets.markLogo) {
      const markLogoPath = this.resolveExistingAsset(['saif-seas-logo.png']);
      if (markLogoPath) {
        try {
          assets.markLogo = await pdfDoc.embedPng(fs.readFileSync(markLogoPath));
        } catch (error) {
          this.logger.warn(`Failed to embed default logo: ${String((error as any)?.message || error)}`);
        }
      }
    }
  ...
  drawStandardHeader(options: StandardPdfHeaderOptions): number {
    ...
    const markScale = assets?.markLogo
      ? Math.min(165 / assets.markLogo.width, 58 / assets.markLogo.height)
      : 1;
    ...
    } else if (!assets?.markLogo) {
      // Only draw the company name here when there is no logo image at all —
      // otherwise it duplicates (and can overlap) the name already shown in
      // the right-hand address block below.
