...
export function normalizeRfqPercentage(value: unknown, label: string): number {
  ...
  async recordRFQResponse(
    ...
    for (const item of itemPayload) {
      ...
      const itemUpdate = {
        ...
        vendor_discount_percent: normalizeRfqPercentage(
          item?.discountPercent ?? item?.vendor_discount_percent,
          'Discount percentage',
        ),
        vendor_gst_percent: normalizeRfqPercentage(
          item?.gstPercent ?? item?.vendor_gst_percent,
          'GST percentage',
        ),
