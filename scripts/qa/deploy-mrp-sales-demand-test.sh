...
stage=/tmp/mizantra-mrp-sales-demand-20260829
backup=/root/sak-deploy-backups/mizantra-mrp-sales-demand-$(date +%Y%m%d-%H%M%S)
...
if [ "$verified" != true ]; then
  ...
  pm2 restart sak-api-test sak-web-test
  echo 'MRP sales-demand deployment failed health checks; previous test runtime restored.' >&2
  exit 1
fi

echo "Mizantra TEST MRP sales demand deployed and verified. Backup: $backup"
