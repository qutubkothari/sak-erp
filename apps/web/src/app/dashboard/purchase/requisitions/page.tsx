...
interface RfqRecord {
  ...
  rfq_items?: Array<{
    ...
    vendor_discount_percent?: number | null;
...
function calculateRfqLineTotal(
  quantity: number | string | null | undefined,
  quotedPrice: number | string | null | undefined,
  discountPercent: number | string | null | undefined,
  gstPercent: number | string | null | undefined,
): number {
  const qty = Math.max(0, Number(quantity) || 0);
  const rate = Math.max(0, Number(quotedPrice) || 0);
  const discount = Math.min(100, Math.max(0, Number(discountPercent) || 0));
  const gst = Math.min(100, Math.max(0, Number(gstPercent) || 0));
...
function PRContent() {
  ...
  const handleSaveDeliveryAddress = async () => {
    ...
    if (!address) {
      toast.error('Enter a delivery address before saving. Your requisition remains open.');
      return;
    }
    ...
    try {
      ...
      toast.error(error?.message || 'Failed to save delivery address. Check the address and retry.');
    } finally {
  ...
  const fetchMasterItems = async () => {
    ...
    try {
      ...
      // Fetch temporary R&D items through their own database-scoped query and
      // merge them with the normal catalogue. This keeps previously generated
      // TEMP items reusable even after the main catalogue exceeds an API page.
  ...
  const handleViewDetails = async (prId: string) => {
    ...
    try {
      ...
      toast.error('Failed to load PR details. Refresh the register and try again.');
  ...
  const handleEditPR = async (prId: string) => {
    try {
      ...
      toast.error(msg ? `Failed to load PR for editing: ${msg}` : 'Failed to load PR for editing. Refresh and retry.');
    }
  };
  ...
  const fetchRFQVendors = async () => {
    try {
      ...
      toast.error('Failed to load vendors. Refresh the RFQ panel and retry.');
    } finally {
  ...
  const fetchRfqHistory = async (prId: string, options?: { silent?: boolean }) => {
    try {
      ...
      if (!options?.silent) {
        toast.error('Failed to load RFQ responses. Existing PR data is unchanged.');
      }
      setRfqHistory([]);
    } finally {
      if (!options?.silent) {
    ...
    setRfqResponseForm({
      ...
      items: sourceItems
        .map((item) => ({
            ...
            discountPercent:
              item.vendor_discount_percent == null || Number.isNaN(Number(item.vendor_discount_percent))
                ? '0'
                : String(item.vendor_discount_percent),
            gstPercent:
              item.vendor_gst_percent == null || Number.isNaN(Number(item.vendor_gst_percent))
                ? '0'
                : String(item.vendor_gst_percent),
            leadTime:
  ...
  const uploadRfqResponseFiles = async (files: FileList | null) => {
    ...
    try {
      ...
      toast.error(`Failed to upload RFQ attachment: ${error?.message || 'Unknown error'}. Choose the file again and retry.`);
    } finally {
  ...
  const saveRfqResponse = async () => {
    ...
    if (rfqResponseForm.followUpDate && !followUpDate) {
      toast.error('Enter a valid follow-up date before saving the supplier response.');
      return;
    }
    ...
    try {
      ...
      toast.error(error?.message || 'Failed to save the RFQ response. Review supplier prices and dates, then retry.');
    } finally {
  ...
  const handlePreviewRFQ = async (options?: { keepIndex?: number }) => {
    ...
    if (selectedVendors.length === 0) {
      toast.error('Select at least one vendor for the requisition items before previewing the RFQ.');
      return;
    }
    ...
    try {
      ...
      toast.error(message === 'Failed to generate RFQ preview' ? `${message}. Review vendor email details and retry.` : `Failed to generate RFQ preview: ${message}`);
    } finally {
  ...
  const handleSendRFQ = async () => {
    if (!RFQ_EMAIL_SENDING_ENABLED) {
      toast.info('RFQ emails are currently disabled. Use Preview RFQ to review the email content.');
      return;
    }
    ...
    const selectedVendors = getSelectedVendorIds();
    if (selectedVendors.length === 0) {
      toast.error('Select at least one vendor for the requisition items before sending the RFQ.');
      return;
    }
    const sanitizedRfqItemVendors = sanitizeRfqItemVendorMap(rfqItemVendors);
    ...
    try {
      ...
      const skippedDetails = skippedCount > 0 && skippedList.length > 0
        ...
        : '';
      toast.info(`RFQ sent: ${sentCount}, failed: ${failedCount}, skipped: ${skippedCount}${failedDetails}${skippedDetails}`, { duration: 10000 });
      ...
      toast.error('Failed to send RFQ. The requisition is unchanged; verify vendor email configuration and retry.');
    } finally {
      setRfqSending(false);
    }
  };
            ...
            <div className="bg-white rounded-lg shadow-xl w-full w-full max-w-none max-h-[92vh] overflow-hidden flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full min-w-[1320px] text-sm border rounded-lg overflow-hidden">
                    <thead className="bg-[#F5EFE3]">
                      <tr>
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setRfqResponseForm((prev) => ({
                                  ...prev,
                                  items: prev.items.map((row, rowIndex) =>
                                    rowIndex === index ? { ...row, discountPercent: e.target.value } : row,
                                  ),
                                }))
                              }
                              className="w-full min-w-[100px] border rounded px-3 py-2 text-right"
                              placeholder="0.00"
                            />
                          </td>
                          <td className="px-3 py-2">
                            <input
                              ...
                              onChange={(e) =>
                                setRfqResponseForm((prev) => ({
                                  ...prev,
                                  items: prev.items.map((row, rowIndex) =>
                                    rowIndex === index ? { ...row, gstPercent: e.target.value } : row,
                                  ),
                                }))
                              }
                              className="w-full min-w-[100px] border rounded px-3 py-2 text-right"
