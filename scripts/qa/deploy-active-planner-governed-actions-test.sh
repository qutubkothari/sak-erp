...
stage=/tmp/mizantra-active-planner-governed-actions-20260830
backup=/root/sak-deploy-backups/mizantra-active-planner-governed-actions-$(date +%Y%m%d-%H%M%S)
...
paths=(
  ...
  scripts/qa/deploy-active-planner-governed-actions-test.sh
)
...
if [ "$verified" != true ]; then
  ...
  pm2 restart sak-api-test sak-web-test
  echo 'Governed prompt-action deployment failed; previous Mizantra test runtime restored.' >&2
  exit 1
fi

echo "Mizantra TEST governed prompt actions deployed and verified. Backup: $backup"
