...
stage=/tmp/mizantra-active-planner-customer-receipt-20260830
backup=/root/sak-deploy-backups/mizantra-active-planner-customer-receipt-$(date +%Y%m%d-%H%M%S)
...
paths=(
  ...
  scripts/qa/deploy-active-planner-customer-receipt-test.sh
)
...
if [ "$verified" != true ]; then
  ...
  pm2 restart sak-api-test
  echo 'Governed customer-receipt deployment failed; previous Mizantra test API restored.' >&2
  exit 1
fi

pm2 save
echo "Mizantra TEST governed customer-receipt prompt deployed and verified. Backup: $backup"
