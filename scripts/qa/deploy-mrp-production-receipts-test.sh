...
stage=/tmp/mizantra-mrp-production-receipts-20260829
backup=/root/sak-deploy-backups/mizantra-mrp-production-receipts-$(date +%Y%m%d-%H%M%S)
...
if [ "$verified" != true ]; then
  ...
  pm2 restart sak-api-test sak-web-test
  echo 'MRP production-receipt deployment failed health checks; previous test runtime restored.' >&2
  exit 1
fi

echo "Mizantra TEST MRP production receipts deployed and verified. Backup: $backup"
