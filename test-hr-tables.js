  ...
  try {
    ...
    if (error) {
      if (error.code === 'PGRST116') {
        console.log('❌ HR tables do not exist yet');
      ...
      console.log('✅ HR tables already exist');
      console.log('Testing HR operations...');
      await testHROperations();
    }
  } catch (err) {
...
async function testHROperations() {
  try {
    ...
    console.log('✅ HR operations working correctly!');
    return true;
  } catch (error) {
    console.error('❌ HR operations test failed:', error);
    return false;
  }
}
