-- Clear all transactional data (BOM, GRN, PR, PO, Job Orders) for tenant
...
BEGIN
    ...
    -- Clear Job Order related data (NEW)
    BEGIN
        RAISE NOTICE 'Deleting job order quality records...';
        DELETE FROM job_order_quality WHERE tenant_id = v_tenant_id;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE NOTICE 'Job order quality table not found, skipping...';
        WHEN undefined_column THEN
            RAISE NOTICE 'Job order quality columns mismatch, skipping...';
    END;
    
    BEGIN
        RAISE NOTICE 'Deleting job order materials...';
        DELETE FROM job_order_materials WHERE tenant_id = v_tenant_id;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE NOTICE 'Job order materials table not found, skipping...';
        WHEN undefined_column THEN
            RAISE NOTICE 'Job order materials columns mismatch, skipping...';
    END;
    
    BEGIN
        RAISE NOTICE 'Deleting job order operations...';
        DELETE FROM job_order_operations WHERE tenant_id = v_tenant_id;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE NOTICE 'Job order operations table not found, skipping...';
        WHEN undefined_column THEN
            RAISE NOTICE 'Job order operations columns mismatch, skipping...';
    END;
    
    BEGIN
        RAISE NOTICE 'Deleting job orders...';
        DELETE FROM production_job_orders WHERE tenant_id = v_tenant_id;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE NOTICE 'Job orders table not found, skipping...';
    END;
    ...
    -- Clear Routing data (if exists)
    BEGIN
        RAISE NOTICE 'Deleting routing operations...';
        DELETE FROM routing_operations WHERE routing_id IN (
            SELECT id FROM routing WHERE tenant_id = v_tenant_id
        );
        
        RAISE NOTICE 'Deleting routing...';
        DELETE FROM routing WHERE tenant_id = v_tenant_id;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE NOTICE 'Routing tables not found, skipping...';
    END;
    ...
    -- Clear inventory stock (optional - CAREFUL!)
    -- RAISE NOTICE 'Resetting inventory stock...';
...
SELECT 'Job Orders' as table_name, COUNT(*) as count FROM production_job_orders WHERE tenant_id = 'f87a5ab0-0619-4f1c-bab9-e78ca750e56c'
UNION ALL
SELECT 'BOMs', COUNT(*) FROM bom_headers WHERE tenant_id = 'f87a5ab0-0619-4f1c-bab9-e78ca750e56c'
UNION ALL
