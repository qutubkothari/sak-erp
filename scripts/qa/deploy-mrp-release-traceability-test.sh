...
stage=/tmp/mizantra-mrp-release-traceability-20260829
backup=/root/sak-deploy-backups/mizantra-mrp-release-traceability-$(date +%Y%m%d-%H%M%S)
...
if [ "$verified" != true ]; then
  ...
  pm2 restart sak-api-test sak-web-test
  echo 'MRP release traceability deployment failed health checks; previous test runtime restored.' >&2
  exit 1
fi

echo "Mizantra TEST MRP release traceability deployed and verified. Backup: $backup"
