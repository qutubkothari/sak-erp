  ...
  it("exposes discounted GRN values to QC while preserving stored rounding", async () => {
    ...
    jest.spyOn(service as any, "getPoItemPricingMap").mockResolvedValue(
      new Map([
        [
          "po-line-1",
          { rate: 792, discountPercent: 10, taxPercent: 18 },
        ],
      ]),
    );

    const result = await (service as any).attachGrnLinePricing({
      id: "grn-1",
      grn_items: [
        {
          id: "grn-line-1",
    ...
    expect(result.grn_items[0]).toEqual(
      expect.objectContaining({
        gross_unit_price: 792,
        discount_percent: 10,
        net_unit_price: 712.8,
        net_amount: 35640,
      }),
    );
  });

  it("rejects an empty GRN before inserting a header", async () => {
    const service = new GrnService({} as any);
    ...
    await expect(
      service.create("tenant-1", "user-1", {
        ...
        invoiceFileUrl: "/uploads/inv-empty.pdf",
        items: [],
      }),
    ).rejects.toThrow("At least one material line is required");

    expect(insert).not.toHaveBeenCalled();
  });

  it("rejects an update that would remove every GRN line", async () => {
    const service = new GrnService({} as any);

    await expect(
      service.update("tenant-1", "grn-1", { items: [] }),
    ).rejects.toThrow("cannot be saved without at least one material line");
  });

  it("rejects QC completion with an empty submitted line set", async () => {
    const service = new GrnService({} as any);

    await expect(
      service.qcAccept("tenant-1", "grn-1", "user-1", { items: [] }),
    ).rejects.toThrow("has no material lines");
  });
...
describe("GrnService supplier invoice sanction controls", () => {
  function createServiceWithGrnMock(currentGrn: any) {
    const service = new GrnService({} as any);
    const currentGrnWithLines = {
      grn_items: [{ id: "grn-line-1" }],
      ...currentGrn,
    };
    const selectQuery: any = {
      ...
      single: jest
        .fn()
        .mockResolvedValue({ data: currentGrnWithLines, error: null }),
    };
    const updateQuery: any = {
      update: jest.fn(() => updateQuery),
      eq: jest.fn(() => updateQuery),
  ...
  it("blocks supplier invoice sanction for an empty completed GRN", async () => {
    const { service } = createServiceWithGrnMock({
      id: "grn-empty",
    ...
    await expect(
      service.approveInvoice("tenant-1", "grn-empty", "approver-1", {}),
    ).rejects.toThrow("has no material lines");
  });
