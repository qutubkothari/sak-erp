...
export default function SelfAssessmentPage() {
  ...
  const [cycle] = useState<ReviewCycle>({
    id: '1',
    name: 'Q4 2024 Performance Review',
    startDate: '2024-10-01',
    endDate: '2024-12-31',
    selfAssessmentDeadline: '2024-12-15',
  ...
  const [competencies] = useState<Competency[]>([
    {
      id: 'c1',
      name: 'Communication',
      description: 'Effectively conveys information and actively listens',
    ...
    },
    {
      ...
      description: 'Demonstrates expertise in job-specific technical skills',
      category: 'Technical Skills',
    },
    {
      id: 'c4',
      name: 'Teamwork',
      description: 'Collaborates effectively with colleagues',
      category: 'Behavioral',
    },
    {
      id: 'c5',
      name: 'Leadership',
      description: 'Guides and motivates team members',
      category: 'Leadership',
    },
  ]);

  const [kpis] = useState<KPI[]>([
    { id: 'k1', name: 'Customer Satisfaction Score', target: 85, unit: '%' },
    { id: 'k2', name: 'Projects Delivered On Time', target: 90, unit: '%' },
    { id: 'k3', name: 'Code Quality Score', target: 95, unit: '%' },
  ]);
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-[#6F4E37]">
            {cycle.name} • Due {new Date(cycle.selfAssessmentDeadline).toLocaleDateString()}
          </p>
          {daysRemaining > 0 && (
            ...
            <div className="space-y-6">
              {kpis.map((kpi) => (
                <div key={kpi.id} className="border-b border-[#F4ECE2] pb-6 last:border-0 last:pb-0">
                  ...
                  {kpiData[kpi.id]?.achieved && (
                    <div className="mt-3">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs text-[#9C8162]">Progress</span>
                        <span className="text-xs font-semibold text-[#36454F]">
                          {Math.round((kpiData[kpi.id].achieved / kpi.target) * 100)}%
                        </span>
                      </div>
                      <div className="h-2 w-full rounded-full bg-[#F4ECE2]">
                        <div
                          className={`h-2 rounded-full transition-all duration-300 ${
                            (kpiData[kpi.id].achieved / kpi.target) * 100 >= 100
                              ? 'bg-green-500'
                              : (kpiData[kpi.id].achieved / kpi.target) * 100 >= 75
                              ? 'bg-[#6F4E37]'
                              : 'bg-yellow-500'
                          }`}
                          style={{
                            width: `${Math.min((kpiData[kpi.id].achieved / kpi.target) * 100, 100)}%`,
                          }}
                        />
                      </div>
                    </div>
