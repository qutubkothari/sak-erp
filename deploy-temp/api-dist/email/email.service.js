...
let EmailService = class EmailService {
    ...
    async applyFromAndReplyTo(mailOptions, fromType = 'noreply') {
        ...
        const sameAddress = this.normalizeEmailAddress(configuredFromEmail) === this.normalizeEmailAddress(transportUser);
        if (sameAddress) {
            return {
                ...mailOptions,
                from: configuredFrom
    ...
    async sendPOTrackingReminder(to, poData) {
        let mailOptions = {
            to,
    ...
    async sendDispatchNote(to, dispatchData) {
        let mailOptions = {
            to,
    ...
    async sendIssueCertificate(to, certificateData) {
        let mailOptions = {
            to,
    ...
    generateTrackingReminderTemplate(poData) {
      ...
      <html>
        ...
        <body>
          ...
          <div class="content">
            ...
            <div class="alert-box">
              ${daysOverdue > 0 ? `<strong>⚠️ URGENT:</strong> This order is ${daysOverdue} days overdue.` : '<strong>📦 Reminder:</strong> Please provide tracking information for this order.'}
            </div>
    ...
    generateDispatchTemplate(dispatchData) {
        return `
      <!DOCTYPE html>
      <html>
        <head>
    ...
    generateCertificateTemplate(certificateData) {
        return `
      <!DOCTYPE html>
      <html>
        <head>
    ...
    async sendLowStockAlert(to, lowStockItems) {
        let mailOptions = {
            to,
    ...
    generateLowStockTemplate(lowStockItems) {
        return `
      <!DOCTYPE html>
      <html>
        <head>
    ...
    constructor(configService, emailConfig, gmailOAuth2Service){
