...
Object.defineProperty(exports, "DocumentWorkflowService", {
    enumerable: true,
    get: function() {
...
const _emailconfigservice = require("../../email/email-config.service");
...
let DocumentWorkflowService = class DocumentWorkflowService {
    async resolveTargetUserId(tenantId, dto) {
        ...
        const { data: user } = await this.supabase.from('users').select('id').eq('tenant_id', tenantId).ilike('email', email.trim()).maybeSingle();
        return user?.id || null;
    }
    normalizeClientDetails(dto) {
    ...
    async forwardToStaff(req, documentId, dto) {
        const tenantId = req.user.tenantId;
        const userId = req.user.userId;
        const targetUserId = await this.resolveTargetUserId(tenantId, dto);
        const { data: doc } = await this.supabase.from('documents').select('*, created_by_user:users!documents_created_by_fkey(id, first_name, last_name, email)').eq('tenant_id', tenantId).eq('id', documentId).single();
        ...
        if (targetUserId) {
            const { data: targetUser } = await this.supabase.from('users').select('email, first_name, last_name').eq('id', targetUserId).eq('tenant_id', tenantId).single();
            if (targetUser?.email) {
        ...
        return {
            ok: true,
            message: 'Document forwarded to staff for review'
        };
    }
    async returnToAdmin(req, documentId, dto) {
        const tenantId = req.user.tenantId;
        const userId = req.user.userId;
        const comments = typeof dto === 'string' ? dto : typeof dto?.comments === 'string' ? dto.comments : undefined;
        const { data: doc } = await this.supabase.from('documents').select('*, created_by_user:users!documents_created_by_fkey(id, first_name, last_name, email)').eq('tenant_id', tenantId).eq('id', documentId).single();
        if (!doc) throw new Error('Document not found');
        await this.supabase.from('documents').update({
        ...
        return {
            ok: true,
            message: 'Document returned to admin for approval'
        };
    }
    async forwardToManager(req, documentId, dto) {
        const tenantId = req.user.tenantId;
        const userId = req.user.userId;
        const targetUserId = await this.resolveTargetUserId(tenantId, dto);
        const { data: doc } = await this.supabase.from('documents').select('*').eq('tenant_id', tenantId).eq('id', documentId).single();
        if (!doc) throw new Error('Document not found');
        await this.supabase.from('documents').update({
            status: 'PENDING_APPROVAL',
        ...
        if (targetUserId) {
            const { data: manager } = await this.supabase.from('users').select('email, first_name, last_name').eq('id', targetUserId).eq('tenant_id', tenantId).single();
            if (manager?.email) {
        ...
        return {
            ok: true,
            message: 'Document forwarded to manager for approval'
        };
    }
    async sendToClient(req, documentId, dto) {
        const tenantId = req.user.tenantId;
        const userId = req.user.userId;
        const { data: doc } = await this.supabase.from('documents').select('*').eq('tenant_id', tenantId).eq('id', documentId).single();
        if (!doc) throw new Error('Document not found');
        ...
        const viewUrl = doc.file_path ? await this.storageService.getSignedUrl(doc.file_path, expiryDays * 24 * 60 * 60) : doc.file_url;
    ...
    async rejectDocument(req, documentId, dto) {
        const tenantId = req.user.tenantId;
        const userId = req.user.userId;
        const { data: doc } = await this.supabase.from('documents').select('*, created_by_user:users!documents_created_by_fkey(id, first_name, last_name, email)').eq('tenant_id', tenantId).eq('id', documentId).single();
        if (!doc) throw new Error('Document not found');
    ...
    async finalApprove(req, documentId) {
        const tenantId = req.user.tenantId;
        const userId = req.user.userId;
        await this.supabase.from('documents').update({
            status: 'APPROVED',
    ...
    async clientUploadRevision(token, file, body) {
        // Find document by token
        const { data: docs } = await this.supabase.from('documents').select('*').not('metadata', 'is', null);
        ...
        if (!doc) {
            throw new _common.UnauthorizedException('Invalid or expired upload token');
        }
        // Upload new file
        const upload = await this.storageService.uploadFile(file, 'documents', doc.id);
        // Calculate next revision number
        const { data: revisions } = await this.supabase.from('document_revisions').select('revision_number').eq('document_id', doc.id).order('created_at', {
            ascending: false
        }).limit(1);
        ...
        // Create new revision
        await this.supabase.from('document_revisions').insert({
        ...
        await this.supabase.from('document_revisions').update({
            status: 'SUPERSEDED'
        }).eq('document_id', doc.id).neq('revision_number', nextRevision);
        // Update document
        await this.supabase.from('documents').update({
        ...
        if (doc.metadata.client_access.created_by) {
            const { data: manager } = await this.supabase.from('users').select('email, first_name').eq('id', doc.metadata.client_access.created_by).single();
            if (manager?.email) {
    ...
    async logWorkflowAction(tenantId, documentId, userId, action, metadata) {
        ...
        try {
            const { data: doc } = await this.supabase.from('documents').select('status').eq('id', documentId).single();
            let actorName = 'System';
            let actorEmail = '';
            if (userId) {
                const { data: user } = await this.supabase.from('users').select('first_name, last_name, email').eq('id', userId).single();
                if (user) {
                    actorName = `${user.first_name || ''} ${user.last_name || ''}`.trim();
                    actorEmail = user.email || '';
                }
            }
            await this.supabase.from('document_workflow_history').insert({
                tenant_id: tenantId,
                document_id: documentId,
                action,
    ...
    async getWorkflowHistory(req, documentId) {
        const tenantId = req.user.tenantId;
        const { data, error } = await this.supabase.from('document_workflow_history').select('*').eq('tenant_id', tenantId).eq('document_id', documentId).order('created_at', {
            ascending: false
        });
        if (error) {
    ...
    async sendReviewRequestEmail(email, doc, requestedBy) {
    ...
    async sendApprovalRequestEmail(email, doc, requestedBy) {
    ...
    async sendClientReviewEmail(email, doc, options) {
    ...
    async sendRejectionEmail(email, doc, reason, rejectedBy) {
    ...
    async sendClientUploadNotification(email, doc, revisionNumber) {
    ...
    constructor(emailService, emailConfig, storageService){
        this.emailService = emailService;
        this.emailConfig = emailConfig;
        this.storageService = storageService;
        this.logger = new _common.Logger(DocumentWorkflowService.name);
        this.supabase = (0, _supabasejs.createClient)(process.env.SUPABASE_URL, process.env.SUPABASE_KEY);
    }
};
DocumentWorkflowService = _ts_decorate([
...
//# sourceMappingURL=document-workflow.service.js.map