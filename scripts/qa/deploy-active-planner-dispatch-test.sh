...
stage=/tmp/mizantra-active-planner-dispatch-20260830
backup=/root/sak-deploy-backups/mizantra-active-planner-dispatch-$(date +%Y%m%d-%H%M%S)
...
paths=(
  ...
  scripts/qa/deploy-active-planner-dispatch-test.sh
)
...
if [ "$verified" != true ]; then
  ...
  pm2 restart sak-api-test
  echo 'Governed dispatch deployment failed; previous Mizantra test API restored.' >&2
  exit 1
fi

pm2 save
echo "Mizantra TEST governed dispatch prompt deployed and verified. Backup: $backup"
