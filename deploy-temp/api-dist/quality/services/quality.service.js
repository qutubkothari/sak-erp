...
Object.defineProperty(exports, "QualityService", {
    enumerable: true,
    get: function() {
...
let QualityService = class QualityService {
    // ==================== Quality Inspections ====================
    async createInspection(tenantId, userId, data) {
        ...
        const { data: inspection, error } = await this.supabase.from('quality_inspections').insert(inspectionData).select().single();
        if (error) {
    ...
    async getInspections(tenantId, filters) {
        let query = this.supabase.from('quality_inspections').select('*').eq('tenant_id', tenantId).order('inspection_date', {
            ascending: false
        });
    ...
    async getInspectionById(tenantId, inspectionId) {
        const { data, error } = await this.supabase.from('quality_inspections').select(`
        *,
        parameters:inspection_parameters(*),
        defects:inspection_defects(*),
        ncr:ncr(*)
      `).eq('tenant_id', tenantId).eq('id', inspectionId).single();
        if (error) throw new _common.NotFoundException('Inspection not found');
        return data;
    }
    async updateInspection(tenantId, inspectionId, data) {
        const updateData = {
            inspection_type: data.inspection_type,
        ...
        const { data: inspection, error } = await this.supabase.from('quality_inspections').update(updateData).eq('tenant_id', tenantId).eq('id', inspectionId).eq('status', 'PENDING') // Only allow updates for pending inspections
        .select().single();
        if (error) throw new _common.BadRequestException(error.message);
        return inspection;
    }
    async deleteInspection(tenantId, inspectionId) {
        // Only allow deletion of pending inspections
        const { data: inspection, error: fetchError } = await this.supabase.from('quality_inspections').select('status').eq('tenant_id', tenantId).eq('id', inspectionId).single();
        if (fetchError) throw new _common.NotFoundException('Inspection not found');
        if (inspection.status !== 'PENDING') {
            throw new _common.BadRequestException('Only pending inspections can be deleted');
        }
        const { error } = await this.supabase.from('quality_inspections').delete().eq('tenant_id', tenantId).eq('id', inspectionId);
        if (error) throw new _common.BadRequestException(error.message);
        return {
            message: 'Inspection deleted successfully'
        };
    }
    async completeInspection(tenantId, userId, inspectionId, data) {
        // Get inspection details
        const { data: inspection, error: fetchError } = await this.supabase.from('quality_inspections').select('*').eq('tenant_id', tenantId).eq('id', inspectionId).single();
        if (fetchError) throw new _common.NotFoundException('Inspection not found');
        // Calculate defect rate
        const totalQty = (data.quantity_accepted || 0) + (data.quantity_rejected || 0) + (data.quantity_on_hold || 0);
        const defectRate = totalQty > 0 ? (data.quantity_rejected || 0) / totalQty * 100 : 0;
        // Update inspection
        const updateData = {
        ...
        const { data: updatedInspection, error: updateError } = await this.supabase.from('quality_inspections').update(updateData).eq('tenant_id', tenantId).eq('id', inspectionId).select().single();
        ...
        if (data.generate_ncr && data.ncr_description) {
            await this.createNCRFromInspection(tenantId, {
                ...inspection,
                ...updatedInspection
            }, {
    ...
    async addInspectionParameters(inspectionId, parameters) {
        ...
        const { error } = await this.supabase.from('inspection_parameters').insert(parameterData);
        if (error) throw new _common.BadRequestException(error.message);
    }
    async addInspectionDefects(inspectionId, defects) {
        ...
        const { error } = await this.supabase.from('inspection_defects').insert(defectData);
        if (error) throw new _common.BadRequestException(error.message);
    }
    // ==================== NCR Management ====================
    async createNCR(tenantId, userId, data) {
        ...
        const { data: ncr, error } = await this.supabase.from('ncr').insert(ncrData).select().single();
        if (error) throw new _common.BadRequestException(error.message);
        // Update inspection with NCR link if applicable
        if (data.inspection_id) {
            await this.supabase.from('quality_inspections').update({
                ncr_generated: true,
                ncr_id: ncr.id
    ...
    async createNCRFromInspection(tenantId, inspection, ncrDetails) {
        const ncrData = {
            tenant_id: tenantId,
    ...
    async getNCRs(tenantId, filters) {
        let query = this.supabase.from('ncr').select('*').eq('tenant_id', tenantId).order('ncr_date', {
            ascending: false
        });
        if (filters?.status) {
            query = query.eq('status', filters.status);
    ...
    async getNCRById(tenantId, ncrId) {
        const { data, error } = await this.supabase.from('ncr').select(`
        *,
        inspection:quality_inspections(*)
      `).eq('tenant_id', tenantId).eq('id', ncrId).single();
        if (error) throw new _common.NotFoundException('NCR not found');
        return data;
    }
    async updateNCR(tenantId, ncrId, data) {
        const { data: ncr, error } = await this.supabase.from('ncr').update({
            ...data,
            updated_at: new Date().toISOString()
        }).eq('tenant_id', tenantId).eq('id', ncrId).select().single();
        if (error) throw new _common.BadRequestException(error.message);
        return ncr;
    }
    async closeNCR(tenantId, ncrId, userId, data) {
        const { data: ncr, error } = await this.supabase.from('ncr').update({
            status: 'CLOSED',
    ...
    // ==================== Quality Analytics ====================
    async getVendorQualityRatings(tenantId, vendorId) {
        // Get all completed incoming inspections grouped by vendor
        const { data: inspections, error: inspError } = await this.supabase.from('quality_inspections').select('vendor_id, vendor_name, status, rejected_quantity, inspected_quantity').eq('tenant_id', tenantId).eq('inspection_type', 'INCOMING').not('vendor_id', 'is', null).in('status', [
            'PASSED',
            'FAILED'
        ]);
        if (inspError) throw new _common.BadRequestException(inspError.message);
        ...
        const { data: ncrs } = await this.supabase.from('ncr').select('vendor_id').eq('tenant_id', tenantId).not('vendor_id', 'is', null);
        const ncrCounts = {};
        ncrs?.forEach((ncr)=>{
            if (ncr.vendor_id) {
        ...
        const ratings = Object.entries(vendorStats).map(([vendorId, stats])=>{
            const passRate = stats.total_inspections > 0 ? stats.passed_inspections / stats.total_inspections * 100 : 0;
            const defectRatePpm = stats.total_quantity > 0 ? stats.total_defects / stats.total_quantity * 1000000 : 0;
    ...
    async calculateVendorQualityRating(tenantId, vendorId, periodStart, periodEnd) {
        // Get inspections for vendor in period
        const { data: inspections, error: inspError } = await this.supabase.from('quality_inspections').select('*').eq('tenant_id', tenantId).eq('vendor_id', vendorId).eq('inspection_type', 'INCOMING').gte('inspection_date', periodStart).lte('inspection_date', periodEnd);
        if (inspError) throw new _common.BadRequestException(inspError.message);
        ...
        // Get NCRs
        const { data: ncrs, error: ncrError } = await this.supabase.from('ncr').select('*').eq('tenant_id', tenantId).eq('vendor_id', vendorId).gte('ncr_date', periodStart).lte('ncr_date', periodEnd);
        if (ncrError) throw new _common.BadRequestException(ncrError.message);
        ...
        // Get vendor name
        const { data: vendor } = await this.supabase.from('vendors').select('vendor_name').eq('id', vendorId).single();
        const ratingData = {
            tenant_id: tenantId,
            vendor_id: vendorId,
        ...
        const { data: rating, error } = await this.supabase.from('vendor_quality_rating').insert(ratingData).select().single();
        if (error) throw new _common.BadRequestException(error.message);
        return rating;
    }
    async getQualityDashboard(tenantId) {
        // Get inspection summary
        const { data: inspections } = await this.supabase.from('quality_inspections').select('status, inspection_type').eq('tenant_id', tenantId);
        ...
        // Get NCR summary
        const { data: ncrs } = await this.supabase.from('ncr').select('status, nonconformance_type').eq('tenant_id', tenantId);
        const ncrSummary = {
            total: ncrs?.length || 0,
            open: ncrs?.filter((n)=>[
                    'OPEN',
        ...
        const { data: defects } = await this.supabase.from('inspection_defects').select('defect_category');
        const defectCounts = {};
        defects?.forEach((d)=>{
        ...
        const topDefects = Object.entries(defectCounts).map(([defect_type, count])=>({
                defect_type,
                count
            })).sort((a, b)=>b.count - a.count).slice(0, 5);
        // Calculate pass rate
        const totalInspections = inspectionSummary.total;
        const passRate = totalInspections > 0 ? inspectionSummary.passed / totalInspections * 100 : 0;
        return {
    ...
    // ==================== Helper Methods ====================
    async generateInspectionNumber(tenantId, type) {
        const prefix = type === 'INCOMING' ? 'IQC' : type === 'IN_PROCESS' ? 'IPQC' : 'FQC';
        const { count } = await this.supabase.from('quality_inspections').select('*', {
            count: 'exact',
            head: true
        }).eq('tenant_id', tenantId).eq('inspection_type', type);
        const nextNumber = (count || 0) + 1;
        return `${prefix}-${String(nextNumber).padStart(6, '0')}`;
    }
    async generateNCRNumber(tenantId) {
        const { count } = await this.supabase.from('ncr').select('*', {
            count: 'exact',
            head: true
    ...
    constructor(){
        this.supabase = new _supabasejs.SupabaseClient(process.env.SUPABASE_URL || '', process.env.SUPABASE_KEY || '');
    }
};
QualityService = _ts_decorate([
