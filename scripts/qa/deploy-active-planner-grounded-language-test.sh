...
stage=/tmp/mizantra-active-planner-grounded-language
backup=/root/sak-deploy-backups/mizantra-active-planner-grounded-language-$(date +%Y%m%d-%H%M%S)
name=active-planner-memory.service
...
rollback() {
  echo 'Grounded-language deployment failed; restoring Mizantra API.' >&2
