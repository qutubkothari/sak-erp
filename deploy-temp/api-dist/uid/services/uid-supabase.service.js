...
Object.defineProperty(exports, "UidSupabaseService", {
    enumerable: true,
    get: function() {
        return UidSupabaseService;
    }
});
...
let UidSupabaseService = class UidSupabaseService {
    /**
   ...
   */ async generateUID(tenantCode, plantCode, entityType) {
   ...
   */ async generateUIDLegacy(tenantCode, plantCode, entityType) {
        // Get next sequence
        const { data: existing } = await this.supabase.from('uid_registry').select('uid').like('uid', `UID-${tenantCode}-${plantCode}-${entityType}-%`).order('created_at', {
            ascending: false
        }).limit(1);
        let sequence = 1;
    ...
    /**
   * Create UID record
   */ async createUID(req, createDto) {
        const tenantId = req.user.tenantId;
        // Generate UID
        const uid = await this.generateUID(createDto.tenantCode || 'SAIF', createDto.plantCode || 'KOL', createDto.entityType || 'RM');
        const { data, error } = await this.supabase.from('uid_registry').insert([
            {
                ...createDto,
                tenant_id: tenantId,
                uid,
   ...
   */ async findAll(req, filters) {
        ...
        let countQuery = this.supabase.from('uid_registry').select('*', {
            count: 'exact',
            head: true
        ...
        let query = this.supabase.from('uid_registry').select('*').eq('tenant_id', tenantId).order('created_at', {
            ascending: false
        ...
        for (const uid of data || []){
            ...
            if (uid.item_id) {
                const { data: item } = await this.supabase.from('items').select('code, name, category').eq('id', uid.item_id).maybeSingle();
                itemDetails = item;
            } else if (uid.entity_id) {
                const { data: item } = await this.supabase.from('items').select('code, name, category').eq('id', uid.entity_id).maybeSingle();
                itemDetails = item;
            }
            processedData.push({
                ...uid,
   ...
   */ async searchUID(req, uid) {
        const tenantId = req.user.tenantId;
        console.log('=== SEARCH UID ===');
        console.log('UID:', uid);
        // First get the UID record
        const { data: uidData, error: uidError } = await this.supabase.from('uid_registry').select('*').eq('tenant_id', tenantId).eq('uid', uid).single();
        if (uidError) throw new Error(uidError.message);
        console.log('UID Data:', {
            supplier_id: uidData.supplier_id,
            po_id: uidData.purchase_order_id,
            grn_id: uidData.grn_id
        });
        ...
        if (uidData.supplier_id) {
            ...
            const { data: vendorData, error: vendorError } = await this.supabase.from('vendors').select('id, name, code').eq('tenant_id', tenantId).eq('id', uidData.supplier_id).maybeSingle();
            if (vendorError) {
        ...
        if (uidData.purchase_order_id) {
            const { data: poData } = await this.supabase.from('purchase_orders').select('id, po_number').eq('id', uidData.purchase_order_id).single();
            purchaseOrder = poData;
            console.log('PO found:', purchaseOrder?.po_number);
        }
        if (uidData.grn_id) {
            const { data: grnData } = await this.supabase.from('grn').select('id, grn_number').eq('id', uidData.grn_id).single();
            grn = grnData;
   ...
   */ async getUIDWithHierarchy(req, uid) {
        const uidRecord = await this.searchUID(req, uid);
        // Get parent records
        let parents = [];
        if (uidRecord.parent_uids && uidRecord.parent_uids.length > 0) {
            const { data } = await this.supabase.from('uid_registry').select('*').in('uid', uidRecord.parent_uids);
            parents = data || [];
        }
        // Get child records
        let children = [];
        if (uidRecord.child_uids && uidRecord.child_uids.length > 0) {
            const { data } = await this.supabase.from('uid_registry').select('*').in('uid', uidRecord.child_uids);
            children = data || [];
        }
        return {
            ...uidRecord,
   ...
   */ async updateLifecycle(req, uid, stage, location, reference) {
        const tenantId = req.user.tenantId;
        // Get current record
        const { data: current } = await this.supabase.from('uid_registry').select('lifecycle').eq('tenant_id', tenantId).eq('uid', uid).single();
        ...
        const { data, error } = await this.supabase.from('uid_registry').update({
            lifecycle: JSON.stringify(lifecycle),
            location
        }).eq('tenant_id', tenantId).eq('uid', uid).select().single();
        if (error) throw new Error(error.message);
        return data;
    }
    /**
   * Link UIDs (parent-child relationship)
   */ async linkUIDs(req, parentUID, childUID) {
        const tenantId = req.user.tenantId;
        // Update parent
        const { data: parent } = await this.supabase.from('uid_registry').select('child_uids').eq('tenant_id', tenantId).eq('uid', parentUID).single();
        ...
        await this.supabase.from('uid_registry').update({
            child_uids: JSON.stringify(childUIDs)
        }).eq('tenant_id', tenantId).eq('uid', parentUID);
        // Update child
        const { data: child } = await this.supabase.from('uid_registry').select('parent_uids, assembly_level').eq('tenant_id', tenantId).eq('uid', childUID).single();
        if (!child) throw new Error('Child UID not found');
        const parentUIDs = Array.isArray(child.parent_uids) ? child.parent_uids : [];
        ...
        }).eq('tenant_id', tenantId).eq('uid', childUID);
        return {
            message: 'UIDs linked successfully'
        };
    }
    /**
   * Update UID status
   */ async updateStatus(req, uid, status, location) {
        const tenantId = req.user.tenantId;
        const updateData = {
            status
        };
        if (location) updateData.location = location;
        const { data, error } = await this.supabase.from('uid_registry').update(updateData).eq('tenant_id', tenantId).eq('uid', uid).select().single();
        if (error) throw new Error(error.message);
        // Add lifecycle event
        await this.updateLifecycle(req, uid, `STATUS_CHANGE_${status}`, location || data.location, `Status changed to ${status}`);
        return data;
    }
    /**
   ...
   */ async updateQualityStatus(req, uid, qualityStatus, notes) {
        const tenantId = req.user.tenantId;
        ...
        const { data, error } = await this.supabase.from('uid_registry').update(updateFields).eq('tenant_id', tenantId).eq('uid', uid).select().single();
        if (error) throw new Error(error.message);
        // Add lifecycle event
        const location = data?.location || 'N/A';
        await this.updateLifecycle(req, uid, `QC_${normalized}`, location, notes ? `QC marked as ${normalized}: ${notes}` : `QC marked as ${normalized}`);
        return data;
    }
    /**
   * Validate UID format
   ...
   */ async getPurchaseTrail(req, uid) {
        const tenantId = req.user.tenantId;
        // Get UID record
        const { data: uidRecord, error: uidError } = await this.supabase.from('uid_registry').select('*').eq('tenant_id', tenantId).eq('uid', uid).single();
        if (uidError) throw new Error('UID not found');
        // Get item details
        const { data: item } = await this.supabase.from('items').select('code, name').eq('id', uidRecord.entity_id).single();
        // Get supplier details
        let supplier = null;
        if (uidRecord.supplier_id) {
            const { data } = await this.supabase.from('vendors').select('name, contact_person').eq('id', uidRecord.supplier_id).single();
            supplier = data;
        }
        // Get purchase order details
        let purchase_order = null;
        if (uidRecord.purchase_order_id) {
            const { data } = await this.supabase.from('purchase_orders').select('po_number, order_date, total_amount').eq('id', uidRecord.purchase_order_id).single();
            purchase_order = data;
        }
        // Get GRN details
        let grn = null;
        if (uidRecord.grn_id) {
            const { data } = await this.supabase.from('grn').select('grn_number, received_date, received_quantity').eq('id', uidRecord.grn_id).single();
            grn = data;
        }
        // Parse lifecycle
        let lifecycle = [];
        try {
            lifecycle = typeof uidRecord.lifecycle === 'string' ? JSON.parse(uidRecord.lifecycle) : uidRecord.lifecycle || [];
        } catch  {
            lifecycle = [];
        }
        return {
   ...
   */ async getCompleteTrace(req, uid) {
        const tenantId = req.user.tenantId;
        console.log('[UID Trace] Looking for UID:', uid, 'Tenant:', tenantId);
        // 1. Get main UID record
        const { data: uidRecord, error: uidError } = await this.supabase.from('uid_registry').select('*').eq('tenant_id', tenantId).eq('uid', uid).maybeSingle();
        console.log('[UID Trace] Query result:', {
            found: !!uidRecord,
            error: uidError
        });
        if (uidError) {
        ...
        const { data: item, error: itemError } = await this.supabase.from('items').select('id, code, name, description, category, uom').eq('id', uidRecord.item_id).maybeSingle();
        console.log('[UID Trace] Item lookup result:', {
            found: !!item,
            error: itemError
        });
        ...
        uidRecord.item = item;
        // Parse lifecycle
        const lifecycle = Array.isArray(uidRecord.lifecycle) ? uidRecord.lifecycle : typeof uidRecord.lifecycle === 'string' ? JSON.parse(uidRecord.lifecycle) : [];
        // 2. Get vendor details directly from supplier_id
        let vendor = null;
        console.log('[UID Trace] UID Record supplier_id:', uidRecord.supplier_id);
        if (uidRecord.supplier_id) {
            const { data: vendorData, error: vendorError } = await this.supabase.from('vendors').select('code, name, contact_person, email').eq('tenant_id', tenantId).eq('id', uidRecord.supplier_id).maybeSingle();
            console.log('[UID Trace] Vendor lookup result:', {
                vendorData,
                vendorError
            });
            if (vendorData) {
        ...
        if (uidRecord.purchase_order_id) {
            const { data: poData, error: poError } = await this.supabase.from('purchase_orders').select('id, po_number, order_date, total_amount').eq('id', uidRecord.purchase_order_id).maybeSingle();
            console.log('[UID Trace] PO lookup result:', {
                poData,
                poError
            });
            if (poData) {
        ...
        let grn_info = null;
        if (uidRecord.grn_id) {
            const { data: grnData } = await this.supabase.from('grn').select('id, grn_number, grn_date, received_date').eq('id', uidRecord.grn_id).maybeSingle();
            if (grnData) {
                grn_info = {
        ...
        const quality_checkpoints = lifecycle.filter((event)=>event.stage.includes('QC') || event.stage.includes('QUALITY') || event.stage.includes('INSPECTION')).map((event)=>({
                stage: event.stage,
                status: event.reference.includes('PASSED') ? 'PASSED' : event.reference.includes('FAILED') ? 'FAILED' : 'PENDING',
                date: event.timestamp,
                inspector: event.user,
                notes: event.reference
            }));
        // 4. Get components (child UIDs) with their details
        const child_uids = Array.isArray(uidRecord.child_uids) ? uidRecord.child_uids : typeof uidRecord.child_uids === 'string' && uidRecord.child_uids ? JSON.parse(uidRecord.child_uids) : [];
        let components = [];
        if (child_uids.length > 0) {
            const { data: childRecords } = await this.supabase.from('uid_registry').select(`
          uid,
          batch_number,
          received_date,
          qc_status,
        ...
        `).in('uid', child_uids);
            if (childRecords) {
                components = childRecords.map((child)=>{
                ...
                for (const component of components){
                    const { data: componentRecord } = await this.supabase.from('uid_registry').select('lifecycle').eq('uid', component.uid).single();
                    if (componentRecord) {
                        const compLifecycle = Array.isArray(componentRecord.lifecycle) ? componentRecord.lifecycle : typeof componentRecord.lifecycle === 'string' ? JSON.parse(componentRecord.lifecycle) : [];
                        const compGrnRef = compLifecycle.find((e)=>e.reference && e.reference.includes('GRN'));
                        if (compGrnRef) {
                            const { data: compGrn } = await this.supabase.from('grn').select('vendor:vendors(name)').eq('tenant_id', tenantId).eq('grn_number', compGrnRef.reference).single();
        ...
        const parent_uids = Array.isArray(uidRecord.parent_uids) ? uidRecord.parent_uids : typeof uidRecord.parent_uids === 'string' && uidRecord.parent_uids ? JSON.parse(uidRecord.parent_uids) : [];
        let parent_products = [];
        if (parent_uids.length > 0) {
            const { data: parentRecords } = await this.supabase.from('uid_registry').select(`
          uid,
          item:items(code, name)
        `).in('uid', parent_uids);
            if (parentRecords) {
                parent_products = parentRecords.map((parent)=>{
        ...
        let customer = null;
        try {
            const { data: deploymentStatus } = await this.supabase.from('v_uid_deployment_status').select('current_level, current_organization, current_location, current_deployment_date, warranty_expiry_date').eq('tenant_id', tenantId).eq('uid', uidRecord.uid).maybeSingle();
   ...
   */ async getAllUIDs(req, status, entityType, itemId, qualityStatus, search, limit, offset, sortBy, sortOrder, jobOrderId) {
        const tenantId = req.user?.tenantId || req.tenantId;
        console.log('[getAllUIDs] Called with:', {
            tenantId,
            status,
            entityType,
            itemId,
        ...
        let query = this.supabase.from('uid_registry').select(`
        uid, 
        entity_id, 
        entity_type, 
        status, 
      ...
      `, {
        ...
        console.log('[getAllUIDs] Query result:', {
        ...
        if (data && data.length > 0) {
            ...
            const entityIds = [
                ...new Set(data.map((uid)=>uid.entity_id).filter(Boolean))
            ];
            console.log('[getAllUIDs] Unique entity IDs:', entityIds);
            if (entityIds.length > 0) {
                const { data: items } = await this.supabase.from('items').select('id, code, name').in('id', entityIds);
   ...
   */ async getUIDDetails(req, uid) {
        const tenantId = req.user.tenantId;
        console.log(`[getUIDDetails] Fetching UID: ${uid} for tenant: ${tenantId}`);
        // First check if UID exists
        const { data: uidCheck, error: checkError } = await this.supabase.from('uid_registry').select('uid, entity_id, supplier_id, grn_id, tenant_id').eq('uid', uid).eq('tenant_id', tenantId).maybeSingle();
        ...
        const { data: uidRecord, error } = await this.supabase.from('uid_registry').select('*').eq('uid', uid).eq('tenant_id', tenantId).maybeSingle();
        ...
        if (uidRecord.entity_id) {
            const { data: item } = await this.supabase.from('items').select('id, name, code, description').eq('id', uidRecord.entity_id).maybeSingle();
            itemData = item;
        }
        // Fetch vendor details separately if supplier_id exists
        let vendorData = null;
        if (uidRecord.supplier_id) {
            const { data: vendor } = await this.supabase.from('vendors').select('id, name, code').eq('id', uidRecord.supplier_id).maybeSingle();
            vendorData = vendor;
        }
   ...
   */ async updatePartNumber(req, uid, clientPartNumber) {
        const tenantId = req.user?.tenantId || req.tenantId;
        if (!tenantId) {
            throw new Error('Tenant ID is required');
        }
        // Verify UID exists and belongs to tenant
        const { data: existing, error: fetchError } = await this.supabase.from('uid_registry').select('id').eq('uid', uid).eq('tenant_id', tenantId).single();
        if (fetchError || !existing) {
            throw new Error(`UID ${uid} not found`);
        }
        // Update the part number
        const { error: updateError } = await this.supabase.from('uid_registry').update({
            client_part_number: clientPartNumber,
            updated_at: new Date().toISOString()
        }).eq('uid', uid).eq('tenant_id', tenantId);
        if (updateError) {
...
UidSupabaseService = _ts_decorate([
    (0, _common.Injectable)()
], UidSupabaseService);

//# sourceMappingURL=uid-supabase.service.js.map