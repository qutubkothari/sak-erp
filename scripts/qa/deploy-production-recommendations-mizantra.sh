...
stage=/tmp/mizantra-production-recommendations-20260906-v2
release=production-recommendations-20260906
page=apps/web/src/app/dashboard/production/reports/page.tsx
...
for _ in $(seq 1 40); do
  status=$(curl -s -o /dev/null -w '%{http_code}' http://127.0.0.1:3001/dashboard/production/reports || true)
  [[ "$status" == 200 ]] && break
  sleep 2
done
[[ "$status" == 200 ]]
