...
import { resolveVendorContactSalutation, WorldClassPoPdfService } from '../services/world-class-po-pdf.service';
...
export function isFinalPurchaseOrderDocumentStatus(status: unknown): boolean {
  return ['APPROVED', 'CLOSED'].includes(String(status || '').trim().toUpperCase());
}
...
export class PurchaseOrdersController {
  ...
  @Get(':id/change-control')
  async getChangeControl(@Request() req: any, @Param('id') id: string) {
    return this.poService.getChangeControl(req.user.tenantId, id, req.user);
  }
  ...
  async generateWorldClassPdf(@Request() req: any, @Param('id') id: string, @Res() res: Response) {
    try {
      ...
      // An approved PO is a commercial document. Its stored total is the
      // amount accepted from the quotation and must take precedence over any
      // attempt by the PDF renderer to rebuild values from rounded line rates.
      const storedDocumentTotal = safeNumber(po.grand_total ?? po.total_amount ?? 0);
      const storedDocumentTax = safeNumber(po.tax_amount ?? 0);
      const hasStoredDocumentTotal = storedDocumentTotal > 0;
      ...
      try {
        ...
        // Only reconstruct a genuinely legacy/missing header. Recalculating an
        // approved PO from rates can differ from its quotation because line
        // amounts and GST were already rounded and saved at approval time.
        if (!hasStoredDocumentTotal) {
      ...
      // Restore the approved header components after freight terms are known.
      // This makes the PDF use the same quotation-approved document value as
      // the PO screen, including the saved GST and any rounding difference.
      if (hasStoredDocumentTotal) {
        const freightAmount = safeNumber((pdfData as any).freightAmount);
        ...
        const itemInclusiveAmount = Math.max(0, storedDocumentTotal - freightAmount - freightGstAmount - customsDuty - additionalExpenses);
        const taxableAmount = Math.max(0, itemInclusiveAmount - storedDocumentTax);
        (pdfData as any).subtotal = itemInclusiveAmount;
        (pdfData as any).taxableAmount = taxableAmount;
        (pdfData as any).cgstTotal = 0;
        (pdfData as any).sgstTotal = 0;
        (pdfData as any).igstTotal = storedDocumentTax;
        (pdfData as any).grandTotal = storedDocumentTotal;
      }
      ...
      try {
        const exactGrandTotal = hasStoredDocumentTotal
          ? safeNumber((pdfData as any).taxableAmount || (pdfData as any).subtotal) +
            safeNumber((pdfData as any).cgstTotal) +
            safeNumber((pdfData as any).sgstTotal) +
            safeNumber((pdfData as any).igstTotal) +
            safeNumber((pdfData as any).freightAmount) +
            safeNumber((pdfData as any).customsDuty) +
            safeNumber((pdfData as any).additionalExpenses)
          : safeNumber((pdfData as any).grandTotal);
        const roundedGrandTotal = Math.round(exactGrandTotal);
        const targetGrandTotal = hasStoredDocumentTotal ? storedDocumentTotal : roundedGrandTotal;
        const roundOff = Number((targetGrandTotal - exactGrandTotal).toFixed(2));
        (pdfData as any).roundOff = Math.abs(roundOff) >= 0.01 ? roundOff : 0;
        (pdfData as any).grandTotal = targetGrandTotal;
      } catch {}
      ...
      // A PO remains an approved commercial document after its receipt flow
      // closes it. Do not regress a fully received PO to a DRAFT PDF merely
      // because its operational status advanced from APPROVED to CLOSED.
      const isDraftDocument = !isFinalPurchaseOrderDocumentStatus(po.status);
      if (isDraftDocument) {
