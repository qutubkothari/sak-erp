...
async function putJsonViaApiProxy<T = any>(
  endpoint: string,
  payload: any,
): Promise<T> {
  const token =
...
export default function SivPage() {
  ...
  >([]);
  const [storeIssueEmployees, setStoreIssueEmployees] = useState<
    StoreIssueEmployee[]
  >([]);
  const [filterAssignedTo, setFilterAssignedTo] = useState<string>("");
  const [activeSivView, setActiveSivView] = useState<"open" | "history">(
    "open",
  );
  const [selectedMaterialJobId, setSelectedMaterialJobId] = useState<
    string | null
  >(null);
  ...
  const [selectedLineIdsByJob, setSelectedLineIdsByJob] = useState<
    Record<string, string[]>
  >({});
  const [uidInputByLine, setUidInputByLine] = useState<Record<string, string>>(
    {},
  );
  const [scannedUidsByLine, setScannedUidsByLine] = useState<
    Record<string, string[]>
  >({});
  ...
  const [scanStatusByJob, setScanStatusByJob] = useState<
    Record<string, { type: "ok" | "error"; message: string }>
  >({});
  const [expandedLineIds, setExpandedLineIds] = useState<
    Record<string, boolean>
  >({});
  const [lineBomData, setLineBomData] = useState<
    Record<string, BomComponent[]>
  >({});
  ...
  const [bomSelectedByLine, setBomSelectedByLine] = useState<
    Record<string, Record<number, boolean>>
  >({});
  const [focusJoNumber, setFocusJoNumber] = useState<string>("");
  const [focusHandled, setFocusHandled] = useState(false);
  const [initialDataLoaded, setInitialDataLoaded] = useState(false);
  const [expandedHistoryJoIds, setExpandedHistoryJoIds] = useState<
    Record<string, boolean>
  >({});
  ...
  const [showAllOpenRequests, setShowAllOpenRequests] = useState(false);
  ...
  const openUidPicker = useCallback(
    async (
      lineId: string,
      itemId: string,
      itemCode: string,
      ...
      try {
        ...
        const r1 = await apiClient
          .get<any>(
            `/uid?itemId=${itemId}&status=GENERATED,IN_STOCK&limit=10000`,
          )
          .catch(() => []);
  ...
  const loadAll = useCallback(async (options?: { silent?: boolean }) => {
    ...
    try {
      const [reqs, sivHist] = await Promise.all([
        apiClient.get<MaterialReq[]>(
          "/job-orders/store/material-requisitions/open",
        ),
        apiClient.get<SivHistoryRow[]>(
          "/job-orders/store/material-requisitions/history",
        ),
      ]);
    ...
    try {
      ...
      const mapped = (Array.isArray(rows) ? rows : [])
        .map((row: any) => ({
          ...
          uid_strategy: String(
            (row?.uid_strategy ?? row?.uidStrategy) || "",
          ).trim(),
        }))
        .filter((row) => row.id);
    ...
    try {
      ...
      const mapped = (Array.isArray(data) ? data : [])
        .map((u: any) => ({
          id: String(u?.id || "").trim(),
          displayName: String(
            u?.displayName || u?.employee_name || u?.name || u?.email || "",
          ).trim(),
          employeeCode: String(
            u?.employee_code || u?.employeeCode || "",
          ).trim(),
        }))
  ...
  const loadStoreIssueEmployees = useCallback(async () => {
    try {
      const data = await apiClient.get<StoreIssueEmployee[]>(
        "/job-orders/store/issue-employees",
      );
  ...
  const isSubassemblyItem = useCallback(
    (itemId?: string) => {
  ...
  const itemRequiresUid = useCallback(
    (itemId?: string) => {
      const normalizedId = String(itemId || "").trim();
      if (!normalizedId) return false;
      ...
      const strategy = String(item.uid_strategy || "")
        .trim()
        .toUpperCase();
      if (strategy) {
  ...
  const fetchLineBom = useCallback(
  ...
  useEffect(() => {
    if (!initialDataLoaded || focusHandled || (!focusJobId && !focusJoNumber))
      return;
    ...
    const matched = materialRequests.find(
      (row) =>
  ...
  useEffect(() => {
    ...
    const printableLineIds = req.materialLines
      ...
      .filter(
        (line) =>
          Number(line.available_quantity || 0) + 1e-9 >=
  ...
  const formatApiErrorMessage = useCallback((error: any) => {
    ...
    if (directMessage.startsWith("{") && directMessage.endsWith("}")) {
      try {
        ...
        return String(
          parsed?.errorMessage || parsed?.message || directMessage,
        ).trim();
      } catch {
  ...
  const addUidToLineCart = useCallback(
    (jobId: string, lineId: string, uidRaw: string) => {
  ...
  const addScannedUid = useCallback(
  ...
  const buildManualIssueDraftLine = useCallback((): ManualIssueLine | null => {
    ...
    if (
      ...
      manualIssueUids.length !== issueQuantity
    ) {
  ...
  const handleManualIssue = useCallback(async () => {
    if (!manualIssuedToEmployeeId) {
      await confirmDialog({
        ...
        message:
          "Select the registered employee receiving this material before creating the manual SIV.",
        confirmLabel: "OK",
        cancelLabel: "Close",
        variant: "warning",
      });
    ...
    try {
      ...
      for (const line of linesToSubmit) {
        const result = await apiClient.post(
          "/job-orders/store/material-requisitions/manual-issue",
          {
            itemId: line.itemId,
    ...
    buildManualIssueDraftLine,
    loadAll,
    manualIssueLines,
    manualIssuedToEmployeeId,
    resetManualIssueDraft,
  ]);

  const scanUidForJob = useCallback(
    async (jobId: string) => {
      ...
      if (
        !req ||
        !Array.isArray(req.materialLines) ||
        req.materialLines.length === 0
      ) {
        setScanStatusByJob((prev) => ({
          ...prev,
          [jobId]: {
            type: "error",
            message: "No material lines found for this job order",
          },
        }));
        return;
      }
      ...
      if (allScanned.has(raw)) {
        setScanStatusByJob((prev) => ({
          ...prev,
          [jobId]: { type: "error", message: `UID already scanned: ${raw}` },
        }));
      ...
      try {
        const trace: any = await apiClient.get(
          `/uid/trace/${encodeURIComponent(raw)}`,
        );
        ...
        const target =
          candidates.find((c) => c.scanned < c.pending) || candidates[0];
        if (target.scanned >= target.pending) {
          setScanStatusByJob((prev) => ({
            ...prev,
            [jobId]: {
              type: "error",
              message: `Scanned UID exceeds pending quantity for ${itemCode || "this item"}`,
            },
          }));
          return;
        }
        ...
        setScanStatusByJob((prev) => ({
          ...
          [jobId]: {
            type: "ok",
            message: `Added ${raw} to ${itemCode || "material line"}`,
          },
        }));
        setScanInputByJob((prev) => ({ ...prev, [jobId]: "" }));
      } catch (err: any) {
  ...
  const approveSivHistoryRow = useCallback(
    async (movementId: string) => {
      ...
      try {
        await apiClient.put(
          `/job-orders/store/material-requisitions/history/${movementId}/approve`,
          {},
        );
        await loadAll();
        await confirmDialog({
  ...
  const approveSivHistoryRows = useCallback(
    async (movementIds: string[], label = "selected SIV entries") => {
      const ids = Array.from(
        new Set(
          movementIds.map((id) => String(id || "").trim()).filter(Boolean),
        ),
      );
      if (ids.length === 0) {
        await confirmDialog({
      ...
      try {
        await putJsonViaApiProxy(
          "/job-orders/store/material-requisitions/history/approve-bulk",
          {
            movementIds: ids,
          },
        );
  ...
  const deleteSivHistoryRow = useCallback(
    async (movementId: string) => {
      const confirmedDel = await confirmDialog({
        title: "Reverse SIV Entry",
        message:
          "Reverse this SIV goods issue? Stock and UID movement will be reversed.",
      ...
      try {
        await apiClient.delete(
          `/job-orders/store/material-requisitions/history/${movementId}`,
        );
        await loadAll();
        await confirmDialog({
          title: "SIV Reversed",
          message:
            "SIV goods issue has been reversed and stock/UID movement has been adjusted.",
          confirmLabel: "OK",
          cancelLabel: "Close",
        });
      } catch (err: any) {
      ...
      try {
        ...
        for (const lineId of selectedLineIds) {
          ...
          try {
            await apiClient.post(
              `/job-orders/store/material-requisitions/${jobId}/issue-line`,
              {
                materialId: lineId,
        ...
        if (failures.length === 0) {
          await confirmDialog({
            ...
            message:
              "Selected material lines were issued successfully. Pending approvals can be reviewed in History.",
            confirmLabel: "OK",
            cancelLabel: "Close",
          });
        } else {
          ...
          await confirmDialog({
            title:
              successLineIds.length > 0
                ? "SIV Partially Issued"
                : "SIV Issue Failed",
            message:
  ...
  const printSiv = useCallback(
    async (jobId: string) => {
      ...
      if (!req) return;
      const selectedLineIds = selectedLineIdsByJob[jobId] || [];
      const lines = (req.materialLines || []).filter((ln) =>
        selectedLineIds.includes(ln.id),
      );
      if (!lines.length) {
        await confirmDialog({
      ...
      const now = `${_now1.getDate().toString().padStart(2, "0")}/${(_now1.getMonth() + 1).toString().padStart(2, "0")}/${_now1.getFullYear()} ${_now1.getHours().toString().padStart(2, "0")}:${_now1.getMinutes().toString().padStart(2, "0")}`;
      ...
      const linesHtml = lines
        .map((ln) => {
          ...
          const sessionUids: string[] = (scannedUidsByLine[ln.id] || []).filter(
            Boolean,
          );
          ...
          const historyUids: string[] = sivHistory
            .filter(
              (h) => h.item_code === ln.item_code && h.job_order_id === jobId,
            )
            .map((h) => h.uid)
          ...
          const uidsCell =
            allUids.length > 0
              ? allUids
                  .map(
                    (uid) =>
                      `<div style="font-family:monospace; font-size:11px; font-weight:600; margin-bottom:2px;">${escapeHtml(uid)}</div>`,
                  )
                  .join("")
  ...
  const printSivHistory = useCallback(
    async (joId: string, rows: SivHistoryRow[]) => {
      ...
      const now = `${_now2.getDate().toString().padStart(2, "0")}/${(_now2.getMonth() + 1).toString().padStart(2, "0")}/${_now2.getFullYear()} ${_now2.getHours().toString().padStart(2, "0")}:${_now2.getMinutes().toString().padStart(2, "0")}`;

      // Group lines by item to consolidate UIDs per item
      const itemMap = new Map<
        string,
        { code: string; name: string; qty: number; uids: string[] }
      >();
      ...
      const linesHtml = Array.from(itemMap.values())
        .map((item, idx) => {
          const uidsCell =
            item.uids.length > 0
              ? item.uids
                  .map(
                    (u) =>
                      `<div style="font-family:monospace;font-size:11px;font-weight:600;margin-bottom:2px;">${escapeHtml(u)}</div>`,
                  )
                  .join("")
              : '<span style="color:#999;font-size:10px;">-</span>';
          return `
      ...
      const issuedDateFmt = issuedDate
        ...
        : "-";
      const issuedToEmployees = Array.from(
        new Set(
          rows
            .map((row) =>
              [row.issued_to_employee_code, row.issued_to_employee_name]
                .filter(Boolean)
                .join(" - "),
            )
            .filter(Boolean),
      ...
      const issuedToLabel =
        issuedToEmployees.join(", ") || "Not recorded (legacy SIV)";
  ...
  const assignMaterialRequest = async (reqId: string, userId: string) => {
    try {
      await apiClient.put(
        `/job-orders/store/material-requisitions/${reqId}/assign`,
        {
  ...
  const pendingLineCount = openMaterialReqs.reduce(
    (sum, req) => sum + Number(req.pendingLines || 0),
    0,
  );
  ...
  const approvedHistoryCount = sivHistory.filter(
    (row) => row.approved_by,
  ).length;
  const awaitingApprovalCount = sivHistory.length - approvedHistoryCount;
  const pendingHistoryRowIds = sivHistory
    .filter((row) => !row.approved_by)
    .map((row) => row.id);
  const selectedPendingHistoryRowIds = pendingHistoryRowIds.filter((id) =>
    selectedHistoryRowIds.includes(id),
  );
  const blockedLineCount = openMaterialReqs.reduce(
    (sum, req) =>
      ...
      (req.materialLines || []).filter(
        (line) =>
          Number(line.pending_quantity || 0) > 0 &&
          Number(line.available_quantity || 0) <= 0 &&
          !itemRequiresUid(line.item_id),
      ).length,
    0,
  );
  const focusedOpenRequest = openMaterialReqs.find(
    (row) =>
      (focusJobId && row.id === focusJobId) ||
      (focusJoNumber && row.job_order_number === focusJoNumber),
  );
  const focusedIssueMode = Boolean(
    focusedOpenRequest && (focusJobId || focusJoNumber),
  );
  const displayedOpenRequests: MaterialReq[] =
    focusedIssueMode && !showAllOpenRequests && focusedOpenRequest
      ? [focusedOpenRequest]
      : openMaterialReqs;
  const getLineStockStatus = (line: MaterialLine) => {
    ...
    if (available >= pending)
      return { label: "Ready to Post", tone: "success" as const };
    if (available > 0)
      return { label: "Partial Stock", tone: "warning" as const };
    if (itemRequiresUid(line.item_id))
      return { label: "UID Required", tone: "info" as const };
    ...
    <div className="min-h-screen bg-[#FAF9F6] px-4 py-4 text-[#2F241D] lg:px-6">
      <div className="flex min-h-[calc(100vh-2rem)] flex-col gap-4">
        <ErpPageHeader
          ...
          description="Select the job, issue the ready material, and let Mizantra keep the stock and approval trail."
          actions={
            <ErpButton
              variant="secondary"
              onClick={() => {
        ...
        <ErpMetricStrip
          ...
          metrics={[
            ...
            {
              label: "Blocked Lines",
              value: blockedLineCount,
              tone: blockedLineCount > 0 ? "danger" : "success",
            },
            { label: "SIV History", value: sivHistory.length },
            {
              label: "Awaiting Approval",
              value: awaitingApprovalCount,
              tone: awaitingApprovalCount > 0 ? "warning" : "success",
            },
          ]}
        />
        ...
        <div className="grid gap-3 lg:grid-cols-4">
          ...
          <div className="rounded-md border border-[#E8DCC4] bg-white p-3">
            ...
            <div className="text-xs text-[#7A6555]">
              Open requests are tied to production demand
            </div>
          </div>
          <div className="rounded-md border border-[#E8DCC4] bg-white p-3">
            <div className="text-xs font-semibold uppercase text-[#7A6555]">
            ...
            <div className="text-xs text-[#7A6555]">
              Approval remains separate from stock movement
            </div>
          </div>
          <div className="rounded-md border border-[#E8DCC4] bg-white p-3">
            <div className="text-xs font-semibold uppercase text-[#7A6555]">
            ...
            <div className="mt-1 flex items-center gap-2 text-sm font-semibold text-[#4A3426]">
              <AlertTriangle
                className={`h-4 w-4 ${blockedLineCount > 0 ? "text-red-700" : "text-emerald-700"}`}
              />
              {blockedLineCount} blocked line{blockedLineCount === 1 ? "" : "s"}
            </div>
            <div className="text-xs text-[#7A6555]">
              Stock shortage and UID gaps are stopped before issue
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-col gap-3 border-b border-[#E8DCC4] px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
            ...
            {activeSivView === "open" && (
              <div className="flex min-w-0 flex-col gap-1 sm:flex-row sm:items-center">
                <label className="text-xs font-semibold uppercase text-[#7A6555]">
                  Assigned
                </label>
                <select
                ...
                >
                  ...
                  <option value={(currentUser as any)?.id}>
                    Assigned to Me
                  </option>
            ...
            {activeSivView === "history" &&
              canApprove &&
              pendingHistoryRowIds.length > 0 && (
                <button
                  type="button"
                  onClick={() =>
                    void approveSivHistoryRows(
                      selectedPendingHistoryRowIds.length > 0
                        ? selectedPendingHistoryRowIds
                        : pendingHistoryRowIds,
                      selectedPendingHistoryRowIds.length > 0
                        ? "selected SIV entries"
                        : "all pending SIV entries",
                    )
                  }
        ...
        {activeSivView === "open" && (
          <div className="min-h-0 flex-1 space-y-4">
              ...
              <div className="flex flex-col gap-3 rounded-xl border border-blue-200 bg-blue-50 p-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <div className="text-xs font-bold uppercase tracking-wide text-blue-800">
                    Job order material issue
                  </div>
                  <div className="mt-1 font-bold text-blue-950">
                    {focusedOpenRequest?.job_order_number} ·{" "}
                    {focusedOpenRequest?.item_code} -{" "}
                    {focusedOpenRequest?.item_name}
                  </div>
                  <p className="mt-1 text-sm text-blue-900">
                    Select only the material ready in stock, enter the quantity,
                    then issue it.
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => setShowAllOpenRequests((value) => !value)}
                  className="shrink-0 rounded-lg border border-blue-300 bg-white px-3 py-2 text-sm font-semibold text-blue-900 hover:bg-blue-100"
                >
                  {showAllOpenRequests
                    ? "Back to this job"
                    : `Show all open jobs (${openMaterialReqs.length})`}
                </button>
              </div>
            ) : null}
            ...
            <div className="rounded-md border border-[#E8DCC4] bg-white">
              <button
                type="button"
                onClick={() => setShowManualIssue((value) => !value)}
                className="flex w-full items-center justify-between px-4 py-3 text-left text-sm font-semibold text-[#4A3426] hover:bg-[#FFFCF5]"
              >
                <span>Manual issue (not linked to a production job)</span>
                <span className="text-xs font-medium text-[#7A6555]">
                  {showManualIssue ? "Hide" : "Open only when required"}
                </span>
              </button>
            </div>
            ...
            {showManualIssue ? (
              <div className="rounded-md border border-[#E8DCC4] bg-white p-4">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                  <div>
                    <h2 className="text-base font-bold text-[#4A3426]">
                      Manual Issue
                    </h2>
                    <p className="mt-0.5 text-xs text-[#7A6555]">
                      Issue material directly when no production job order is
                      available.
                    </p>
                  </div>
                </div>
                <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
                  ...
                  <div>
                    ...
                    />
                    <p className="mt-1 text-xs text-[#7A6555]">
                      Required for physical custody and SIV audit tracking.
                    </p>
                  </div>
                  <div>
                    <label className="mb-1 block text-xs font-semibold uppercase text-[#7A6555]">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          onChange={(e) =>
                            setManualIssueUidInput(e.target.value)
                          }
                          onKeyDown={(e) => {
                            if (e.key === "Enter") {
                              e.preventDefault();
                      ...
                      {manualIssueUids.length > 0 ? (
                        <div className="mt-2 text-xs text-gray-700">
                          <span className="font-medium">
                            {manualIssueUids.length} UID(s) selected:
                          </span>{" "}
                          {manualIssueUids.join(", ")}{" "}
                          <button
                            type="button"
                        ...
                        <div className="mt-2 text-xs text-gray-500">
                          Tip: Scan UIDs manually or click "Pick UID" to select
                  ...
                  <div className="mt-4 rounded-md border border-[#E8DCC4] bg-[#FFFCF5] p-3">
                    <div className="flex items-center justify-between gap-4">
                      <div>
                        ...
                        <p className="mt-0.5 text-xs text-[#7A6555]">
                          Add multiple items before creating the issue entries.
                        </p>
                      </div>
                      <ErpButton
                        type="button"
                    ...
                    <div className="mt-3 space-y-2">
                      {manualIssueLines.map((line, index) => (
                        <div
                          key={line.id}
                          className="flex flex-col gap-2 rounded-md border border-[#E8DCC4] bg-white p-3 md:flex-row md:items-start md:justify-between"
                        >
                          <div>
                            <div className="font-medium text-[#36454F]">
                              {index + 1}.{" "}
                  ...
                  <div className="mb-3 flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0">
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-2">
                        <span className="text-xs font-semibold uppercase text-[#7A6555]">
                          Assigned
                        </span>
                        <select
                          value={req.assigned_to || ""}
                          onChange={(e) =>
                            assignMaterialRequest(req.id, e.target.value)
                          }
                      ...
                      <div className="mt-2 flex flex-wrap gap-2 text-xs">
                        <span className="rounded-full border border-[#E8DCC4] bg-[#FAF9F6] px-2 py-1 text-[#5E4635]">
                          Required {req.requiredQuantity || 0}
                        </span>
                        <span className="rounded-full border border-emerald-200 bg-emerald-50 px-2 py-1 text-emerald-800">
                          Issued {req.issuedQuantity || 0}
                        </span>
                        <span className="rounded-full border border-amber-200 bg-amber-50 px-2 py-1 text-amber-800">
                          Pending {req.pendingQuantity || 0}
                        </span>
                        {(() => {
                          const readyCount = (req.materialLines || []).filter(
                            (line) =>
                              Number(line.pending_quantity || 0) > 0 &&
                              Number(line.available_quantity || 0) + 1e-9 >=
                                Number(line.pending_quantity || 0),
                      ...
                      <div className="mt-3 grid gap-2 text-xs md:grid-cols-4">
                        <div className="rounded-md border border-[#E8DCC4] bg-[#FFFCF5] px-3 py-2">
                          <div className="font-semibold uppercase text-[#7A6555]">
                            Reference
                          </div>
                        ...
                        <div className="rounded-md border border-[#E8DCC4] bg-[#FFFCF5] px-3 py-2">
                          ...
                          <div className="mt-1 font-medium text-[#4A3426]">
                            {(() => {
                              const blocked = (req.materialLines || []).filter(
                                (line) => {
                    ...
                    <div className="flex flex-wrap gap-2">
                      <ErpButton
                        onClick={() =>
                          setSelectedMaterialJobId(expanded ? null : req.id)
                        }
                        size="sm"
                      >
                        ...
                        <table className="min-w-full divide-y divide-[#E8DCC4]">
                                ...
                                <div className="flex items-center gap-2">
                                  {(() => {
                                    const selectable = (req.materialLines || [])
                                      .filter(
                                        (l) =>
                                          Number(l.pending_quantity || 0) > 0,
                                      )
                                      .filter(
                                        (l) =>
                                          Number(l.available_quantity || 0) > 0,
                                      )
                                      .map((l) => l.id);
                                    const allSelected =
                                      selectable.length > 0 &&
                                      selectable.every((id) =>
                                        selectedLineIds.includes(id),
                                      );
                                    return (
                                      <input
                                        type="checkbox"
                          ...
                          <tbody className="bg-white divide-y divide-gray-200">
                            {req.materialLines.map((ln) => {
                              const isSelected = selectedLineIds.includes(
                                ln.id,
                              );
                              const scannedUids =
                                scannedUidsByLine[ln.id] || [];
                              const pending = Number(ln.pending_quantity || 0);
                              const available = Number(
                                ln.available_quantity || 0,
                              );
                              const isSubassembly = isSubassemblyItem(
                                ln.item_id,
                              );
                              ...
                              const needsBom =
                                ...
                                available < pending;
                              const disableRow =
                                pending <= 0 || (hasNoStock && !requiresUid);
                                  ...
                                  <tr className="hover:bg-gray-50">
                                    <td className="px-4 py-3">
                                      <input
                                        ...
                                        checked={isSelected}
                                        onChange={() =>
                                          toggleLineSelection(req.id, ln.id)
                                        }
                                        disabled={disableRow}
                                    ...
                                    <td className="px-4 py-3 text-gray-700">
                                      {ln.required_quantity || 0}
                                    </td>
                                    <td className="px-4 py-3 text-gray-700">
                                      {ln.issued_quantity || 0}
                                    </td>
                                    <td className="px-4 py-3 text-[#8B6F47] font-medium">
                                      {pending}
                                    </td>
                                    <td className="px-4 py-3">
                                      <ErpStatusBadge
                                    ...
                                    <td className="px-4 py-3 text-sm text-gray-700">
                                      <div className="font-medium text-[#4A3426]">
                                        261
                                      </div>
                                        ...
                                        <input
                                          ...
                                          value={
                                            scannedUids.length > 0
                                              ? String(scannedUids.length)
                                              : issueQtyByLine[ln.id] || ""
                                          }
                                          onChange={(e) =>
                                            setIssueQtyByLine((prev) => ({
                                              ...prev,
                                    ...
                                    <tr className="bg-sky-50/70">
                                      ...
                                      >
                                        {lineBomLoading[ln.id] ? (
                                          ...
                                          lineBomData[ln.id].length === 0 ? (
                                          <div className="text-sm text-gray-500 italic py-2">
                                            No BOM found for this item. Select
                                            and issue this line directly.
                                          </div>
                                        ) : (
                                          <>
                                            ...
                                            <p className="mt-2 text-xs text-sky-600 italic">
                                              Select the parent line above and
                                              click Issue to pull {ln.item_code}{" "}
                                              from stock.
                                            </p>
                                          </>
                                        )}
        ...
        {activeSivView === "history" && (
          <div className="min-h-0 flex-1 space-y-4">
            ...
            {(() => {
              ...
              const getMovementTs = (row: SivHistoryRow) =>
                row.movement_date
                  ? new Date(row.movement_date).getTime() || 0
                  : 0;
              return Array.from(groupMap.entries())
                .map(([joId, rows]) => {
                  const sortedRows = [...rows].sort(
                    (a, b) => getMovementTs(b) - getMovementTs(a),
                  );
                ...
                .map(([joId, rows, latestDate]) => {
                  ...
                  const pendingRowIds = rows
                    ...
                    .map((r) => r.id);
                  const selectedPendingRowIds = pendingRowIds.filter((id) =>
                    selectedHistoryRowIds.includes(id),
                  );
                  const allPendingSelected =
                    pendingRowIds.length > 0 &&
                    pendingRowIds.every((id) =>
                      selectedHistoryRowIds.includes(id),
                    );
                  return (
                    <div
                    ...
                    >
                      ...
                      <div className="mb-3 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                        <div className="flex min-w-0 items-center gap-3">
                          <input
                            ...
                            onChange={(e) => {
                              if (e.target.checked) {
                                setSelectedHistoryRowIds((prev) => [
                                  ...new Set([...prev, ...pendingRowIds]),
                                ]);
                              } else {
                                setSelectedHistoryRowIds((prev) =>
                                  prev.filter(
                                    (id) => !pendingRowIds.includes(id),
                                  ),
                                );
                              }
                            }}
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="mt-0.5 text-sm text-[#7A6555]">
                              ...
                              Approved {approvedCount}/{rows.length}
                              {latestDate
                                ? ` · Last issued ${new Date(latestDate).toLocaleDateString()}`
                                : ""}
                            </p>
                            <div className="mt-2 grid gap-2 text-xs sm:grid-cols-5">
                              ...
                              <div className="rounded-md border border-[#E8DCC4] bg-[#FFFCF5] px-3 py-2">
                                ...
                                <div className="mt-1 font-medium text-[#4A3426]">
                                  {[
                                    rows[0]?.issued_to_employee_code,
                                    rows[0]?.issued_to_employee_name,
                                  ]
                                    .filter(Boolean)
                                    .join(" - ") || "Legacy SIV — not recorded"}
                                </div>
                              </div>
                              <div className="rounded-md border border-[#E8DCC4] bg-[#FFFCF5] px-3 py-2">
                                <div className="font-semibold uppercase text-[#7A6555]">
                              ...
                              <div className="rounded-md border border-[#E8DCC4] bg-[#FFFCF5] px-3 py-2">
                                ...
                                <div className="mt-1 font-medium text-[#4A3426]">
                                  {approvedCount === rows.length
                                    ? "Complete"
                                    : `${rows.length - approvedCount} pending`}
                                </div>
                              </div>
                            </div>
                          </div>
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          {pendingRowIds.length > 0 &&
                            selectedPendingRowIds.length > 0 && (
                          ...
                          {canApprove && pendingRowIds.length > 0 && (
                            ...
                            >
                              ...
                              Approve{" "}
                              {selectedPendingRowIds.length > 0
                                ? selectedPendingRowIds.length
                                : `${pendingRowIds.length} Pending`}
                            </button>
                          )}
                          <button
                          ...
                          <table className="min-w-full divide-y divide-[#E8DCC4]">
                            <thead className="bg-[#F5EFE3]">
                              <tr>
                                ...
                                <th className="px-3 py-2 text-right text-xs font-semibold uppercase text-[#5E4635]">
                                  Actions
                                </th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-[#E8DCC4] bg-white">
                              {rows.map((row) => {
                                const isChecked =
                                  selectedHistoryRowIds.includes(row.id);
                                const isApproved = Boolean(row.approved_by);
                                return (
                                  <tr
                                    key={row.id}
                                    className="hover:bg-[#FFFCF5]"
                                  >
                                    ...
                                    <td className="px-3 py-2 text-sm text-gray-700">
                                      <div className="font-medium text-[#4A3426]">
                                        {row.issued_to_employee_name ||
                                          "Not recorded"}
                                      </div>
                                      {row.issued_to_employee_code && (
                                        <div className="text-xs text-gray-500">
                                          {row.issued_to_employee_code}
                                        </div>
                                      )}
                                    ...
                                    <td className="px-3 py-2 text-sm">
                                      <ErpStatusBadge
                                        ...
                                        tone={
                                          row.approved_by ? "success" : "info"
                                        }
                                        title={
                                          row.approved_by
                                            ? `Approved by ${row.approved_by}`
                                            : "Issued and awaiting approval"
                                        }
                                      />
                                    </td>
                                    <td className="px-3 py-2">
                                      <div className="flex justify-end gap-2">
                                        {canApprove && !row.approved_by && (
                                          <button
                                            type="button"
                                            onClick={() =>
                                              void approveSivHistoryRow(row.id)
                                            }
                                        ...
                                        {canDelete && !row.approved_by && (
                                          <button
                                            type="button"
                                            onClick={() =>
                                              void deleteSivHistoryRow(row.id)
                                            }
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg mx-4 flex flex-col max-h-[80vh]">
            ...
            <div className="sticky top-0 z-10 flex items-center justify-between gap-4 px-5 py-4 border-b bg-white">
              <div>
                <h2 className="text-base font-semibold text-gray-900">
                  Pick UIDs - {uidPickerItemCode}
                </h2>
                <p className="text-xs text-gray-500 mt-0.5">
                  Select up to {uidPickerMaxQty} UID
                  {uidPickerMaxQty !== 1 ? "s" : ""} from available stock
                </p>
              </div>
              <div className="flex items-center gap-3">
                <button
                ...
                >
                  Confirm{" "}
                  {uidPickerSelected.length > 0
                    ? `(${uidPickerSelected.length})`
                    : ""}
                </button>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 px-5 py-2 border-b bg-gray-50">
              <span className="text-sm text-gray-700">
                {uidPickerSelected.length} / {uidPickerMaxQty} selected
              </span>
              <button
                onClick={() =>
                  setUidPickerSelected(
                    uidPickerUids
                      .slice(0, uidPickerMaxQty)
            ...
            <div className="overflow-y-auto flex-1 px-5 py-3">
              {uidPickerLoading ? (
                ...
                <div className="text-center text-gray-400 py-10">
                  No available UIDs found for this item.
                </div>
              ) : (
                    ...
                    {uidPickerUids.map((u: any) => {
                      ...
                      return (
                        <tr
                          ...
                          onClick={() => {
                            if (checked) {
                              setUidPickerSelected((p) =>
                                p.filter((x) => x !== uid),
                              );
                            } else if (
                              uidPickerSelected.length < uidPickerMaxQty
                            ) {
                        ...
                        >
                          ...
                          <td className="py-2 pr-6 font-mono text-xs text-gray-800">
                            {uid}
                          </td>
                          ...
                          <td className="py-2 text-gray-500">
                            {u.received_date
                              ? new Date(u.received_date).toLocaleDateString()
                              : "-"}
                          </td>
                        </tr>
                      );
