...
stage=/tmp/mizantra-mrp-date-effective-supply-20260830
backup=/root/sak-deploy-backups/mizantra-mrp-date-effective-supply-$(date +%Y%m%d-%H%M%S)
...
if [ "$verified" != true ]; then
  ...
  pm2 restart sak-api-test sak-web-test
  echo 'MRP date-effective supply deployment failed health checks; previous test runtime restored.' >&2
  exit 1
fi

echo "Mizantra TEST date-effective MRP supply deployed and verified. Backup: $backup"
