...
let PurchaseOrdersService = class PurchaseOrdersService {
    async create(tenantId, userId, data) {
        ...
        if (data.prId && data.vendorId) {
            const { data: existingPOs, error: checkError } = await this.supabase.from('purchase_orders').select('id, po_number, vendor_id').eq('tenant_id', tenantId).eq('pr_id', data.prId).eq('vendor_id', data.vendorId).limit(1);
            if (checkError) {
                console.error('Duplicate check error:', checkError);
                throw new _common.BadRequestException(checkError.message);
            }
            if (existingPOs && existingPOs.length > 0) {
                // Fetch vendor name separately to avoid relation issues
                const { data: vendorData } = await this.supabase.from('vendors').select('name').eq('id', existingPOs[0].vendor_id).single();
                const vendorName = vendorData?.name || 'this vendor';
                throw new _common.BadRequestException(`A Purchase Order (${existingPOs[0].po_number}) already exists for this PR and ${vendorName}. Cannot create duplicate PO.`);
            }
        }
        // Generate PO number
        ...
        if (data.items && data.items.length > 0) {
            ...
            const { error: itemsError } = await this.supabase.from('purchase_order_items').insert(items);
    ...
    async findOne(tenantId, id) {
      ...
      `).eq('tenant_id', tenantId).eq('id', id).single();
        if (error) throw new _common.NotFoundException('Purchase Order not found');
        return data;
    }
    async update(tenantId, id, data) {
        ...
        const { error } = await this.supabase.from('purchase_orders').update({
            vendor_id: data.vendorId,
        ...
        if (data.items) {
            await this.supabase.from('purchase_order_items').delete().eq('po_id', id);
    ...
    async updateStatus(tenantId, id, status) {
        ...
        const { data, error } = await this.supabase.from('purchase_orders').update(updateData).eq('tenant_id', tenantId).eq('id', id).select().single();
        if (error) {
    ...
    async updateTracking(tenantId, id, trackingData) {
        console.log('Updating PO tracking:', {
            tenantId,
            id,
            trackingData
        });
    ...
    async delete(tenantId, id) {
        const { data: po, error: poError } = await this.supabase.from('purchase_orders').select('id, po_number').eq('tenant_id', tenantId).eq('id', id).maybeSingle();
        if (poError) {
            throw new _common.BadRequestException(poError.message);
        }
        if (!po) {
            throw new _common.NotFoundException('Purchase Order not found');
        }
        const grnBlock = await this.findBlockingGrnForPo(tenantId, id);
        if (grnBlock) {
            throw new _common.BadRequestException(`Cannot delete Purchase Order ${po.po_number} because GRN ${grnBlock.grnNumber} exists. Delete/void the GRN first.`);
        }
        const { error } = await this.supabase.from('purchase_orders').delete().eq('tenant_id', tenantId).eq('id', id);
        if (error) {
            // Foreign key violations are common here (e.g., GRN references); show a clearer message.
            if (error.code === '23503') {
                throw new _common.BadRequestException(`Cannot delete Purchase Order ${po.po_number} because it is referenced by other records (e.g., GRN). Remove related records first.`);
            }
            throw new _common.BadRequestException(error.message);
        }
        return {
    ...
    async findBlockingGrnForPo(tenantId, poId) {
        const tryFindInTable = async (table)=>{
            const { data, error } = await this.supabase.from(table).select('id, grn_number').eq('tenant_id', tenantId).eq('po_id', poId).order('created_at', {
                ascending: false
            }).limit(1).maybeSingle();
            if (error) {
    ...
    async generatePONumber(tenantId) {
        ...
        const { data } = await this.supabase.from('purchase_orders').select('po_number').eq('tenant_id', tenantId).like('po_number', `${prefix}%`).order('po_number', {
            ascending: false
        }).limit(1).single();
        if (!data) {
            return `${prefix}-001`;
    ...
    dataUrlToNodemailerAttachment(input) {
        const filename = this.sanitizeFilename(input.filename);
        const fileUrl = input.fileUrl;
        if (!fileUrl) {
            throw new _common.BadRequestException('Attachment fileUrl missing');
        }
        if (fileUrl.startsWith('data:')) {
            const commaIndex = fileUrl.indexOf(',');
            if (commaIndex === -1) {
                throw new _common.BadRequestException('Invalid data URL');
            }
            ...
            if (!isBase64) {
                throw new _common.BadRequestException('Unsupported data URL encoding (expected base64)');
            }
            return {
                filename,
    ...
    async sendPOEmail(tenantId, poId) {
        const po = await this.findOne(tenantId, poId);
        if (!po.vendor?.email) {
            throw new _common.BadRequestException('Vendor email not found');
        }
        ...
        if (uniqueItemIds.length > 0) {
            const { data: itemsByIdData, error: itemsByIdError } = await this.supabase.from('items').select('id, code, name, drawing_required').eq('tenant_id', tenantId).in('id', uniqueItemIds);
            if (itemsByIdError) {
                throw new _common.BadRequestException(itemsByIdError.message);
            }
        ...
        if (uniqueItemCodes.length > 0) {
            const { data: itemsByCodeData, error: itemsByCodeError } = await this.supabase.from('items').select('id, code, name, drawing_required').eq('tenant_id', tenantId).in('code', uniqueItemCodes);
            if (itemsByCodeError) {
                throw new _common.BadRequestException(itemsByCodeError.message);
            }
        ...
        const resolvedItemIds = Array.from(new Set(poItems.map((poItem)=>{
            const resolvedItem = (poItem?.item_id ? itemsById.get(poItem.item_id) : null) || (poItem?.item_code ? itemsByCode.get(poItem.item_code) : null);
            return resolvedItem?.id || poItem?.item_id || null;
        }).filter(Boolean)));
        const activeDrawingByItemId = new Map();
        if (resolvedItemIds.length > 0) {
            const { data: activeDrawings, error: activeDrawingsError } = await this.supabase.from('item_drawings').select('id, item_id, file_name, file_type, file_url, file_size, version, is_active').eq('tenant_id', tenantId).in('item_id', resolvedItemIds).eq('is_active', true).order('version', {
                ascending: false
            });
            if (activeDrawingsError) {
                throw new _common.BadRequestException(activeDrawingsError.message);
            }
        ...
        for (const poItem of poItems){
            ...
            attachments.push(this.dataUrlToNodemailerAttachment({
        ...
        if (missingCompulsory.length > 0) {
            throw new _common.BadRequestException(`Cannot send PO email. ACTIVE drawing missing for compulsory item(s): ${missingCompulsory.join(', ')}`);
        }
        const emailData = {
        ...
        await this.supabase.from('purchase_orders').update(statusUpdate).eq('tenant_id', tenantId).eq('id', poId);
        return {
            message: 'PO email sent successfully',
            recipient: po.vendor.email
        };
    }
    async sendTrackingReminder(tenantId, poId) {
        const po = await this.findOne(tenantId, poId);
        if (!po.vendor?.email) {
            throw new _common.BadRequestException('Vendor email not found');
        }
        ...
        return {
            message: 'Tracking reminder sent successfully',
            recipient: po.vendor.email
        };
    }
