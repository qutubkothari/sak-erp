...
export class ManagerService {
  ...
  async getPendingApprovals(tenantId: string, managerId: string): Promise<PendingApproval[]> {
    ...
    try {
      ...
      if (qcGrns) {
        const terminalStatuses = new Set(['REJECTED', 'CANCELLED', 'CANCELED', 'VOID', 'VOIDED', 'REVERSED']);
        const actionableQcGrns = qcGrns.filter((grn: any) =>
          grn.is_active !== false &&
          !terminalStatuses.has(String(grn.status || '').trim().toUpperCase()) &&
          (grn.grn_items || []).length > 0,
        );
        const userNames = await resolveUserNames(actionableQcGrns.map((grn: any) => grn.created_by));
        for (const grn of actionableQcGrns) {
          approvals.push({
            id: grn.id,
            number: grn.grn_number,
            type: 'QC',
