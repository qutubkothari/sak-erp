...
describe('ItemsService temporary R&D procurement items', () => {
  it('applies the R&D-only filter in the database so reusable TEMP items are not lost to pagination', async () => {
    const projectsService = { ensureSchema: jest.fn().mockResolvedValue(undefined) };
    const service = new ItemsService(projectsService as any);
    const query: any = {
      ...
      then: (resolve: (value: any) => void) => resolve({
        data: [{
          id: 'temp-1',
          tenant_id: 'tenant-1',
          code: 'TEMP-REUSABLE-1',
          name: 'Reusable prototype',
          is_active: true,
          is_rnd_item: true,
        }],
        error: null,
    ...
    (service as any).getCurrentStockTotals = jest.fn().mockResolvedValue({});

    const result = await service.findAll('tenant-1', undefined, false, false, {
      includeRnd: true,
      onlyRnd: true,
    });

    expect(query.eq).toHaveBeenCalledWith('is_rnd_item', true);
    expect(result).toEqual([
      expect.objectContaining({ code: 'TEMP-REUSABLE-1', current_stock: 0 }),
    ]);
  });
  ...
  it('preserves long technical descriptions while keeping the display name within its database limit', async () => {
    const service = new ItemsService({} as any);
    const longDescription = `Motor - ${'technical specification '.repeat(14)}`;
    const duplicateQuery: any = {
      select: jest.fn(() => duplicateQuery),
      eq: jest.fn(() => duplicateQuery),
      ilike: jest.fn(() => duplicateQuery),
    ...
    const itemInsertQuery: any = {
      ...
      select: jest.fn(() => itemInsertQuery),
      single: jest.fn().mockResolvedValue({
        data: { id: 'item-long', code: 'TEMP-OEM-LONG', description: longDescription },
        error: null,
      }),
    };
    let itemCalls = 0;
    ...
    await service.createRndTemporary('tenant-1', 'user-1', {
      identifier: 'OEM-LONG',
