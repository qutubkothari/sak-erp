...
stage=/tmp/mizantra-mrp-time-buckets-20260830
backup=/root/sak-deploy-backups/mizantra-mrp-time-buckets-$(date +%Y%m%d-%H%M%S)
...
if [ "$verified" != true ]; then
  ...
  pm2 restart sak-api-test sak-web-test
  echo 'MRP time-bucket deployment failed health checks; previous test runtime restored.' >&2
  exit 1
fi

echo "Mizantra TEST MRP time buckets deployed and verified. Backup: $backup"
