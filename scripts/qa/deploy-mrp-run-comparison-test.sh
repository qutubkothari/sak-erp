...
stage=/tmp/mizantra-mrp-run-comparison-20260830
backup=/root/sak-deploy-backups/mizantra-mrp-run-comparison-$(date +%Y%m%d-%H%M%S)
...
if [ "$verified" != true ]; then
  ...
  pm2 restart sak-api-test sak-web-test
  echo 'MRP run-comparison deployment failed health checks; previous test runtime restored.' >&2
  exit 1
fi

echo "Mizantra TEST MRP run comparison deployed and verified. Backup: $backup"
