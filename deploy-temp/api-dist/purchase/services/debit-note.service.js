...
Object.defineProperty(exports, "DebitNoteService", {
    enumerable: true,
    get: function() {
...
let DebitNoteService = class DebitNoteService {
    ...
    async findAll(tenantId, filters) {
        let query = this.supabase.from('debit_notes').select(`
        *,
    ...
    async findOne(tenantId, id) {
        const { data, error } = await this.supabase.from('debit_notes').select(`
        *,
      ...
      `).eq('tenant_id', tenantId).eq('id', id).single();
        if (error) {
            ...
            throw new _common.NotFoundException('Debit note not found');
        }
    ...
    async approve(tenantId, id, userId) {
        const { data, error } = await this.supabase.from('debit_notes').update({
            status: 'APPROVED',
            approved_by: userId,
    ...
    async updateReturnStatus(tenantId, debitNoteId, itemId, returnStatus, disposalNotes) {
        const validStatuses = [
            'PENDING',
            'RETURNED',
            'DESTROYED',
        ...
        const { data, error } = await this.supabase.from('debit_note_items').update({
            return_status: returnStatus,
            return_date: returnStatus !== 'PENDING' ? new Date().toISOString().split('T')[0] : null,
            disposal_notes: disposalNotes || null
        }).eq('debit_note_id', debitNoteId).eq('id', itemId).select().single();
        if (error) throw error;
    ...
    async findByGrn(tenantId, grnId) {
        const { data, error } = await this.supabase.from('debit_notes').select(`
        *,
        debit_note_items(
          *,
      ...
      `).eq('tenant_id', tenantId).eq('grn_id', grnId).order('created_at', {
            ascending: false
        });
        if (error) throw error;
        return data;
    ...
    async getVendorPayables(tenantId) {
        ...
        net_payable_amount,
        vendor:vendors(id, name, code)
      `).eq('tenant_id', tenantId).eq('status', 'COMPLETED').gt('net_payable_amount', 0);
        if (error) throw error;
        // Group by vendor
    ...
    async sendEmail(tenantId, id) {
        ...
        let itemsHtml = '';
        debitNote.debit_note_items?.forEach((item)=>{
            itemsHtml += `
        <tr>
    ...
    `;
        ...
        return {
            message: 'Debit note sent successfully'
        };
    }
    // Record payment against a GRN
    async recordPayment(tenantId, grnId, paymentData) {
        // Get GRN with current payment info
        const { data: grn, error: grnError } = await this.supabase.from('grns').select('id, net_payable_amount, paid_amount, payment_status').eq('id', grnId).eq('tenant_id', tenantId).single();
        if (grnError || !grn) {
            throw new _common.NotFoundException('GRN not found');
        }
        ...
        // Update GRN
        const { error: updateError } = await this.supabase.from('grns').update({
        ...
        const { data: verifyGrn } = await this.supabase.from('grns').select('paid_amount, payment_status').eq('id', grnId).single();
