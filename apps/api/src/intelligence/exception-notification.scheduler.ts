...
export class ExceptionNotificationScheduler {
  ...
  private async notifyTenant(tenantId: string) {
    ...
    for (const item of data || []) {
      ...
      if (!stage) continue;
      rows.push({ tenant_id: tenantId, module: 'OPERATIONS', document_type: 'MIZANTRA_EXCEPTION', document_id: item.id, document_number: item.source_key, channel: 'IN_APP', direction: 'OUTBOUND', recipient: this.roleFor(item.source_type || item.title), subject: `${stage === 'ESCALATION' ? 'Escalation' : 'Action required'}: ${item.title}`, message_preview: `${item.explanation || ''} Recommended: ${item.recommendation || 'Review the source record.'}`.slice(0, 1000), delivery_status: 'DELIVERED', dedupe_key: `MIZANTRA_EXCEPTION:${item.id}:${stage}:${day}`, metadata: { exception_id: item.id, priority_score: item.priority_score, severity: item.severity, stage, route: item.source_route, deduplicated: true } });
    }
