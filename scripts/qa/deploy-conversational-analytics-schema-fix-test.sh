...
stage=/tmp/mizantra-conversational-analytics-schema-fix-20260830
backup=/root/sak-deploy-backups/mizantra-conversational-analytics-schema-fix-$(date +%Y%m%d-%H%M%S)

test "$PWD" = "$app"
test "$(readlink -f "$app")" = "$app"
test -f "$stage/apps/api/src/intelligence/conversational-analytics.service.ts"
...
if [ "$verified" != true ]; then
  ...
  pm2 restart sak-api-test
  echo 'Schema hotfix failed; prior Mizantra API files restored.' >&2
  exit 1
fi

pm2 save
echo "Mizantra analytics schema hotfix deployed and verified. Backup: $backup"
